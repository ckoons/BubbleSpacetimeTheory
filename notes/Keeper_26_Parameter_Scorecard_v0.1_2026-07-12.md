# BST 26-Parameter Scorecard — v0.5, 2026-07-15 EOD (fermion sector, honestly tiered)

*v0.5 (2026-07-15 EOD) — THE MASS/MIXING SPLIT, honestly re-stated (K704): **the MASSES are done** — all 20 banked/identified-strong parameters are the singular values Σ of M=UΣV† (radial norms, pinned by the E-ladder grounds + spin weight E₀=2). **The 6 MIXING params (V_us, V_ub, δ_CKM, PMNS θ12/θ23/δ_PMNS) are a DISTINCT geometric object** — the relative eigenVECTOR orientation U_L†U_L' between sectors — which is NOT a function of the norms (the finish-line "run" gave PMNS≈0 because it used radial-only positions; F498 proved real single-direction positions → J=0 identically). The fix (not a wall): build the eigenvector-orientation/texture object from maps we already have — the **refraction rotation** (up/down, F548 index N_c/rank → small CKM; Gatto bridge sinθ_C≈√(m_d/m_s)) and the **d=5→d=4 projection + Majorana locus** (ℓ/ν → large PMNS, F413). Tomorrow: Lyra's refraction-rotation swing at the Cabibbo. Also v0.5: **α fully closed** (4π = descent's 3D Coulomb solid angle, K701); **muon 4→5 banked** (K698); **curvature forward** (n_C/N_max, α 0.0004% + Higgs 0.02%); **m_τ forward**, **m_b=m_t/42**, **m_ν2∝√N_c**, **m_u √(3/14) Dirac-optics-grounded**.*


*v0.4 (2026-07-15): **α COMPLETE** (Casey ruling). **Majorana nature BANKED** (Cal co-signed K695; pred_004 → 1–4 meV 0νββ floor). **m_H 0.02%** via α's own curvature (Grace: λ=(1/8)(1+n_C/N_max); identified-strong, Engine-C audit=proof). **m_τ forward** (bulk Weyl 49·71 evaluated + boundary √π). **m_b = m_t/42** (b's home is the top). **m_ν2 ∝ √N_c** grounded (retires 7/12; bulk √N_c mirror of tau's boundary √π). **m_u** strong-lead (√(3/14) block-reflection; premise fails target-innocence → held). **Muon HOLD 4** (residue √rank·π² derived, c_S=1 gated on N_c/rank cancellation, K358/consistency). Mixing sector: ONE FK-overlap-integral from unblocking (Lyra→Grace). See K693/K694/K695.*


*v0.3 changes (2026-07-14): α — the forward route CONVERGED to the CHARGE-COUNT route and is now **forced-adjacent** (K678/K679, F532). The boundary 27×27 overlap RAN and came out ≠ I (35× charge-block spread) → the norm-weighted/orthonormality milestone is RETIRED; the ≠ I result FAVORS the charge-count (a norm-weighted α would inherit the 35× mess, α=1/137 doesn't). Democracy verified as an SO(7)/Q⁵ fact (equal-norm on the compact dual, broken on ∂ by {1:7/2:35/8}). **α = 1/(count) forced-adjacent** (norm-independence from T2470 proved + norm-route eliminated); **α = 1/137 exactly gates on KEYSTONE A** (the U(1) gauge-coupling normalization = unity, AND the count = 137 = 27·n_C+rank — two sub-steps, K679). Higgs λ=1/8 is a PASSENGER on Keystone A (closes for free / falls with it). Tier stays IDENTIFIED — but the route is now clean and single-step, not Wyler. v0.2: retired Wyler-1/4; two-piece framing; F525 matrix core; Big Bang handle.*
...
| 3 | m_τ/m_e | **NEAR-FORCED (FORWARD-ADDITIVE — upgraded K690, supersedes K682)** | 49·71 − √π, **forward-additive** (Elie 4665): **49·71 = g²(g+2^{C_2}) = BULK Weyl count** (F115); **−√π = the odd-3-ball BOUNDARY correction** (Γ(3/2)=√π/2) — **FORWARD, not fit** (exists from the 3-ball regardless of the residual; resolves K682's "reverse-engineered" concern) | 3477.23 = obs 0.0%. Discrete bulk count + continuous boundary 3-ball. Remaining: EVALUATE the bulk Weyl sum forward [Engine C]; bulk-Weyl currently F115-identified. |
...
| # | Param | Tier | BST | vs obs |
|---|---|---|---|---|
| 17 | θ12 | **IDENTIFIED** (demoted from BANKED — Grace/K678) | matrix element \|U_e2\|² = N_c/(rank·n_C) = **3/10** (0.06%); the "42/137" was a sin²θ12 near-coincidence with 27/88 | ≥4 competing forms were the tell; 3/10 is the mechanism form, carry it |
...
| # | Param | Tier | BST | note |
|---|---|---|---|---|
| 21 | α | **NEARLY PROVEN (identified-strong)** — count derived + democracy computed + scale falsifier PASSED; residue = the 4π convention (definitional) | α⁻¹ = 137 + 0.036. 137 = compact-boundary mode-count (derived, over-determined 3 ways: 27·n_C, 2^g+N_c², M_7+10). Democracy computed (S¹ Haar unit-norm winding = charge, T2470). **Scale falsifier PASSED (K681, Grace): 1/α=137, no stray 2π/c_FK/4π** — 2π drops (Haar unit-norm), c_FK is S⁴-matter not gauge, 4π has no free knob (integer charge). The "why" (K680/BST_Why_Alpha): finite=COMPACT boundary → finite capacity → α=1/capacity (why SM can't, BST can); primeness forces the count+correction shape. **Physics-framed: target-innocent falsifiable prediction that SURVIVES.** Residue to fully-derived: the 4π convention resolves definitionally, not by independent derivation. | the deep keystone — now framed as TWO CLEAN PIECES (not the Wyler Vol^(1/4), a retired 4-reading numerology trap): **(137)** the integer via the mode-count, Lemma A; **(0.036)** a separate boundary-curvature correction (κ_Bergman=−n_C; 1.4% principled, Grace). **Lemma A's combinatorial core CLOSED by matrix (F525): the naive-15 is not conformally closed → the boundary must carry 27.** Residual: does the boundary realize the conformal SO(5,2) action at level-rank (ρ-shift convergence). **NEW physical handle (2y/2z, core confirmed by 3 independent matrix witnesses): the generator forcing 15→27 is the non-compact boost J(V₅→V₂) — the SAME generator whose opening makes time (Euclidean→Lorentzian, Casey's Big Bang conjecture); so the boundary is conformal because the substrate opened via it.** *Retracted (do not cite): "α is a square ⇒ degree=rank" (F522); the trace=dim demotion (4643); n_C/N_max as a V_Wyler series; the Wyler-1/4 route.* α does not move from IDENTIFIED until the residual + the 0.036 clear + Keeper/Cal sign-off. |
