...
## ★ G4 — THE NEUTRINO RECONCILIATION (task #76 open part, 2026-08-12): small-CKM/large-PMNS split is FORCED by the SOURCE structure
The two mixings share the SAME cross-parity skeleton (G1), so the small-vs-large split cannot come from the skeleton — it comes from the mass-diagonalizing rotations, which come from the SOURCES:
- **Linear-algebra fact:** CKM = U_up†·U_down, PMNS = U_e†·U_ν. Same source → aligned U's → they cancel → mixing ≈ 1 (small). Different sources → uncorrelated U's → mixing = O(1) angles (large).
- **QUARKS — small CKM, FORCED:** up & down are BOTH Dirac overlaps on the SHARED rank-1 condensate O (F603, flagship — O = SO(5) vector, the one condensate direction). So U_up ≈ U_down ≈ U_O → CKM ≈ 1 + (δ_d−δ_u) = 1 + small. The shared O forces near-diagonal CKM.
- **LEPTONS — large PMNS, FORCED:** charged leptons are Dirac from O (like quarks), but the neutrino mass is MAJORANA from a DIFFERENT source φ (T1985: no ν_R → the Dirac operator ν̄_L ν_R cannot exist → mass comes from a distinct Majorana operator, φ ≠ O). U_ν diagonalizes φ, uncorrelated with U_O → PMNS = U_O†·U_φ = mismatch of two unrelated bases → LARGE.
- **Numerical illustration (mechanism, not the angles):** shared-O quark toy → CKM off-diag ~0.23 (small); different-φ neutrino toy → PMNS off-diag ~0.59 (large). Swapping ν-source to O collapses PMNS to small — confirming the LARGE is caused by O≠φ, not by tuning.
- **Corroboration (target-innocent):** the ν even-grid starts at k=0 (massless m₁=0, T1985/pred_003) with NO ν_R partner — the extra unpaired zero-mode is the same "different structure" fact, a second forced reason U_ν is unconstrained by U_e.

**TIER:** the small-CKM/large-PMNS SPLIT is STRUCTURAL-DERIVED — forced by (shared source O for quarks, F603) + (different Majorana source φ for ν, T1985 no-ν_R). This is the "one Peirce statement" (Cal §137) made concrete: same skeleton, opposite mixing size, because the sources differ. **The exact angles ride the radial FK/Majorana overlaps — Elie's blind fire, NOT claimed here.** #76 neutrino-reconciliation: CLOSED at the structural (split-forced) tier.
