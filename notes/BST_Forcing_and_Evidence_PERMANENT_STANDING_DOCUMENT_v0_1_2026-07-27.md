...
- **Node 3 — chirality.** Domain chirality (Dolbeault ±4, n_C odd) is a THEOREM; observed 4D parity is derived-CONDITIONAL on an un-computed gravi-weak input (the clean "oddness→parity" lock was refuted, F642). Target-innocent (g=7 fixed 2022) but incomplete.
...
**Selector-1 detail (the fulcrum — occupancy derivation, K944/K945):**
- DERIVED: strata count = rank+1 = 3 (Korányi-Wolf, uniform). Upper bound "no 4th generation" (≤3) — three independent routes (rank-2 Wallach 2 points / matryoshka terminates / Q⁵ no h⁷); LEP-confirmed N_ν=3.
