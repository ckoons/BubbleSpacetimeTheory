...
- **Color = the Peirce off-diagonal V₁₂** of the type-IV Jordan algebra, dim N_c = 3; leptons = the color-neutral frame. *(BST T2511; the lepton-generation-count result.)*
...
*Status: v1.6, external-ready (DE corrected per K1040). Cal cleared + 2 polishes ("≈𝟙" clarified; "one shared condensate" tightened). Dark energy fixed to BST's derived value: **w = −1 (cosmological constant, from the fixed bulk volume; deviation → 0)** — supersedes the earlier −0.949 (T2079). Consequence: Σm_ν = 0.0588 eV is a genuine edge kill (BST is committed to w = −1, no dynamical-DE escape); DE shown as its own row (BST w = −1 vs DESI's w₀ ≈ −0.84 hint = a live test). Ready for Cal's consistency re-read + Casey's GO. The qualitative theorem (CKM small / PMNS large) is Derived from gauge invariance + the g = 7 Majorana lock; exact mixing values are separate per-sector results, honestly tiered (Section 5). Falsifiability (Section 6) checked against 2024 data with model-dependence stated: **Σm_ν ≈ 0.0588 eV is a live kill under ΛCDM** (edge of the DESI bound 0.064 eV) **and a consistency check under dynamical dark energy** (bound ~0.16 eV, mildly favored by the same data) — not headlined beyond what the model supports; mass ordering (JUNO, ~2030); 0νββ signal below current reach (a null does not refute); θ₂₃ maximal sits between the octants (NuFIT-6.0 NO best-fit is lower-octant 43.3°), robust and consistent — and 4/7-upper would be on the wrong side; δ_CP near-180° agrees within ~1σ. Scope and limitations stated (Section 6.5). Repo-internal; pending Cal's final clearance and Casey's GO; not pushed or distributed.*
