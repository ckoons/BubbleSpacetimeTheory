...
Charge=ℂ (all n) and color=ℝ (**n≥5**, re-derived: SO(n−2) non-abelian ⟺ n−2≥3 ⟺ n≥5; n=4 is the SO(2)-abelian exception, color→ℂ) are n-invariant on the physical range; only the spinor moves (Bott mod-8). So the color/hypercharge/size/Higgs rows ARE swept — the census leg is rigorous across the family on **n≥5**, not just the spinor row. The census separator D_IV⁷ (spinor ℝ → EW size 0+1+0=1) is concrete. This scope note is SUPERSEDED by `grace_laneB_HFn_construction_bank_2026-08-20.md`. The self-correction (census separator = D_IV⁷, not D_IV⁴) stands and is now load-bearing.
