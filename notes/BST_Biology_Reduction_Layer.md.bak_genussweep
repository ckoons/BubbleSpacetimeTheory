...
3. **D_IV^5 Geometry** — a property of the bounded symmetric domain (root system B_2, five integers, Bergman kernel, Shilov boundary, volume)
...
| N2 | Weyl group W(B_2) | The 8 symmetries of the B_2 root system |
...
|------|----------|---------------|
| G1 | B_2 root system | The pattern of roots: short roots (multiplicity N_c=3) and long roots (multiplicity 1) |
...
|------|------|---------|---------------|-----------------|-------|---------------|
| T333 | Genetic Code Structure | S1: Bounded enumeration | N1: Lambda^k(C_2) gives 64 codons, C(6,3)=20 amino acids | G6: Sp(6) representation ring; G1: B_2 root hierarchy sets wobble | 0 | Count how many words you can make from 4 letters in groups of 3. The answer (64 to 20) comes from choosing 3 things out of 6 on a specific geometric shape. |
...
| T445 | Genetic Code Forcing | S2: Channel capacity forcing | N5: Z_3 reading frame closure; N1: Lambda^3(6)=20 | G1: B_2 root lengths force wobble hierarchy; G2: five integers fix each step | 0 | The genetic code is not an accident. Each step is forced: rank gives 4 bases, C_2 gives 6-bit codons, N_c gives triplet length, and 20 amino acids follow. The geometry leaves no room for a different code. |
| T453 | Code Invariance Under Stress | S3: Error correction robustness | N9: C_2=6 bit information quantum preserved under perturbation | G1: B_2 root hierarchy maintains wobble protection | 0 | Even under radiation, heat, or chemical stress, the genetic code does not change. The error correction built into the wobble position (from the root system) is robust enough to survive anything biology encounters. |
| T463 | Annotated Codon Information Budget | S6: Rate-distortion / water-filling | N9: C_2=6 bits per codon; positions carry unequal info (2,2,2 raw but ~1.6,1.8,0.6 effective) | G1: B_2 short vs long roots map to high-info vs low-info positions | 0 | Each codon carries 6 bits, but the three positions are NOT equal. Positions 1 and 2 carry the important stuff; position 3 (wobble) absorbs errors. The root system tells you which position is which. |
| T516 | Genetic Code Parameters | S1: Bounded enumeration + S3: Error correction | N5: Z_3 codon length; N4: 2^rank=4 bases; N3: C(6,3)=20; redundancy 64/20 near pi | G2: Five integers determine every parameter; G1: B_2 root system | 0 | All the numbers in the genetic code — 4 bases, 3-letter codons, 64 total, 20 amino acids, 3 stops — are fixed by five integers from geometry. The redundancy ratio (64/20) is close to pi, which means the code is near the theoretical minimum error rate. |
...
| T476 | Protein Folding Geometry | S3: Error correction budget | N7: Helix spacings {N_c, 2^rank, n_C} = {3, 4, 5}; alpha-helix = N_c x C_2 / n_C = 3.6 | G1: B_2 root lengths determine helix hydrogen bond distances | 0 | The three types of protein helix have hydrogen bond spacings of exactly 3, 4, and 5 residues — the three core BST integers. The dominant helix (alpha) has pitch 3.6 = 18/5 = (N_c x C_2)/n_C. |
...
| T443 | Environmental Management Completeness (extended) | S1: Bounded enumeration | N7: 20 = 4 x n_C; 4 = 2^rank categories, 5 = n_C subcategories | G1: 4 positive roots of B_2 give 4 categories; G4: n_C=5 compact dims give subcategories | 0 | 20 environmental problems = 4 categories (from the 4 positive roots) times 5 sub-problems each (from the 5 compact dimensions). Same derivation as 20 amino acids. Minimum viable life needs 2^rank = 4 solutions. |
| T522 | Environmental Problems | S1: Bounded enumeration + S9: Zero-sum budget | N4: 2^rank=4 categories x n_C=5 each = 20; minimum organism needs N_c=3 | G1: B_2 positive roots; G4: Shilov boundary; G2: five integers | 0 | Exactly 20 things an organism must handle, in 4 groups of 5. Organ systems are the forced answers. Any viable organism needs at least 3 independent solutions. Casey's insight: the problems come before the solutions. |
...
| T372 | Molecular Haldane Number | S3: Error correction (maximum correctable distance) | N2: |W(B_2)| = 8 = 2^N_c; Golay distance | G1: B_2 Weyl group determines the maximum error correction radius | 0 | The maximum number of mutations a molecular code can correct per generation is 8 = 2^N_c = the size of the Weyl group of B_2. This is the molecular Haldane limit. |
...
| T481 | Ion Channel Architecture Universal | S7: Threshold selection (all-or-nothing gating) | N4: 2^rank=4 domains x C_2=6 segments; HH gating m^3h = N_c+1 = 2^rank | G1: B_2 root system; G2: |V_rest|/V_peak = g/N_c = 7/3 | 0 | Voltage-gated ion channels have 4 domains (2^rank) of 6 transmembrane segments (C_2) each. The Hodgkin-Huxley gate has 3 activation + 1 inactivation = 4 = 2^rank particles. Resting-to-peak voltage ratio = 7/3 = g/N_c. |
