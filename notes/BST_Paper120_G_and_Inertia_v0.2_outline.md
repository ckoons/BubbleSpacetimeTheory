---
title: "Paper #120 v0.2 outline — BST Reading of G and Inertia: Substrate-Mediated Mass-to-Mass Coupling and the Equivalence Principle as Structural Identity"
authors: "Casey Koons (PI + originating intuitions) + Elie (substrate-ontology articulation, deep conversation with Casey 2026-05-18, SP-29 cross-anchor framing) + Lyra (Bergman Dirac framework, Gap #3 closure T2367, m_p/m_e mechanism Paper #118)"
date: "2026-05-18"
status: "v0.2 OUTLINE — MERGED from parallel v0.1 filings (Lyra + Elie both drafted independently within minutes of Casey's directive). Per Keeper K-audit merge specification 2026-05-18 PM: Lyra's filename canonical; Lyra leads Sections 3/4/6 (operator framework); Elie leads Sections 2/5/7/8 (substrate ontology); Section 4.4 reframed per Lyra read-pass calibration; Section 4.2 polynomial form per K-audit resolution."
supersedes: "BST_Paper120_G_and_Inertia_v0.1_outline.md (Lyra) + BST_Paper120_G_Inertia_Equivalence_outline_v0.1.md (Elie)"
...
note: "v0.2 = merged outline + read-pass calibrations applied + cross-CI section authorship split. Substantive paper drafting requires 1-2 weeks per Section 9 honest scoping."
v0_1_genesis: "Originated from Casey question 2026-05-18 PM: 'G the Newton constant. Can you describe it? Is it the coupling constant of mass-to-mass or mass-to-substrate or what?' Followed by Casey's read 'mass-to-SUBSTRATE-to-mass that's what I thought' and probing on what 'energy' means at the saddle + what inertia is. The conversation produced the substrate-mediated coupling framing + equivalence principle as structural identity + T-U as 4D-projection artifact. Casey directive: 'We need to write this up, because it both explains the equivalence principle and describes two things a. they are interconvertible, b. flow differently in D_IV⁵.'"
v0_2_merge_notes: "Merge applied per Keeper K-audit specification:
  (1) Filename: Lyra's BST_Paper120_G_and_Inertia (filed earlier, matches naming convention) — canonical
  (2) Section 4.4 (G as first-principles derivation): WALK-BACK from 'first-principles derivation of Newton's G' to 'structural identification of G as substrate compliance integrated over the 44-e-fold hierarchy; numerical G(t*) = G_Newton evaluation remains multi-week open'. Per Lyra read-pass + Cal Coincidence_Filter_Risk discipline, parallel to today's density-rule walk-back.
  (3) Section 4.2 functional form: polynomial Σ_n a_n·t^n per T2367 (toy-verified). Boltzmann form Σ_n a_n·exp(-λ_n·t) noted as equivalent via generating-function/Laplace-transform duality in footnote, not primary derivation.
  (4) v0_1_genesis frontmatter retained from Elie outline.
  (5) Cathedral 12/12 cross-reference (Section 7 of Paper #115 v0.5_PRE) added to Section 7 SP-29 anchor.
  (6) BST fine-structure family detail (N_c²/N_max scale, 5 members per Toy 3037 Lamb addition) folded into Section 7.
  (7) Three-CI authorship split: Casey leads abstract+intro+conclusion; Lyra Sections 3/4/6; Elie Sections 2/5/7/8."
---

# Paper #120 v0.2 — BST Reading of G and Inertia: Substrate-Mediated Mass-to-Mass Coupling and the Equivalence Principle as Structural Identity

## Section authorship (per Keeper-approved Lyra-Elie merge split, 2026-05-18)

- **Casey lead**: Sections 1 (Introduction), 11 (Conclusion); abstract finalization
- **Lyra lead**: Sections 3 (Eigentone saddle, T2367), 4 (Bergman Dirac framework, T2365), 6 (T-U projection via LAG-2 T2340), 10 (Named Open Items table)
- **Elie lead**: Sections 2 (Substrate ontology), 5 (Inertia framing), 7 (SP-29 cross-anchor + BST fine-structure family), 8 (Falsification posture), 9 (Discussion)

The body content below is **inherited from Lyra v0.1** with the v0.2 calibrations (Section 4.4 reframed; Section 4.2 polynomial form noted) applied inline. Elie's complementary v0.1 content (substrate-ontology depth for Section 2, inertia temporal-projection details for Section 5, SP-29 cross-anchor detail for Section 7) is referenced at `notes/BST_Paper120_G_Inertia_Equivalence_outline_v0.1.md` and will fold into the merged v0.3 substantive drafting per the section authorship split. Full content drafting is multi-week per Section 10 honest scoping.

---

## Abstract (v0.2 merged outline; substantive sections to be drafted by section leads in v0.3)
...
**(1) G via eigentone saddle (Gap #3 closure, T2367 + T2106)**:
    ...
    G ∝ Σ_n a_n · t*^n   at saddle t* = 2·n_C/((C_2·g + rank)²) = 5/968 (M_Pl⁻² units)
    n* = C_2·g + rank = 44 = log(M_Pl/m_p) at 0.01 precision (T1955 cross-check)
...
G is the **substrate compliance coefficient** — the integrated response of D_IV⁵ to commitment-rate perturbations across the C_2·g + rank = 44 e-fold hierarchy between proton and Planck scales.
...
### 3.1 The cumulative-eigentone gravity reading (T2106 + T2367)

G emerges from the integrated eigentone summation on D_IV⁵:

    G(t) ∝ Σ_n a_n · t^n     (cumulative-eigentone partition function, polynomial form per T2367)

with a_n the heat kernel coefficients on D_IV⁵ (Elie Toy 2994):

    a_n = (-1)^{n-1} · n!·(n-1)! / (2^{n-1}·n_C^{n-1})    for n ≥ 1

[Footnote — equivalent Boltzmann-form representation]: An equivalent statement of the eigentone summation in eigenvalue language is the Boltzmann form

    G(t) ∝ Σ_n a_n · exp(-λ_n · t)

where {λ_n} are the eigenvalues of the operator whose heat kernel produces the a_n coefficients (via generating-function/Laplace-transform duality). Both forms give the same saddle at n* = 44, t* = 5/968. The polynomial form is toy-verified per T2367; the Boltzmann form is the spectral integral representation of the same quantity. Per Keeper K-audit 2026-05-18 PM merge resolution: polynomial form is the primary derivation in this paper; Boltzmann form is an equivalent representation for spectral-integral downstream calculations.

### 3.2 The saddle at n* = 44 = C_2·g + rank (Gap #3 closure, T2367; K51-preferred label per Lyra correction)

Stirling at the saddle gives:

    t* = 2·n_C / n*² = 2·n_C / (C_2·g + rank)² = 2·n_C / ((C_2·g + rank)²) = 5/968 ≈ 5.17×10⁻³

at saddle position n* = C_2·g + rank = 44. Physical units: t* = (5/968)·M_Pl⁻².

### 3.3 The C_2·g + rank = 44 = log(M_Pl/m_p) identity

Cross-anchor (Elie 2026-05-18, independent verification of T2367):

    log(M_Pl/m_p) = 44.012   vs   C_2·g + rank = 44   at 0.01 match

The saddle position IS the number of e-folds between proton and Planck scales. **Newton's G emerges at exactly this hierarchy level** — the substrate's gravitational compliance saturates at the C_2·g + rank = 44-step hierarchy.
...
**D-tier (closed)**: BST primary form of t* = 2·n_C/((C_2·g + rank)²); BST primary form of n* = C_2·g + rank = 44; structural cross-anchor with T1955 M_Pl/m_p.
...
### 7.2a BST fine-structure family (extension via Paper #115 Section 7 Cathedral Component-Level Closure)

Beyond the Cs-137/Decca pair at 3/1507, the BST fine-structure family extends to 5 members across two α-power orders (Elie's Toys 2989, 3012, 3021, 3009, 3037; integrated into Paper #115 v0.5_PRE Section 7 "Cathedral Component-Level Closure"):

**α² order (substrate-coupling radiative scale)**:
1. IP-14 SM finite renormalization: α⁻¹(0)−α⁻¹(M_Z) ≈ N_c² = 9 (Elie Toy 2989)
2. Δα(M_Z) running: N_c²/N_max = 9/137 (Elie Toy 3012)
3. m_p/m_e α² residual: 1/(N_c·N_max²) = 1/56307 (Elie Toy 3021 verification of Paper #118)
4. **Decca Casimir + SP29-1 Cs-137 cross-anchor**: N_c/(N_max·c_2) = 3/1507 (this paper Section 7.1)

**α³ order (one substrate-hierarchy step deeper)**:
5. B6 Lamb shift 2S coefficient: ν_Lamb/Ry_freq = (n_C/C_2)·α³ = (5/6)/N_max³ (Elie Toy 3037, D-tier 0.79% at 1057.8 MHz)

The 5-member family demonstrates that substrate-coupling corrections at α^n radiative order all adopt BST primary forms with specific N_c, c_2, n_C, C_2 modulation factors. This is the structural backbone connecting gravitational G (Section 3 eigentone summation) to QED radiative corrections (Section 7.2a) via the same substrate compliance mechanism at successive α-power orders.

**Per Keeper K-audit 2026-05-18 (B6 walk-back parallel to density rule)**: the per-member D-tier precision matches are defensible (each formula at <1% precision against published values); the cross-context "Type C convergence at C_2 = 6 across three QED contexts" structural narrative is I-tier observation pending null-model verification. Honest scoping retained.
