...
| **Bulk-color SU(3) mechanism** (NEW — Lyra structural finding) | **OPEN** — load-bearing | **SU(3) ≠ SO(5)** as rank-2 Lie algebras (A₂ vs B₂, different Cartans/Coxeter), so color CANNOT come from K = SO(5)×SO(2). Must come from the **bulk** (non-compact directions of SO(5,2) acting on the interior) or be **counting-not-symmetry** from h^∨=3. **Same structural item as route-(II)'s two-structures burden for generations** — one mechanism would close both (Lyra's "unforced unification"). Joint gate: bulk-color resolution unlocks (a) quark/gauge per-particle layer of the Periodic Table AND (b) route-(II) generation mechanism. Elie E7 (mult(spinor in spinor³) = 3, B₂-specific) is a candidate STRUCTURAL anchor for the spinor-tower side of generations. |
