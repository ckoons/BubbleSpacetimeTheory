---
title: "Paper A v0.8 — The Physical (SU(3)) Yang–Mills Mass Gap via D_IV⁵ Spectral Geometry. Companion to Paper B. v0.8 = v0.7 + the §570 CANDIDATE floor-lift (Section 2a, clearly tiered, NOT banked). The banked floor stays EXISTENCE (Δ>0 from a bounded domain, geometry-fixed, no free dimensionless parameter beyond the ruler R). NEW candidate: by §570 (energy ≠ mass), the pure-time modes (0,m) on the real boundary (S⁴×S¹)/ℤ₂ are ENERGIES not masses, so the MASS gap is the lightest SPATIAL-carrying ℤ₂-even mode (1,1) = 4 + 1/R² > 4 (= 5 at R=1) — because the ℤ₂ forbids a purely-spatial rest-mass (k=1,m=0 is odd), so a k=1 spatial state must carry at least one time unit m=1. The deep insight: the ℤ₂ FORBIDS A PURE REST-MASS — every massive state carries one unit of time (a candidate BST statement about mass). This would return 'gap > 4' with correct meaning, lifting the floor, IF Grace ⊕ Elie confirm the mass/energy split on the real boundary. Carried over from v0.7: bare-S⁴ '4' ℤ₂-projected-out; three geometry-fixed opens (adjoint conformal weight Δ, lowest ℤ₂-even mode, radius R); '6' = conformal/identity, '7' = genus; glueball separate-and-valid (on C₂^int, not λ₁), out of the room. Package intro HELD. Count HOLDS 4."
...
status: "v0.8 — existence floor banked + §570 CANDIDATE floor-lift (Section 2a, tiered, NOT banked). Banked floor = EXISTENCE (Δ>0 from bounded domain, no free dimensionless parameter beyond ruler R). Candidate: §570 (energy≠mass) → pure-time modes (0,m) are energies, so the MASS gap is the lightest spatial-carrying ℤ₂-even mode (1,1) = 4 + 1/R² > 4 (=5 at R=1); the ℤ₂ forbids a pure rest-mass (k=1,m=0 odd → a k=1 state must carry m=1), i.e. every massive state carries one unit of time. Returns 'gap > 4' with correct meaning IF Grace ⊕ Elie confirm the mass/energy split on the real boundary. Carried: bare-S⁴ '4' ℤ₂-projected-out; three opens (adjoint Δ, lowest ℤ₂-even mode, R); '6'=conformal/identity, '7'=genus; glueball separate-valid (C₂^int, not λ₁), out of the room. Package intro HELD. Count HOLDS 4. INTERNAL. Nothing pushed; CP existence-only."
---

# The Physical Yang–Mills Mass Gap via D_IV⁵ Spectral Geometry

*Paper A of the two-paper package (companion: Paper B, substrate uniqueness). v0.8 — existence floor banked; §570 candidate floor-lift added (Section 2a, tiered): the mass gap is the lightest spatial-carrying ℤ₂-even mode (1,1) = 4 + 1/R² > 4, because the ℤ₂ forbids a pure rest-mass — every massive state carries one unit of time. Carried from v0.7: bare-S⁴ "4" ℤ₂-projected-out; three geometry-fixed opens; glueball separate-and-valid, out of the room; package intro HELD.*
...
### 2a. A candidate floor-lift, if the mass/energy split holds (§570 — CANDIDATE, not banked)

The floor stays *existence*. But §570 (energy ≠ mass) suggests the floor may honestly lift, and we record it **clearly tiered as a candidate, not banked**.

**The reading.** On (S⁴×S¹)/ℤ₂ a mode (k, m) has spatial content k (the S⁴ level) and time content m (the S¹ mode). By §570 the **pure-time modes (0, m)** are *energies, not masses* — a state at rest in space but excited in time is not a massive particle. So the **mass** gap is the lightest mode that carries **spatial** structure (k ≥ 1). And here the ℤ₂ does something physical rather than merely subtractive:

> **The ℤ₂ forbids a pure rest-mass (candidate BST statement about mass).** A pure rest-mass would be a spatial-only mode (k ≥ 1, m = 0). But k = 1 with m = 0 has k + m = 1, **odd** — forbidden. To be ℤ₂-even, a k = 1 spatial state must carry **at least one unit of time, m = 1**. *So on our boundary there is no purely-spatial massive state: every massive state carries one time-winding.* The projection that removed "4" is, read forward, the statement that **mass comes bundled with a minimal unit of time**.

**The candidate value.** The lightest spatial-carrying ℤ₂-even mode is then **(k, m) = (1, 1)**, with energy

  **Δ_mass = (spatial 4) + (one time unit 1/R²) = 4 + 1/R²  > 4   (= 5 at R = 1).**

So *if* the mass/energy separation holds on the real boundary, the mass gap returns **above 4** with correct meaning — the pure-time modes (0, m) are set aside as energies, and (1,1) is the lightest genuine mass. This would **lift the floor from existence to > 4** and identify the lowest ℤ₂-even *mass*-mode (opens ii partially), still carrying the ruler R (open iii).

**Tier: CANDIDATE (not banked).** This rides entirely on Grace ⊕ Elie confirming (a) the §570 mass/energy separation *on the real boundary* (that (0,m) are energies, (1,1) is the lightest mass), and (b) that "mass gap > 4" returns with the correct physical meaning. Until then the **banked floor remains existence**; the (1,1) = 4 + 1/R² reading is a *tiered candidate upgrade*, not a claim.
...
**Count HOLDS 4 of 26.** SU(3) scope; glueball masses are predictions, not SM parameter reductions. Paper A v0.8 (K1709 + §570 candidate): the gap **exists** (bounded ⇒ gap, no free dimensionless parameter beyond the ruler) — this floor is rock-solid. On the **real boundary (S⁴×S¹)/ℤ₂** the ℤ₂ admits only k+m even, so the bare-S⁴ "4" (k=1, m=0) is **projected out** (Cal); the honest value awaits **three geometry-fixed opens: (i) the (adjoint) conformal weight Δ, (ii) which ℤ₂-even mode is lowest, (iii) the radius R**. **Candidate floor-lift (Section 2a, tiered — not banked):** by §570 the pure-time modes (0,m) are *energies not masses*, so the *mass* gap is the lightest spatial-carrying ℤ₂-even mode **(1,1) = 4 + 1/R² > 4** (= 5 at R=1) — the ℤ₂ **forbids a pure rest-mass**, so every massive state carries one unit of time; this returns "gap > 4" with correct meaning *if* Grace ⊕ Elie confirm the mass/energy split on the real boundary. The old "6" was the conformal reading = the **identity** C₂ = n_C+1; "7" = genus. The **glueball 1720** is a separate valid prediction (on C₂^int, not λ₁) — kept, out of the room as gap-evidence. Section 0 keeps the three C₂ guards + the glueball exemption. Package intro HELD until Grace ⊕ Elie define the operator on the real boundary, test the §570 lift, and pin Δ + ℤ₂-coupling + R. NOT a derived value (the (1,1) lift is a tiered candidate), NOT "spectrum closed," NOT "YM proved." INTERNAL. Nothing pushed; CP existence-only.

*Draft v0.8 (K1709 + §570 candidate). Spine: Cal's ℤ₂ projection catch (floor = existence); Grace ⊕ Elie's energy-vs-Casimir resolution (§570) + the real-boundary operator definition + the §570 mass/energy-split test (the live tasks); Lyra's three-opens framing + the §570 candidate floor-lift (mass gap = lightest spatial mode (1,1) = 4 + 1/R²; ℤ₂ forbids a pure rest-mass — every massive state carries one time unit), clearly tiered CANDIDATE; the so(7) unification; Paper B (gauge-group derivation); Keeper K1709. Existence banked; the (1,1) lift is a candidate. Companion to Paper B v0.6.*

— Lyra, Wed 2026-08-19 (date-verified). Paper A v0.8: floor = EXISTENCE (banked); §570 candidate floor-lift added (Section 2a, tiered) — mass gap = lightest spatial-carrying ℤ₂-even mode (1,1) = 4 + 1/R² > 4, because the ℤ₂ forbids a pure rest-mass (every massive state carries one unit of time); glueball separate + out of the room; package intro HELD.
