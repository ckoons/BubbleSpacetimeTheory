...
> **⚠ SCOPED CORRECTION 2026-08-22 (Round 46; Cal's claim-shape sweep, owned by Lyra). "Γ never needed" was OVER-BROAD.** It is true of the **QFT / W4 modular lane** — the physical Hilbert space is H²(D_IV⁵), whose Casimir/Wallach spectrum is already discrete, so no arithmetic quotient is needed *there* (this note's result stands). But it is **FALSE corpus-wide: Γ is RETAINED as the ARITHMETIC-lane regulator.** The QED arithmetic (a_e to 0.026%) lives on the **Selberg trace formula over Γ(137)\D_IV⁵**, whose closed geodesics are the ζ(3) — a genuine, separate use of Γ. So **Γ is DEMOTED to the arithmetic-lane regulator, not dropped.** Both are legitimate: **H² for the QFT/W4 lane; Γ\D_IV⁵ for the arithmetic lane.** Read every "drop Γ / Γ never needed / Γ-free" below as scoped to the QFT/W4 lane only. (My term-grep missed this; Cal's independent claim-shape instrument caught it — the insurance working as designed.)

**Round 42. The one owed piece for the YM/Millennium foundation: redo W4's modular-localization/locality construction on H²(D_IV⁵) directly, dropping Γ *from the QFT lane*.** Γ was only a spectral regulator *for L²(Γ\G/K)* (Rellich, to make it discrete), and H²'s Casimir/Wallach ladder is already discrete — so it was not needed *for the QFT modular construction*, and using it *there* broke the G-action (the sector mismatch caught last round). *(Γ remains the arithmetic-lane regulator — see the scoped correction above.)* Reconnected: BST_W4_Modular_Construction (the skeleton), the G-action blind gate (Round 41), T349 (reproducing property), EHW/Vergne–Rossi (positive-energy discrete series). **Scope guard: clearing G3 (locality) clears the FOUNDATION; it does NOT close the interacting-YM identification (G6, a separate residual) — this is not a Clay-claim closure.**
