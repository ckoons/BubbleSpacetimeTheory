...
|---|---|---|---|---|
| DE eq. of state **w** | **−1** — *derived* from the fixed bulk volume C·π⁵ (a geometric invariant) | −1 (*assumed*) | DESI DR2: −0.84 ± 0.06 (+CMB+SNe) | **BST derives what ΛCDM assumes**; refuted by confirmed dynamical DE (K1040) |
| DE *deviation* from −1 | **≤ ~10⁻⁴ and *decreasing*** — the substrate coupling ε(a) → 0 | 0 (exact Λ) | — | test: a ~5% deviation or phantom crossing kills BST |
...
- **★ NEAR-TERM KILL CONDITIONS (current data, done right):**
  1. **★ Σm_ν — both columns (see the comparison table):** BST predicts Σm_ν ≈ **0.0588 eV** (the m₁=0 floor). Under **ΛCDM**, DESI DR2 + Planck + ACT (2024) bound Σm_ν < 0.064 eV — BST sits ~0.005 eV from that edge, a sharp live kill. The bound is model-dependent: under **dynamical dark energy** it relaxes to ~0.16 eV, and DESI DR2's own data now favors dynamical DE over ΛCDM at ~3σ. (BST's *own* dark-energy equation of state is not yet uniquely derived — two candidate forms, deferred to the companion cosmology study, K1039 — so we do not lean on it here.) The test is real either way: a definitive Σm_ν measurement clearly ≠ 0.0588 eV refutes m₁=0.
