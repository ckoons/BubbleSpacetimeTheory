...
- **Domain-side realization (#418 RESOLVED):** on D_IV⁵ itself, color is realized on the Hardy space H² by the **covariant so(5,2)_ℂ generators V_a** — the complexified so(7,ℂ) = so(5,2)_ℂ acting on the (g,K)-module (Harish-Chandra), which close exactly as a Lie subalgebra (a genuine bosonic su(3) operator-algebra on H²). The **naive bilinear-Schwinger** (coordinate-oscillator) realization is a **clean negative**: the color 7-vector straddles the SO(5)-vector and the SO(2)/boost direction, and its 2-part is an odd-charge SO(5)-singlet **absent from H²** (a whole K-type missing, not a small correction) — established by three independent structural routes (K-type level tower / octonion SU(3)⊂G₂ geometry / Hardy K-type content). So color is the **covariant** operator algebra, not the coordinate number algebra. Two tempers (Cal #380): (i) the domain V_a are the **non-unitary** so(5,2)_ℂ continuation — **physical (unitary) color lives on the compact dual Q⁵** (where su(3) ⊂ g₂ ⊂ so(7) is compact and acts unitarily on the quark triplet), with the Lorentzian domain carrying its analytic continuation; (ii) the V_a realization is the **alternative/replacement** for the (negative) naive construction, **not a vindication** of the bulk-color-Toeplitz program — that program is a clean negative. Domain (operator) and compact-dual (geometric) remain two faces of the duality.

**Honest tier:** N_c = 3 is solid as the dimension-free T1829 output (value-recurrence). The color *group* is geometric on the compact dual (su(3) ⊂ g₂ ⊂ so(7), SOLID there) and operator-algebraically realized on the domain by the covariant V_a (SOLID, bosonic); the naive bilinear-Schwinger is a clean negative (#418-as-posed). The uniqueness **spine never needed the color reading** — only the multiplicity value 3 — so the theorem is untouched and the color claim is correctly downstream. (Per Cal #376: this is a purely *bosonic* realization; it does not bear on the separately-posited F(4) odd part, #359.)
...
| 3 (color) | resolved | **RESOLVED** (N_c=3 = T1829 output, value-recurrence per Cal #335; compact-dual geometric / domain operator-algebraic via covariant V_a [#418 resolved: V_a positive, naive bilinear-Schwinger negative]; bosonic per Cal #376) |
