...
**Number theory names the landmarks.** The five integers {3,5,7,6,2} are discrete features of the geometry. The root system B_2, the Weyl dimensions, the Casimir C_2=6, the signature weight g=7 -- these are names for geometric features. They are the landmarks on the terrain.
...
- **Landmarks**: signature weight g=7 (= C₂+1; Coxeter number h(B₃)=6=C₂; the Bergman genus proper = n_C=5, Hua 1963) -- the signature, NOT the pole order of the Bergman kernel (that is n_C=5) on D_IV^5
- **Reading**: The spectral reader decomposes K(z,w) into eigenmodes. The spectral gap between the first and second eigenvalues reads the signature weight. g=7 appears as the number of independent spectral layers the kernel resolves.

#### A2. Coxeter x Shilov boundary: (N8, G4)

- **Structure**: The Shilov boundary S^4 x S^1 -- where measurements commit
- **Landmarks**: signature weight g=7
- **Reading**: The spectral reader restricted to the Shilov boundary has g=7 independent boundary modes. The boundary spectral decomposition reads the signature weight as the maximum number of distinguishable boundary signals.

#### A3. Coxeter x Rank=2: (N8, G5)

- **Structure**: The rank-2 decomposition of D_IV^5 into two independent spectral parameters
- **Landmarks**: signature weight g=7 = 2*rank + 3 = 2*2 + 3
- **Reading**: The spectral reader on a rank-2 domain has two families of eigenvalues. The signature weight reads the interleaving pattern: how the two spectral ladders mesh. g=7 modes fit in the gap.

#### A4. Coxeter x L-group: (N8, G6)

- **Structure**: The Langlands dual L-group Sp(6)
- **Landmarks**: signature weight g=7 (shared by B_2 and its dual)
- **Reading**: The spectral reader on the L-group side produces the same signature weight. The spectral reading is invariant under Langlands duality -- Fourier in one costume reads the same landmark from the dual side.
...
- **Landmarks**: signature weight g=7
- **Reading**: The spectral reader applied to the de Rham complex reads the Betti numbers through the heat kernel on forms. The signature weight organizes the Hodge decomposition into g=7 independent cohomological layers.
...
#### B4. Entropy x signature weight g=7: (S5, N8)

- **Structure**: The g=7 independent spectral layers
- **Landmarks**: signature weight g=7
- **Reading**: The statistical reader counts states across g=7 layers. Maximum entropy = log(g) = log 7 per independent subsystem. The signature weight sets the maximum state count per spectral layer.
...
#### B10. Zero-sum x signature weight g=7: (S9, N8)

- **Structure**: The g=7 layer structure
- **Landmarks**: signature weight g=7
- **Reading**: The statistical reader distributes budget across g=7 layers: total = sum of 7 layer budgets. The signature weight reads as the number of independent budget lines. Seven departments, one treasury.
...
#### B16. Threshold x signature weight g=7: (S7, N8)

- **Structure**: The g=7 spectral layers
- **Landmarks**: signature weight g=7
- **Reading**: The statistical reader finds that exactly g=7 layers pass the stability threshold. The 8th would require more energy than available. The signature weight reads as the number of layers that survive the phase transition.
...
#### C8. Dimensional analysis x signature weight g=7: (S12, N8)

- **Structure**: The signature weight g=7
- **Landmarks**: Spectral gap value
- **Reading**: The arithmetic reader takes the ratio g/rank = 7/2 = 3.5 and g/(rank+1) = 7/3 -- the mean spectral layers per tier. These ratios are the arithmetic costume's reading of how the signature weight distributes across the rank structure.
...
#### C14. Uniqueness x signature weight g=7: (S11, N8)

- **Structure**: The signature weight g=7
- **Landmarks**: Spectral gap
- **Reading**: The arithmetic reader checks: is g=7 unique? Yes. The signature weight of B_2 is 7 (= C₂+1), and no other rank-2 root system gives the same combination of invariants. The arithmetic reader reads uniqueness through prime factorization: 7 is prime, allowing no factoring ambiguity.
...
#### D2. Channel capacity x signature weight g=7: (S2, N8)

- **Structure**: The g=7 spectral gap
- **Landmarks**: signature weight g=7
- **Reading**: The information reader finds that the D_IV^5 channel has g=7 independent sub-channels (one per spectral layer). Total capacity = sum of sub-channel capacities. The signature weight reads as the channel multiplicity.
