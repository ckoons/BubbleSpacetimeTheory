...
- **One dimensionful scale:** the electron mass, measured to three parts in ten billion, non-gravitationally. It plays the role Newton's constant plays in general relativity — one anchor that sets the scale of everything. (From it, as it happens, the geometry *reduces* Newton's constant to that same one scale — G follows to about 0.065%, with the fine-structure constant also taken as measured. That is a two-inputs-to-one *reduction* — trading the constant you measure by weighing the Earth for one you measure by trapping an electron — not a prediction from nothing: no theory predicts a dimensionful G with no scale, and we do not claim to.)
- **Boundary data:** the initial conditions and the single measurement outcomes — the things every theory takes as given.

Against that floor, the **ten axioms of quantum mechanics are *derived from the geometry* rather than *postulated*** — and that, not a parameter count, is the win. (We resist the reflex to score this against the Standard Model's ~19 free constants: those are *particle content*, a different subject; the axioms of quantum mechanics are not the Standard Model's parameters, and comparing the two would be a category slip.) Physics has carried these ten as postulates for a century — assumed because they *worked*, with no one able to say *why*. Here they are consequences of one shape, on a floor no longer than general relativity's: one dimensionful scale, and a spacetime signature we *derive* rather than assume. The question that matters is not how many parameters we replace, but whether the rules are **handed down or fall out** — and they fall out. The floor is a strength, not an apology.
...
The axioms of quantum mechanics are features of one shape — ten of ten accounted for (derived, proved, or structure-derived, each at its stated tier, with **no posit left standing *among the ten axioms themselves***), on a floor no longer than general relativity's. That floor is not empty, and we name it precisely rather than let "no posit" be misread as "no assumptions": the theory rests on **four premise-posits** that select the geometry — P0, the commitment arena is a Euclidean Jordan algebra; P1, a commitment is one binary act; P2a, type-IV exclusivity; P2b, minimality (the four are documented and tiered in the companion, #67) — plus the one dimensionful scale and the boundary data. The *axioms* are derived; the *premise* is posited, and we say exactly which four posits it is. With that floor named, two support-axiom caveats stated, and the hardest axiom — *why matter is fermionic* — closed by a forced computation only after we refused it three times — that account is a stronger and more checkable statement than "quantum mechanics is derived," and — as far as we can tell, and we have tried hard to break it — it is the true one.
