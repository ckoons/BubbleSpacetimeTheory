...
**Status:** Framework draft **v0.4 — 2026-07-11 PM.** *Honestly tiered.* This paper presents a geometric realization, not a completed derivation. Section 7 is a complete ledger of what is derived, what is a candidate mechanism, and what remains open. Nothing here is claimed as proved that is not proved in Section 7.
...
**OPEN (mass sector — the forcing lanes that would complete the mechanized tower):**
- **The last Koide gate:** derive A² = rank from the Bergman overlap norm at the three F86 strata — this flips the charged-lepton sector from near-forced (K672) to fully forced. (Section 3.2)
- **Force the up-type interior** (the charm's generation degree) via the boundary-cohomology. *The Pin(2)/Möbius doublet-flip of the down's {1,3,5} is CLOSED as disfavored* — the up reads its masses through the boundary, not a flip of the down's formula. (Section 3.2)
- **The neutrino sector** — the last hole; Koide closes the *charged* leptons only.
- **The up ground value (n=0) is soft** (~22% undershoot) — the constant-mode overlap is the least-reliably-computed number; the fix is a proper Bergman-normalized overlap at n=0, not a second mass. Casey's two-address (bulk-partner vs decay-product) reading was tested against the neutron–proton splitting and leans no (~1.5σ; revives if lattice (M_n−M_p)^QCD → ~3.0). (Section 3.2)
