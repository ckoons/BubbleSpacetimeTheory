...
Bubble Spacetime Theory derives Standard Model observables from D_IV⁵ = SO₀(5,2)/[SO(5)×SO(2)] via five primary integers (rank=2, N_c=3, n_C=5, C_2=6, g=7) and their derived structure. Where Paper #109 (Lyra) reads these integers as the counting primitives of arithmetic, we offer a parallel reading: the same integers are the **information-theoretic primitives of D_IV⁵ as an information substrate**. The bit lives at rank, the trit at N_c, the Reed-Solomon block-length exponent at g, the function alphabet at 2^g = 128, the channel capacity bound at N_max = 137 (one more than the Mersenne M_g = 127). Particles in this reading are not "things that happen to have winding numbers" — they ARE information units, and the winding number IS the information label (T1922, Casey 2026-05-15). We make this explicit by cataloging the information-theoretic identity of each BST integer, exhibiting how every Standard Model quantum number reads as a substrate-information channel, and connecting to Reed-Solomon coding theory on GF(2^g). The framework is complementary to (and consistent with) Paper #109's counting-primitives reading and Paper #121's Bridge Objects category: information substrate is the layer that makes geometric Bridge Objects load-bearing for physics.
