...
- **The angle values** read as equipartition overlaps sin²θ = d/D (fraction of a maximal-entropy angular state in a primary subspace): sin²θ₂₃ = rank²/g = 4/7, sin²θ₁₂ = N_c/(rank·n_C) = 3/10, sin²θ₁₃ = 1/(N_c²·n_C) = 1/45 — combinatorial fractions, not mass ratios. **CKM:** V_us = √(m_d/m_s) = 0.2243 (0.4%) and the ordering derived; V_cb, V_ub magnitudes structural. **Honest status:** the forms are **identified, not yet derived.** A tempting single-group reading (an so(10) generation rotation, D = 10 vector and 45 adjoint) fits θ₁₂ and θ₁₃ but **not θ₂₃** (D = g = 7), and the D = 10 has a value-coincidence (rank·n_C = N_c+g = 10) — so that reading is **retracted** (K749; Five-Absence never at risk — it was generation-space combinatorics, not a gauged group). The three angles have three different-dimensioned homes (7, 10, 45), and they map cleanly onto the three pairs of the domain's three Korányi–Wolf boundary orbits (bulk, intermediate, Shilov): θ₁₂ = bulk↔intermediate, θ₂₃ = intermediate↔Shilov, θ₁₃ = bulk↔Shilov (C(3,2) = 3 pairs = 3 angles). **The closure route is per-pair connecting-mode counting** — and the first count is pinned: **θ₁₂'s D = 10 = dim SO(5)** (a genuine geometric dimension, not the value-coincidence). (Both the SO(3)_gen and so(10) single-group readings are retracted — the D's are three distinct geometric counts, not one group.) θ₂₃ (D=7) and θ₁₃ (D=45) are the remaining computation (Section 9). **[mechanism derived; forms identified — 1 of 3 D's geometrically pinned]**
- **Flavor physics is one object — the SVD of the two-stratification structure.** For each mass map M = UΣV†: the masses are Σ (radial norms, §6, done), the mixing is U, V (the flag ⟂ frame angle, derived), and CP violation is the phase of U, V (a lead — possibly a single geometric overlap phase relating δ_CKM and δ_PMNS = rank/g = 2/7). Masses radial, mixing angular, CP the phase: the whole flavor sector as one linear-algebra decomposition.
...
- **Computable, not cleanly observable (a distinct, honest tier — not "soft"):** the containment theorem (§9¾) says BST *computes* every observable as a functional of the proven measure μ; the items below are ones for which **no scheme-independent number exists to compare against** — the limitation is in the observable, not the computation.
  - **m_u** — the up-quark is confined, so there is no pole mass; the quoted "up-quark mass" is a scheme/scale-dependent Lagrangian parameter, and *even experiment has no clean value*. BST computes its radial moment of μ; the observable side is intrinsically un-sharp. (A live idea: m_u may be a **rank-2 Born doublet** — two mass eigenvalues on the two Jordan idempotents, each with a Born probability, the observed "mass" being their weighted mixture; "why two" = rank = 2, the same rank-2 that gives two right-handed neutrinos.) *(V_ub is **not** in this list — it is the CKM bulk↔Shilov overlap, the exact analog of PMNS θ₁₃; §7.)*
