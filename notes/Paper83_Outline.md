...
### Section 1. Introduction: One Geometry (3-4 pages)

- **Section 1.1 The question**: Why these numbers? Why alpha = 1/137, why 3 generations, why m_p/m_e = 1836?
- **Section 1.2 The answer**: D_IV^5 — the unique bounded symmetric domain satisfying five simultaneous conditions (APG definition, T1427). Define the five integers as Cartan invariants.
- **Section 1.3 The claim**: Every entry in the table that follows is a closed-form evaluation of a geometric quantity on D_IV^5. No series expansion, no perturbation theory, no fitting to data.
- **Section 1.4 How to read this paper**: Each section presents a class of invariants. Each entry has: symbol, name, BST formula, geometric source, BST value, observed value, precision. Formulas can be verified with a pocket calculator.

### Section 2. The Five Seeds (2-3 pages)
...
- **Section 2.2 The derived integers**: Q = rank^2 + C_2 + N_c^2 = 19. 1920 = state count. 1728. These are not new inputs — they are polynomial expressions in the five.
- **Section 2.3 Why D_IV^5**: The uniqueness argument (T1427). Five conditions, one geometry.

### Section 3. Coupling Constants (3-4 pages)
...
- **Section 3.1 Fine structure constant**: The Shilov boundary argument. This is the headline derivation.
- **Section 3.2 Weinberg angle**: GUT value 3/8, geometric running to low energy.
- **Section 3.3 Strong coupling**: Currently a series (running); target: closed-form at m_Z.
- **Section 3.4 Honest gap**: alpha_s is the only coupling still expressed as a running series, not closed form at a reference scale.

### Section 4. Lepton Masses (2-3 pages)
...
- **Section 4.1 The proton/electron ratio**: 6 pi^5 = C_2 x pi^{n_C}. Why pi^5. The flagship prediction.
- **Section 4.2 The muon**: Spectral eigenvalue ratio.
- **Section 4.3 Honest gap**: Muon g-2 is MISSING. The electron g-2 is a series. These are the hardest entries in the table — vertex corrections, not tree-level.

### Section 5. Quark Masses: The Six-Layer Cascade (3-4 pages)
...
- **Section 5.1 Layer structure**: Each generation adds one geometric operation (color lift → isospin flip → Cabibbo → spectral lift → curvature bridge → Yukawa).
- **Section 5.2 Cross-check**: m_t/m_c = 136 = N_max - 1.
- **Section 5.3 Honest gaps**: (1) Layer ordering is observed not derived. (2) b-quark bridges to lepton sector. (3) Charm has largest error (1.3%). (4) N_max/10 needs cleaner derivation.

### Section 6. Gauge Bosons and Higgs (2 pages)
...
### Section 7. CKM, PMNS, and CP (2-3 pages)
...
- **Section 7.1 The strong CP problem**: Solved by topology (D_IV^5 contractible → theta = 0). No axion needed.
- **Section 7.2 Cabibbo angle**: Geometric origin as 1/sqrt(20).

### Section 8. Hadrons and Mesons (3-4 pages)

~15 entries. Meson masses, glueball ratios, string tension, nuclear SEMF.

Sub-groups:
- **Section 8.1 Pseudoscalar mesons**: pi, K*, eta', phi, J/psi, Upsilon, D, B, Bc
- **Section 8.2 Glueball spectrum**: 0++, 0-+/0++, 2++/0++
- **Section 8.3 QCD string tension**: sqrt(sigma)
- **Section 8.4 SEMF coefficients**: a_V, a_S, a_A, delta
- **Section 8.5 Meson mass ratios and charge radii**: r_pi, r_K, r_K/r_pi, m_J/psi / m_rho, m_B/m_D

### Section 9. Nuclear and Hadronic (2-3 pages)

10 entries. Nuclear magic numbers, neutron lifetime, deuteron binding, proton radius, neutron stars.

- **Section 9.1 Magic numbers**: All 7 from HO + spin-orbit = C_2/n_C.
- **Section 9.2 Proton charge radius**: r_p.
- **Section 9.3 Neutron stars**: M_max, R_NS.
- **Section 9.4 Proton spin fraction**: Delta Sigma.

### Section 10. Neutrinos (1-2 pages)
...
- **Section 10.1 Dirac, not Majorana**: BST predicts Dirac neutrinos. Falsifiable by neutrinoless double-beta decay.

### Section 11. Cosmological Constants (3-4 pages)

14 entries. Dark energy, dark matter, Hubble, baryon density, spectral index, etc.

Sub-groups:
- **Section 11.1 Dark sector**: Omega_Lambda, Omega_m, DM ratio, w_0
- **Section 11.2 Baryogenesis**: eta_b, Omega_b
- **Section 11.3 CMB observables**: n_s = 1 - 5/137, A_s, r (tensor-to-scalar)
- **Section 11.4 Hubble constant**: H_0 (Route C)
- **Section 11.5 Cosmological constant**: Lambda from Bergman kernel
- **Section 11.6 Matter-radiation equality**: z_eq
- **Section 11.7 Honest gap**: Some cosmological entries have ~1% precision. The spectral index n_s = 0.9635 vs Planck 0.9649 is a 0.15% deviation that could tighten.

### Section 12. Anomalous Magnetic Moments (2-3 pages)
...
- **Section 12.1 Electron g-2**: Current status — C_1 = 1/(2*rank) topologically protected (vertex protection theorem). Full a_e conjectured as finite sum over Bergman spectrum (137 terms). Phase 3 of W-15 program.
- **Section 12.2 Muon g-2**: MISSING. Most important open problem in the table.
- **Section 12.3 Why g-2 is hard**: Vertex corrections are 3-point functions; most BST results are 2-point (propagator level) or 0-point (vacuum level). The Bergman kernel naturally gives 2-point correlators. 3-point requires convolution.

### Section 13. Cross-Domain: Mathematics and Graph Theory (2-3 pages)

~10 entries. Ising exponents, Kolmogorov, KPZ, AC graph self-description, Tsirelson bound.

- **Section 13.1 Statistical mechanics**: beta_2D (exact), beta_3D (2.1%), gamma_3D (5.7%), K41 (exact), KPZ (exact).
- **Section 13.2 Quantum information**: Tsirelson bound (exact).
- **Section 13.3 Self-description**: The AC theorem graph's own statistics (avg degree, clustering, hyperbolicity, diameter) are BST expressions. The geometry describes the graph that describes the geometry.
- **Section 13.4 Honest gap**: 3D Ising exponents have the largest errors in the table (2-6%). These are cross-domain applications and may need refined geometric arguments.

### Section 14. Number Theory and Elliptic Curves (2-3 pages)

~10 entries. 49a1, Frobenius, Kim-Sarnak, supersingular fraction, Pell discriminant.

- **Section 14.1 The BST curve 49a1**: j-invariant, c_4, c_6, discriminant, conductor = N_c^2 * n_C + rank = 47... wait, N = 49 = g^2. The canonical elliptic curve.
- **Section 14.2 Kim-Sarnak**: theta_KS = g/2^{C_2} = 7/64.
- **Section 14.3 Supersingular fraction**: 1/rank = 1/2 (T1437, corrected from N_c/g; p=7 excluded as bad reduction).
- **Section 14.4 Frobenius on GF(128)**: Period and orbit structure.

### Section 15. Biology and Emergence (1-2 pages)

6 entries. Genetic code, amino acids, metabolic scaling, brain energy, cooperation.

- **Section 15.1 Genetic code**: N_codons = 4^N_c = 64. N_amino = rank^2 * n_C = 20.
- **Section 15.2 Scaling laws**: Metabolic 3/4, brain 19%.
- **Section 15.3 Cooperation**: f_crit, optimal group size. These are structural predictions, not numerical.

### Section 16. Structural and Derived (1-2 pages)

Remaining structural entries: 19, 1920, 1728, Q, Wallach set, APG conditions, uniqueness, etc.

### Section 17. The Missing Three and the Two Series (1-2 pages)
...
### Section 18. Precision Distribution (1-2 pages)
...
### Section 19. Falsifiability (1-2 pages)
