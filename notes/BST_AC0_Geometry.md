...
**The Einstein constant $-(n_C + 1) = -6$ and the spectral gap $\lambda_1 = n_C + 1 = 6$ share the value 6 in this normalization.** *(CORRECTED 2026-08-08, K1294/K1296/Elie: the earlier claim that this equality "is a theorem, by Obata" is WITHDRAWN. Obata's theorem is **sphere rigidity** — $\lambda_1 = mK$ with equality **iff the round sphere** — and it gives a dimension factor $\lambda_1 = m\,K$, NOT $\lambda_1 = E$. D_IV⁵'s compact dual is **not** a round sphere: the ladder $\lambda_k = k(k+5)$ matches the round-$S^6$ eigenvalues but its degeneracies are SO(5)-harmonics-on-$S^4$ [1,5,14,30,55], **not** $S^6$'s [1,7,27,77,182] — so it is the $S^4$ boundary Laplacian $k(k+3)$ conformally shifted by $2k$ (the SO(5,2) discrete-series Casimir; the "+5" is the conformal dimension $n_C$, not a sixth sphere dimension). So $\lambda_1 = 6 = C_2$ is the **conformal Casimir first eigenvalue**, and any relation to the Einstein constant is a same-value observation requiring the actual Kähler–Einstein computation — NOT Obata, NOT a proven equality.)*
