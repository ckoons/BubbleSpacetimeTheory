...
Every count in the genetic code is a small BST-integer expression in (rank, N_c, n_C, C_2, g, χ) — a **striking null-model match**: the code's tallies (4 / 3 / 20 / 64) appear as BST small integers more than chance would give. **But the honest tier here is CONTINGENCY, not forcing** *(W7 re-scope 2026-08-22)*: geometry supplies the small integers, and life's discrete counts *organize around* them, but the standard genetic code remains a **frozen evolutionary contingency** — Crick's 1968 reading stands, and documented variant codes (mitochondrial, ciliate) show the tally is *not* geometrically forced. The claim in an earlier draft that "the 64 / 20 / 4 / 3 tally is *not* contingent — it is forced by D_IV⁵, like m_p/m_e" is **WITHDRAWN**: unlike m_p/m_e (a derived physical constant), the genetic code is a contingent biological system whose counts *match* the BST integers, not a derivation of them.

The honest contrast with the conventional reading is therefore narrower and still substantive: the standard view (Crick's 1968 "frozen accident," Woese's coevolution, Koonin–Novozhilov's information-theoretic work) is that *some* code would have crystallised by chance from a vast chemistry space. BST *adds* that the counts life crystallised around are the small integers that also organize the physics — a null-model regularity worth reporting — **without** claiming the specific code is forced. Geometry gives the integers; evolutionary contingency gives the code.
