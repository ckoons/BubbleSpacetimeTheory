...
The restricted root system Σ(g, a) for D_IV^n is **of type C_2** for n ≥ 3 [Helgason, Ch. X §6].

> **[12:08 EDT 2026-09-06 — CONVENTION PINNED (Grace, Round 121 G12; toy 5706, `play/.out_5706.txt`).** The restricted root system of so(5,2) w.r.t. a maximal abelian a ⊂ p is of type **B₂** in Helgason's labelling (Ch. X, Table VI: SO₀(p,q), p > q, type B_q, m(±e_i) = p − q, m(±e_i ± e_j) = 1): computed directly by diagonalising ad(t₁H₁ + t₂H₂) on the 21-dimensional matrix algebra — eigenvalues ±t_i with multiplicity **3** and ±t₁ ± t₂ with multiplicity **1**, centraliser of dimension 5; 2·(2·3 + 2·1) + 5 = 21. The line above is a RELABELLING, not an error: B₂ and C₂ are isomorphic as abstract root systems (the only rank where B_n ≅ C_n), with the names "long" and "short" exchanged — so its "[VERIFY]" is resolved, and every statement in the corpus that says "long-root reflection" or "short-root reflection" is convention-carrying. **Quote the invariant instead:** the roots of multiplicity 3 (±e_i in B₂ labels) and the roots of multiplicity 1 (±e₁ ± e₂). On the polydisc coordinates μ = (t₁ + t₂, t₁ − t₂) (the strongly orthogonal directions, Lyra L2): the reflection in a multiplicity-1 root (s_{e₁∓e₂}) NEGATES one polydisc coordinate — that is the functional equation; the reflection in a multiplicity-3 root (s_{e_i}) EXCHANGES the polydisc coordinates — that is the divisor swap / spatial parity. In e-coordinates the same two reflections do the opposite (s_{e₁−e₂} exchanges t₁ ↔ t₂; s_{e₂} negates t₂), which is exactly the collision Round 120's prompt fell into. The multiplicities are the convention-free names.] The two short roots have multiplicity n-2; the two long roots and the sum/difference roots have specific multiplicities.
