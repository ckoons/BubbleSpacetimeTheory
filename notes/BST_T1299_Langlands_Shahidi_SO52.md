> **[T1299 v3 — Lyra L7 row text (2026-09-06, Cal §862 C7 word): Langlands–Shahidi Intertwining Operator for SO₀(5,2) (T1299 v3, STRUCTURAL, D1 — Siegel parabolic P₂ of G = SO₇ (form of signature (5,2), ℚ-rank 2), Levi M = GL₂ × SO(q₀), dim q₀ = 3. The L-functions of the constant term are indexed by the adjoint action of ^L M = GL₂(ℂ) × Sp₂(ℂ) on the unipotent radical of the corresponding parabolic of the DUAL group ^L G = Sp₆(ℂ): ^L 𝔫 = (std_{GL₂} ⊗ std_{Sp₂}) ⊕ Sym²(std_{GL₂}), dimensions 4 + 3 = 7. Hence **r₁ = std ⊗ std_{SL₂}, degree 4**, L(s, π × σ) for π on GL₂ and σ on SO(q₀); **r₂ = Sym²(std), degree 3**, L(2s, π, Sym²) (Gelbart–Jacquet). For σ the trivial representation of the anisotropic SO(q₀), with Satake parameter diag(q^{½}, q^{−½}) at split places, L(s, π × 1) = L(s−½, π)·L(s+½, π) — two shifted copies of the standard L-function, which at the minimal parabolic (π a character) are the two ζ-ratios of L1's factor B, verified by E6 (5704) and by Cal §855's 2-adic integral; at 2 and ∞ the trivial representation of the compact kernel is replaced by its Jacquet–Langlands partner (Steinberg; weight-2 discrete series). M(s,π) = ε(s,π×σ)·ε(2s,π,Sym²)·[L(1−s,π̃×σ̃)/L(s,π×σ)]·[L(1−2s,π̃,Sym²)/L(2s,π,Sym²)]. SUPERSEDED FROM THE APRIL ROW: (a) r₁ = "std^{⊕3}, cubed, 3 = N_c" — that 3 is the GROUP-side root multiplicity (n − 2, correct as a root-system fact, Grace 5706) misread as an L-function multiplicity; the dual side has degree 4 and no cube. (b) The "ε-parity constraint" of Steps D–D′ — Maass–Selberg M(s)M(1−s) = Id holds for every π (ε(s,r)ε(1−s,r̃) is a constant sign), so it constrains nothing; the six Arthur-type eliminations are WITHDRAWN, together with Step E and the registry line "PROVED for D_IV⁵ temperedness" (Lyra L5, 2026-09-06; Cal §853/§854 concur). Temperedness of the cuspidal spectrum of SO(5,2) is OPEN and in general FALSE as stated (Arthur's classification contains non-tempered cuspidal parameters); the well-posed target is "generic ⟹ tempered" (Shahidi's conjecture). (c) The group-side radical Hom(V₀,W) ⊕ ∧²W (my L5 §1) is true of SO₇ and irrelevant to the L-functions; the earlier "∧² = det" correction is withdrawn in favour of the dual-side reading (Cal §853). Consumers: T1262 (→ CONDITIONAL: its constraints (A)–(G) were never audited, and (0) is void); T1298 (conclusion stands: the c-function gives no constraint; its "double root" is the long root e₁+e₂); T2621 (the spherical specialisation). The edge T1299 → T186 carries "multiplicity 3 = n − 2 at n = 5", an IDENTIFICATION (family-generic n − 2), not a derivation of N_c (K1866 §1). Files: BST_T1299_Langlands_Shahidi_SO52.md (April; Steps A–C retained with r₁, r₂ corrected), Lyra_L5_* (withdrawal), Cal §853–§855, Lyra_L7_* (this text). Lyra (April computation and withdrawal), Cal (dual-side correction).) — Steps D, D′, E below are WITHDRAWN and left readable as the April record.]**

> **STATUS 2026-09-06 (Lyra L5/L7; Cal §853–§855; Keeper K1865/K1866): Steps D, D′ and E are WITHDRAWN and the registry line "PROVED for D_IV⁵ temperedness" is retracted — the Maass–Selberg ε-product is an identity for every π and constrains nothing. Step B is corrected on the DUAL group: r₁ = std ⊗ std_{SL₂} (degree 4), r₂ = Sym² (degree 3); "std^{⊕3}" read the group-side root multiplicity as an L-function multiplicity. Current row text: `Lyra_L7_T2621_resonance_theorem_row_and_T1299_v3_*_2026-09-06.md`. The text below is the April record, kept readable, not current.**
