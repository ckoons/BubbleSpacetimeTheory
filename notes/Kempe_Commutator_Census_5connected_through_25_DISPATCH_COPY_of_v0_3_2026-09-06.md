---
title: "Kempe commutators at a stuck pentagon: an exhaustive census on 5-connected triangulations through 25 vertices — DISPATCH COPY of v0.3 (internal pointers as footnotes to the public record)"
author: "Casey Koons, with Lyra, Elie, Grace, Keeper (CI co-authors)"
date: "2026-09-06, Sunday (DISPATCH COPY of v0.3 prepared 16:09 EDT for K1852-D from the v0.3 draft of 15:55 (n = 25 row on two instruments; Cal §864 heading fix; K1852-C (a) met: the 1,171 depth column on two instruments); author line and citation form are Casey's call (B7); internal pointers as footnotes)"
status: "DISPATCH COPY of v0.3. Every number below is a census number produced on two independent instruments with blind-hashed witness lists — the stuck colourings themselves, as (n, plantri index, vertex, colouring) — (Appendix B); every lemma marked DERIVED has an artifact and an independent re-derivation on record; every open clause is marked. Nothing in this document claims a proof of the Four Colour Theorem, and nothing in it concerns triangulations with more than 25 vertices."
---
...
of every 5-connected sphere triangulation with 12 to 25 vertices: 34,250,085 stuck colourings. All but 1,520 reach the
gate phase (τ ≤ 5, one swap from insertable) within one fully legal word of the frozen 186-word commutator menu; all
1,520 reach it within two; none needs three. The 1,520 are *commutator-locked at depth one*, a menu-relative notion: in
the unrestricted alphabet of plain Kempe swaps, of the 349 at n ≤ 24, 27 exit in one swap and Kempe's own pairing inserts on two; of the 1,171 at n = 25, 100 exit in one swap and none needs more than four.
We give the census in both alphabets, a measured necessary condition for the commutator lock that holds on all 349 at n ≤ 24
...
bounded number of fixed-seed commutators always suffices, and if so, why. The word count stays at two through n = 25, where the population of stuck colourings is 3.6 times everything before it.
...
that *does* meet the road forbids nothing (two of the 349 commutator-locked colourings at n ≤ 24 have Kempe's pairing insert v).
...
349 locks at n ≤ 24 for both bridge words (and on 28,549 of 28,558 bridge-stuck images, the nine exceptions not being locks).
...
the middle road and the singleton road on its side — all four bits — on 349/349 locks at n ≤ 24 (256 of them out of sample at
...
copy-pairings' chains meet) holds on 349/349 at n ≤ 24 and on 26 percent of that same population of 374,658; none of the 22 bits or the five conditions was re-measured on the 1,171 locks at n = 25. No incidence pattern at any refinement
tried is sufficient (Section 4: purity by fragmentation is not classification).

# 3. The census

Generator: plantri 5.8, `-c5`, n = 12…25 (graph counts 1 · 0 · 1 · 1 · 3 · 4 · 12 · 23 · 71 · 187 · 627 · 1,970 ·
6,833 · 23,384). At every degree-five vertex, every proper 4-colouring of T − v modulo colour permutation, every stuck one
...
| 25 | 26,870,832 | 44,566 | 43,395 | 1,171 | 0 |
| total | 34,250,085 | 57,794 | 56,274 | **1,520** | **0** |

The n = 25 row is on two instruments (one union-find/BFS pipeline and one independent Laplacian-rank pipeline; the 44,566-record population hashed before any count, the 1,171 lock witness lists hashed before the diff — 1,171 common, none either-side-only)[^n25]. The 1,520 are commutator-locked at depth one (n = 17: 8 · 21: 18 · 22: 23 · 23: 44 · 24: 256 · 25: 1,171). The classical Kempe
...
configuration to be commutator-locked. At the measured lock rate (4.4 × 10⁻⁵ of stuck colourings at both n = 24 and n = 25: 256/5,787,744 and 1,171/26,870,832), with the
probability that a bridge word's image is itself commutator-locked at 457/698 = 0.655 (the two bridge-word images of
each of the 349 locks[^d1]) and near zero along the other sixteen orbits, the expected number of
depth-three configurations in this census is below one (the bridge-word image rate is measured on the 349 at n ≤ 24 and not re-measured at n = 25). The observed zero therefore supports no mechanism, and we
claim none.

**The two-word fraction, as numbers only.** Of the stuck colourings without a direct one-word exit, those needing two
words: 26/334, 23/827, 44/1,579, 256/10,488, 1,171/44,566 (7.8, 2.8, 2.8, 2.4, 2.6 percent) — falling through n = 24 by exact counts, rising at n = 25 (2.44 to 2.63). Not extrapolated in either direction. At n = 25 the two-word configurations are reported as a count only: the sub-split of their exits into "gate" and "direct" second words depends on the order in which the menu is enumerated (an image that gates but does not insert, met earlier, masks a direct image met later), so it is not a property of the triangulation and is not printed; the structural statement that replaces it awaits its second instrument.

## 3.2 The second word and the exit mechanism

On every one of the 349 at n ≤ 24 (the exit mechanism was not analysed at n = 25), the two-word exits run through a first word that moves the middle colour and a second word
...
4,564 of 6,630 exits by a bridge word (0.688[^d2]) against a census base near
...
On the 349 at n ≤ 24, plain Kempe swaps with any seed: **one swap reaches the gate phase on 27** (on the 1,171 at n = 25: 100, Table 3.3); Kempe's own pairing (ζ then η
...
reach the gate phase (a separate instrument[^8]) — a statement about those 49 only; on the 349 the unrestricted plain-swap depth
...
| the 1,171 commutator locks, n = 25 | 100 | 426 | 629 | 16 | 4 | 0 |
| all 10,488 stuck colourings without a direct one-word exit, n = 24 | 3,682 | 5,652 | 1,142 | 12 | 4 | 0 |
| all 44,566 stuck colourings without a direct one-word exit, n = 25 | 18,341 | 22,046 | 4,160 | 19 | 4 | 0 |
| generic stratified sample, n = 17–22 (936 configurations) | 929 | 7 | 0 | 0 | 2 | 0 |

Instruments: a BFS over plain swaps on raw colourings with the terminal gate verified by the rank instrument
(records hash 54cd2ced…) and an independent-code instrument in three modes (hashes in Appendix B)[^9][^d3]; the two agree exactly on
the 349. At n = 25 two independent instruments agree to the unit in every bin of the 44,566 row (prediction "maximum 4, unreached 0" hashed before the run; the two runtimes differ by two orders of magnitude on identical answers — per-state cost, not exploration — and no result depends on it), and the depth-1 count 18,341 equals the single-swap gate count of the commutator census independently; the 1,171-lock row likewise on two (the first instrument's depth function run verbatim on the hashed 1,171 witness list agrees with the second in every bin)[^n25]. Read against the menu: a two-word exit is
at most eight plain swaps and the unrestricted distance is at most four on every lock; the thirteen locks at depth 4
through n = 24 (one at n = 23, twelve at n = 24) are not among the 49 through n = 22, and sixteen of the nineteen
depth-4 configurations at n = 25 are among the 1,171 locks — the structural link between the two tables.
...
155/186 over the 93 two-word locks at n ≤ 22 and 93 matched depth-one configurations on the same (T, v)[^d4]) —
...
kill are given so the order can be checked. (1) The one-word form of the claim — pre-registered 2026-09-01 (referee items 819–820; instruments[^d5]); refuted in frame by 49 exhaustive witnesses at n ≤ 22 and 349 at n ≤ 24 (two instruments[^d7],
...
(6) Six-bit and 68-bit incidence types as a classification of the lock (instruments[^d6]; referee item 824, 2026-09-02):
...
**Not asserted:** any count beyond n = 25; any depth bound for all n; any mechanism for the depth-two observation;
...
bounded, fixed-seed form and given 1,520 witnesses of where one commutator is not enough.
...
[^n25]: n = 25 instruments: stage-1/stage-2 census and unrestricted-depth scripts 5600/5601/5664a and the 1,171-lock depth run 5708 (Elie; prediction sha256 prefix 49670a3b) and the independent rank pipeline 5666 (Grace; population e9ae614c, lock witnesses d4fdec14), Bubble Spacetime Theory public record, Zenodo, https://doi.org/10.5281/zenodo.19454185, four-colour section.
[^d1]: Instrument script 5640 (bridge-word images of the 349), Bubble Spacetime Theory public record, Zenodo, https://doi.org/10.5281/zenodo.19454185, four-colour section.
[^d2]: Instrument script 5616 (Δ-NO leaf rate after an exiting word), Bubble Spacetime Theory public record, Zenodo, https://doi.org/10.5281/zenodo.19454185, four-colour section.
[^d3]: Instrument scripts 5625 (Grace) and 5624 modes A/B/C (Elie), Bubble Spacetime Theory public record, Zenodo, https://doi.org/10.5281/zenodo.19454185, four-colour section.
[^d4]: Instrument script 5610 (image-count feature), Bubble Spacetime Theory public record, Zenodo, https://doi.org/10.5281/zenodo.19454185, four-colour section.
[^d5]: Instrument scripts 5595–5599 (Elie, 2026-09-01), Bubble Spacetime Theory public record, Zenodo, https://doi.org/10.5281/zenodo.19454185, four-colour section.
[^d6]: Instrument scripts 5613/5615 (Elie), Bubble Spacetime Theory public record, Zenodo, https://doi.org/10.5281/zenodo.19454185, four-colour section.
[^d7]: Instrument scripts 5601 and 5613 (Elie), Bubble Spacetime Theory public record, Zenodo, https://doi.org/10.5281/zenodo.19454185, four-colour section.
