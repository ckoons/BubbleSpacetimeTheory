...
The Euler characteristic of the compact dual is $\chi(Q^5) = 6 = C_2$ — the same *value* that appears as the spectral gap $\lambda_1 = 6$, in the Einstein-constant expression, and in the factor of $m_p = 6\pi^5 m_e$. These are $C_2 = 6$ in **different roles** (topology / spectrum / curvature / mass): the shared value is the integer web showing through, but a *common counting mechanism* is a **candidate, not an asserted identity** — do not weld them. *(CORRECTED 2026-08-08 per K1294 / no-weld: the earlier "$\lambda_1$ = the Einstein constant, one counting, not a coincidence" over-identification is retracted — Obata is sphere-only, so $\lambda_1 \ne$ Einstein constant even in value; the honest tie is at most a factor-of-rank relation, gated on the operator ID.)* **CLOSED 2026-08-08 (Elie operator-ID, K1299/K1300): the operator is the $S^4$ conformal Casimir (SO(5) degeneracies [1,5,14,30,55]), not the $Q^5$ Laplacian, so neither Obata nor Kähler–Einstein applies — the retraction is final, not "in flux."**
