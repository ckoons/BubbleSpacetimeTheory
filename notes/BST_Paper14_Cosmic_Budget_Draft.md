...
## Cosmic Composition from Five Integers

---

## Abstract

We derive the cosmic energy budget — dark energy, dark matter, and baryonic matter fractions — from the bounded symmetric domain $D_{IV}^5 = SO_0(5,2)/[SO(5) \times SO(2)]$. Three independent routes yield $\Omega_\Lambda = 13/19 = 0.68421$ (Planck: $0.6847 \pm 0.0073$, $0.07\sigma$). The dark matter–baryon ratio $\Omega_{DM}/\Omega_b = 16/3 = 5.333$ (Planck: $5.364$, $0.58\%$). All five cosmic fractions fall within $1\sigma$ of observation. Zero free parameters. The five integers $N_c = 3$, $n_C = 5$, $g = 7$, $C_2 = 6$, $N_{\max} = 137$ determine the universe's entire energy allocation. Dark matter is not a particle — it is the gravitational signature of unused information bandwidth. Dark energy is not a mystery — it is the majority vote of 13 geometric modes against 6.
...
The geometry of spacetime is the bounded symmetric domain $D_{IV}^5 = SO_0(5,2)/[SO(5) \times SO(2)]$. It is built from five integers: $N_c = 3$ (colors), $n_C = 5$ (complex dimensions), $g = 7$ (Bergman genus), $C_2 = 6$ (Casimir eigenvalue), and $N_{\max} = 137$ (channel capacity). Every cosmic fraction is a ratio of these integers. No parameter is adjusted.
...
The coefficients sum to $1 + 5 + 11 + 13 + 9 + 3 = 42 = C_2 \times g$ — the Rosetta Number (T689). The Chern polynomial speaks all three bedrock languages: Shannon information theory ($g = $ source coding bound), number theory ($42 = \text{den}(B_6)$), and geometry ($\Sigma c_k$ of the tangent bundle).
...
Now read the cosmic fraction by cross-pairing:
...
The geometry of $D_{IV}^5$ has $N_c = 3$ color channels. These are the diagonal entries of the $3 \times 3$ color matrix — the committed channels that carry baryonic matter.
...
### 3.1 The Sum Identity

The dark energy numerator (13) and the cosmic denominator (19) satisfy (T681):
...
### 3.2 Why Binary Matters

In a binary universe, every information channel carries exactly 1 bit per mode. The identity $13 + 19 = 32$ relates the dark energy numerator to the total denominator through the binary architecture of the complex dimension.

The matter numerator $6 = C_2$ (Casimir eigenvalue) gives $6 + 13 = 19$, which is $\Omega_\Lambda + \Omega_m = 1$ in integer form. In integer form it says something precise: the Casimir ($C_2 = 6$) plus the Weinberg denominator ($13$) equals the information dimension ($19$). The budget closes exactly.

### 3.3 The Chern Polynomial Connection

Route 1 derives $\Omega_\Lambda$ from the Chern polynomial of $Q^5$:

$$c(Q^5) = \frac{(1+h)^7}{1+2h} = 1 + 5h + 11h^2 + 13h^3 + 9h^4 + 3h^5$$

The coefficient $c_3 = 13$ IS the dark energy numerator. The integer coefficients sum to $1+5+11+13+9+3 = 42 = C_2 \times g$. The Chern polynomial of the universe's tangent bundle has total weight equal to the product of the two spectral invariants.

**Cross-check: the alternating sum.**
$$c_0 - c_1 + c_2 - c_3 + c_4 - c_5 = 1 - 5 + 11 - 13 + 9 - 3 = 0$$

The Euler characteristic of $Q^5$ is zero. The universe's budget balances to zero alternating sum — a topological constraint providing an independent consistency check.
...
19 = $N_c^2 + 2n_C$ is the sum of two capacities:
...
19 is the total capacity of the geometry: the number of independent information channels available to the universe's partition function. Every appearance of 19 is this same capacity seen through a different lens.

### 4.3 The Uniqueness of 19

For general $D_{IV}^n$, the cosmic denominator would be $N_c^2(n) + 2n$. But only for $n = n_C = 5$ does the denominator satisfy:
...
The 13 vacuum-committed modes carry no localized excitations. They are geometrically committed to maintaining the vacuum structure of the domain. This commitment creates isotropic pressure: the modes push equally in all directions because they have no preferred direction (no excitation to break symmetry).
...
- **LZ, XENONnT, DARWIN**: Null results at all WIMP masses.
- **ADMX, ABRACADABRA, CASPEr, IAXO**: Null results for QCD axions.
- **LHC Run 3+**: No dark matter candidates beyond Standard Model.

Every null result from a direct detection experiment is consistent with BST. A confirmed detection of a dark matter particle at any mass would falsify the BST interpretation.

**Prediction 5: No WIMP annihilation signal.**

Indirect detection experiments (Fermi-LAT, CTA, IceCube) searching for WIMP annihilation products will find no signal attributable to dark matter annihilation.

### 6.3 Structural Predictions

**Prediction 6: $13 + 19 = 32 = 2^{n_C}$.**

A structural identity. No alternative decomposition of the cosmic budget into integer fractions from the same geometry produces this binary closure. Testable by mathematical proof: show that no other bounded symmetric domain with different invariants produces all five cosmic fractions within $1\sigma$ of Planck while satisfying the binary sum condition.

**Prediction 7: The five-pair cycle has period exactly $n_C = 5$.**

Pair 6 ($k = 30, 31$) gives $G_6 = 87 = 3 \times 29$ and $G'_6 = 93 = 3 \times 31$, which are $N_c$ times backbone primes. Computational verification of $a_{30}(n)$ will resolve whether the cycle period exceeds 5.

**Prediction 8: MOND acceleration from the same geometry.**

$$a_0 = \frac{cH_0}{\sqrt{n_C(n_C + 1)}} = \frac{cH_0}{\sqrt{30}}$$

BST derives the MOND acceleration scale $a_0 \approx 1.2 \times 10^{-10}$ m/s² from the same $n_C = 5$ that gives the cosmic composition (T191). The $\sqrt{30} = \sqrt{n_C(n_C+1)}$ also appears in the pion mass formula $m_\pi = 25.6\sqrt{30}$ MeV. The connection between nuclear physics and galactic dynamics is a single number: the complex dimension of $D_{IV}^5$.
...
T192 (Cosmological Composition), T205 (Dark Matter = UNC), T297 (Dark Matter Fraction), T676 (Backbone Sequence), T677 (Cycle Length), T678 (Cosmic Composition Prediction), T681 (Cosmic Dimension Sum), T689 (Shannon Source Coding Bound)

## Numerical Verification

Toy 667 (cosmic budget, 10/10 PASS), Toy 668 (Chern polynomial, 10/10), Toy 669 (speaking pairs, 10/10), Toy 670 (source coding bound, 10/10)
...
*"The universe does not have a dark energy problem. It has a budget. We just read it."*
