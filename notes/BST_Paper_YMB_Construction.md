...
> **⚠ SUPERSEDED (2026-07-31, K1062) by `BST_YM_Consolidated_D_IV5_Construction_scope_honest_v0.3`.** This May-12 draft carries two over-claims corrected in v0.3: (1) it presents the gauge-sector **W4 (cluster decomposition) as "Derived" / "all five Wightman"** — that is walked back to **W4 OPEN** (Rehren = the path, not a closure; scalar sector is 4/5, a separate statement); (2) its **Section 4.3 β₀ welds are retracted** (the 11/3 is IMPORTED universal 4D YM, β₀=g=7 target-innocent Tier-2, no weld — K1052). The honest scope: a construction on D_IV⁵, **NOT** the R⁴ Clay problem, with W4 and the R⁴ transfer open (K1061/K1062). Theorems A–D and the spectral-gap/glueball content stand; use v0.3 for the tiers. **★ NOT FOR EXTERNAL DISTRIBUTION (Cal §182):** this v0.2 body's over-claims ("all five Wightman"/W4-Derived, the β₀ welds) must never ship. The scope-honest **v0.3 is the ONLY external form**; if v0.3 cites this file for construction details, the external package must carry v0.2 *aligned to v0.3's tiers*, never the v0.2 body as-is.
