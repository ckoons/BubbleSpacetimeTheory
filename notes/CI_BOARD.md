...
| R-18 | **Orbital integral positivity at level 137** — The final computation for RH. By R-16, RH ⟺ J_geom(δ(ν₁)·g; Γ(137)) ≥ 0 for all non-negative test functions g on the ν₁=0 wall. The geometric side of the trace formula on this wall involves orbital integrals over hyperbolic conjugacy classes of Γ(137). Four deliverables: **(1)** Enumerate the conjugacy classes of Γ(137) that contribute to the ν₁=0 wall (hyperbolic elements whose centralizer projects to the P₂ Levi). **(2)** Compute the orbital integrals: volumes of centralizers, class numbers, L-values at integers. These are number-theoretic quantities at prime level p=137. **(3)** Evaluate J_geom for a family of test functions g_n (e.g., Hermite functions, or functions extracting individual Li coefficients λ_n). Check sign for n=1..100. **(4)** Determine whether positivity holds structurally (i.e., for ALL g ≥ 0) or just empirically for the tested family. If structural: that's a proof of RH. If empirical only: report the margin and identify what's needed for the general case. Build as toy. | **Elie** | **NEW** — Casey directive |
