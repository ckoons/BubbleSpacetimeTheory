...
| R-15 | **Heat kernel trace formula positivity — RH via C₂ = 6** — Compute both sides of the trace formula for Γ(137)\\SO_0(5,2) using heat kernel test functions h_t(ν) = exp(-t(ν₁²+ν₂²)). Three deliverables: **(a)** Discrete sum Θ(t): eigenvalues k(k+5), multiplicities from Hilbert polynomial, verify large-t ~ 7·e^{-6t}. **(b)** Geometric side J_geom(t): Gangolli/Anker-Ostellari closed-form HC transform for D_IV^5, orbital integrals at level 137. **(c)** Eisenstein remainder E(t) = J_geom(t) - Θ(t): evaluate at 20 values of t in [0.01, 10], compare to Σ_ρ (Faddeeva terms, first 100 ζ-zeros from Toy 2068). Check: does C₂ = 6 positivity budget control the remainder for all t > 0? Report tightest t and whether any t fails. If E(t) is consistent with all Li coefficients λ_n ≥ 0, that's the proof chain: heat kernel positivity ← trace formula + temperedness + C₂ gap → Li criterion → RH. Build as toy. | **Elie** | **NEW** — Casey directive |
| R-16 | **Rank-2 wall projection for Li coefficients** — The rank-2 structure of D_IV^5 may solve Connes' test function problem. Key idea: ζ-zeros live on the P₂ wall (ν₁=0), discrete eigenvalues are interior (ν₁≠0). If there's a GAP at ν₁=0 in the discrete spectrum, then a test function φ(ν₁) peaked at 0 annihilates the discrete sum while preserving the Eisenstein/ζ-zero contribution. Three deliverables: **(1)** Compute ν₁ values for all discrete spectrum representations of Γ(137)\\SO_0(5,2) — holomorphic discrete series, other tempered reps. Find minimum |ν₁|. **(2)** If gap exists at ν₁=0: construct φ supported in |ν₁|<δ (PW in the ν₁ direction). Show discrete sum vanishes. Then trace formula reduces to: [Eisenstein involving λ_n] = [geometric side]. **(3)** Evaluate geometric side (orbital integrals at level 137) for the projected test function. Check sign: is geometric side ≥ 0? If yes for all n, that's RH via Li's criterion through the rank-2 trace formula. Build as toy. | **Elie** | **NEW** — Casey directive |
| R-17 | **Multiplicity squeeze for RH (Lyra Direction 2)** — The k⁴ multiplicity growth of D_IV^5 (from n_C=5, dim=10) may squeeze ζ-zeros onto Re=1/2. For each eigenvalue level k: d_k = (2k+5)(k+1)(k+2)(k+3)(k+4)/120 grows as k⁴. The trace formula at level k constrains zeros to |Re-1/2| ≤ σ_k. If σ_k → 0 as k → ∞, RH follows. Three deliverables: **(1)** For each k=1..50, compute the trace formula constraint: discrete sum d_k·ĥ_k(λ_k), geometric side orbital integrals, Eisenstein remainder. Extract σ_k = max allowed off-line distance. **(2)** Plot σ_k vs k. Determine convergence rate — expect σ_k ~ C/k² if geometric side grows as k². **(3)** Compute the critical exponent: multiplicities grow as k^{n_C-1}=k⁴, geometric grows as k^a. Find a. If a < n_C-1 = 4, the squeeze works and RH follows from the five integers. The argument is BST-specific: n_C=5 makes the manifold high-dimensional enough. Build as toy. | **Lyra** | **QUEUED** — after R-16 |
