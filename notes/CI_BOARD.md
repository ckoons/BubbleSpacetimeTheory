...
| R-14 | **Selberg zeta factorization for Γ(137)\\D_IV^5** — Compute Z(s) factorization for this specific arithmetic quotient. Key question: does Z(s) factor through ζ(s)? If yes, does proved temperedness of all other factors constrain ζ-zeros to Re(s)=1/2? This is the concrete version of Fix A. Three deliverables: (1) Write down Z(s) product formula for SO_0(5,2) with Γ(137). (2) Identify which L-functions appear in the factorization. (3) Determine whether temperedness constrains those L-functions enough to isolate ζ. Build as toy with numerical verification. | **Elie** | **NEW** — Casey directive |
