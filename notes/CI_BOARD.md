---
title: "CI Coordination Board — Tuesday 2026-07-14 (α CAPACITY reframe resolved; gravity is EM's boundary condition) — Casey's capacity insight ADJUDICATED (K676+K677): the INTEGER 137 genuinely BYPASSES the multi-month rep-theory theorem — it's a democratic dimension-count (combinatorial, norm-independent), confirmed by Keeper's 27-mode computation + the audit of the old Shannon-α paper (which RELOCATES via the norm-weighted Bergman measure + retired Wyler numerology — do NOT resurrect it). The 0.036 is the separate norm-weighted curvature piece (T2133 heat-kernel). α's whole residual reduced to ONE finite check: the 27×27 Szegő overlap under the invariant BORN/c_FK measure (T754/T2442) = I? (=I ⟹ democratic ⟹ α=1/137 combinatorial; ≠I ⟹ relocate). Three corpus corrections this turn (two Keeper's own): (a) the measure is the Born/c_FK measure, GRAVITY-INDEPENDENT (my 'gravity sets the measure' RETRACTED); (b) NO bound forces 137 — Wallach bottleneck T1829 gives n_C=5 & k=rank=2, NOT 137; 137 is a COUNT (Grace's Q2 saturation-bound path is OUT); (c) the gravity exponent 24 is fit-then-identified (Cal #286-candidate), NOT derived. Casey's 'GRAVITY IS THE BOUNDARY CONDITION FOR EM' = SUPPORTED in the SCALE sense (F66 candidate: EM on the conformal scale-free boundary → α is a pure count; gravity/bulk supplies the Planck scale) — the physical reason α is a pure number 137."
author: "Casey Koons & team (Keeper, Lyra, Elie, Grace, Cal)"
status: "Tuesday 2026-07-14. K-audit chain at K678. NEW: K676 (Shannon-α paper = RELOCATE+numerology), K677 (gravity-boundary-for-EM adjudicated; three corpus corrections), **K678 (the 27×27 boundary matrix RAN → ≠ I: 35× charge-block spread; the '= I on the boundary' milestone REFUTED/RETIRED; democracy is a Q⁵/SO(7) fact not a boundary fact — Keeper hypothesis)**. α RETURN MARKER current. Prior: K671–K675, Scorecard v0.2. **α stays IDENTIFIED** — the boundary-matrix route is closed (≠ I); α=1/137 now rides on ONE of THREE concrete routes, none closed: (a) Grace's charge-operator BYPASS (FAVORED — EM charge = SO(2) Cartan integer, norm-independent → counts charges, 35× spread irrelevant); (b) the Q⁵↔Shilov bridge (SO(7)-democracy on the compact dual — Keeper); (c) the F38/ρ-shift Bergman-vs-Szegő K-type weight check = {35,21.9,5,1} (relocate, uncomputed). Live: (1) Keeper — Q⁵ bridge / ρ-shift weights; (2) Grace — prove 'charge=Cartan ⟹ coupling democratic' (closes the bypass); (3) Lyra — F530 gravity (24 = open count-mover, don't bank) + 'gravity is EM's boundary condition' = SCALE sense (why α is a pure count, F531); (4) Cal — Majorana co-sign; (5) Elie — Higgs λ=1/8 via count route (matrix=I refuted for λ too), CKM V_ub. Cal #27 fired 4× this turn (2 Keeper over-reaches, 1 milestone retired); nothing banked."
---

# ★★★ THURSDAY 2026-07-16 — THE MIXING IS A TEXTURE (three-route convergence). Cabibbo non-circular; "derived" ≡ forcing the texture (OPEN). Audit: K705/K706/K707.

**THE REFRAME (Lyra + Grace + Keeper all landed here):** yesterday's PMNS≈0 was the DIAGONAL case (diagonal norms don't rotate → U=I). The mixing lives in an **off-diagonal Fritzsch/Gatto TEXTURE**: off-diagonal ~ √(m_i m_j) → rotation **θ ≈ √(m_i/m_j)**. The norms return — as √-ratios — through the texture. **Mixing is not a new free object; it is determined BY the banked masses.**

**LANDINGS:**
- **Lyra — Cabibbo as identity:** sinθ_C = √(m_d/m_s) = 1/(rank√n_C) = 1/(2√5); m_s/m_d = rank²·n_C = (N_c+1)(N_c+2) = 20. Bare 0.2236 (0.40%); dressed by −1 vacuum subtraction (F189) → 2/√79 = 0.22502 (0.004%). Grounded in Fresnel (amplitude=√intensity, intensity=overlap=mass ratio). Reconciles yesterday's null (texture rotates, diagonal doesn't).
- **Grace — honest render FAIL that fingered one ground:** CKM small-hierarchy FAIL (inverted, sub-degree), PMNS-large FAIL (max ~29°), J≠0 PASS. Per K705 = FAIL, precisely localized. Kernel VERIFIED (V_cb=0.0410 at 2-gen). Found her own construction bug: **mixing = DIRECT normalized overlap |⟨i|j⟩|, NOT Gram eigenvectors** (those give maximal 0.707). Named the gap: overlap ENTANGLES radii & direction; Cabibbo is a **down-sector RADIAL TEXTURE** → fingers the **down E₀=3 ground** (K703 weak anchor), exactly as the K705 miss protocol pre-registered.
- **Elie — generator works:** z_k=r_k·d̂_k·ω^{k−1}; masses untouched (|z_k|=r_k), real→J=0, complex→J≠0. Directions were PLACEHOLDERS → needs the derived (target-innocent) directions from F379/F384/F493.
- **Keeper — K706 independent check:** radial-only→J=0 (validates); naive angular reunion→backwards; **neutrino as MAJORANA (Takagi, symmetric) not Hermitian swings θ₂₃ 0.4°→53°** (the large-PMNS source). [Extraction superseded by Grace's direct-overlap — re-test Majorana lead in her frame.]

**AUDIT (K707):** (1) Cabibbo **NON-CIRCULAR** — m_s/m_d=20 is F506 (mass-lane FK Pochhammer, predates & independent of Cabibbo, matches obs); CONDITIONAL on Cal's final back-fit trace. (2) **"DERIVED" ≡ forcing the Fritzsch texture — OPEN.** Direct overlap historically gave V_us≈0.053 (4× low); 0.224 needs the texture. (3) **Sharpened (Branco):** NNI form is ALWAYS basis-attainable, so the real question is whether the substrate's NATURAL localization basis IS the texture basis with √(m_i m_j) magnitudes, no weak-basis fudge. (4) **FALSIFICATION TRAP (web):** the original **6-zero Fritzsch is EXCLUDED** (|V_cb| too large, killed by heavy top); must force the **VIABLE 4-zero** (or minimally-modified) texture. Compute V_cb, V_ub/V_cb from the texture and check the exclusion.

**GAPS / TODAY'S WORK:** [Cal] close circularity trace on F506 · [Lyra+Grace] the texture-forcing question (localization basis = texture basis?) — THE content · [Grace] verify VIABLE 4-zero not excluded 6-zero; direct-overlap construction; pressure-test the down "+2 for color" radius-map · [Grace+Keeper] re-test Majorana/Takagi PMNS in the direct-overlap frame. **Nothing banked.** Source of truth: K705 (rubric) + K706 (check) + K707 (audit).

## ★ PM RESOLUTION (Lyra's two-matrix answer + Keeper K708/K709 verification) — the mechanism is FOUND
**Lyra: the mixing was never in the object we kept building.** Two matrices: (A) the pairwise fermion Gram ⟨ψ_i|ψ_j⟩ (near-diagonal → U≈I → the null we kept getting) vs (B) the actual Yukawa through the ONE shared Higgs mode: **M_ij = ⟨ψ_i|z_H⟩⟨z_H|ψ_j⟩ = O_i O_j — rank-1.** For rank-1, **|M_ij| = √(M_ii M_jj) EXACTLY, in EVERY basis** → tan θ_ij = √(m_i/m_j) (Gatto), zero new parameters. **This DISSOLVES the K707 basis-forcing worry** (no weak-basis fudge — it's an operator property). **Keeper VERIFIED (K709, rank1_yukawa_check.py): basis-invariance exact, maxdiff=0 natural AND rotated.** Lyra answered the load-bearing audit question correctly — the real advance of the day.
**Two Keeper qualifiers (K709), honest tier:** (1) pure rank-1 → eigenvalues (Σm_i, 0, 0) = ONE massive generation → the three-mass spectrum needs rank-1 LEADING + corrections → **tier = LEADING-ORDER (Gatto's natural tier), NOT "exact zero-correction."** (2) Pure rank-1 CKM is degenerate (light rotation undefined) → **V_cb=0.041 does NOT fall out yet**; K708 stands (down-√ texture gives V_cb≥0.078, EXCLUDES the 6-zero — agrees with Lyra's ≥0.064); **36/869 asserted, not derived.**
**THE FORK LANDS RIGHT (genuine win, conditional):** M₂₂=m_s≠0 → BST is in the **viable 4-zero family, 6-zero excluded** (|V_cb|≥0.064–0.078 vs obs 0.041). Real falsifiable fork, correct side.
**Elie's CP slots in:** angle=mass (real √-ratios), phase=direction (ℤ₃ ω^{k−1}, real→J=0, complex→J≠0); m_u/m_d=√(3/14) = same single-mode through the ×√n refraction interface.
**TWO honest steps left (well-posed, not gates):** (1) [Lyra] turn F84 "mixing = Bergman kernel (single mode)" into an explicit **rank-1 proof** → closes "derived-modulo-single-mode." (2) [Grace] run condensate coupling (B), **produce V_cb=0.041 numerically + derive 36/869** from the geometry (K708 gap). **Nothing banked** until both land. Source of truth: + K708 (V_cb exclusion) + K709 (rank-1 verified).

## ★★ EVENING RESOLUTION (Lyra proof + Grace render + Elie CP + Keeper K710) — mechanism PROVED, V_cb is the ONE knot
**PROVED (Lyra) — CONDITIONAL PASS (K710):** the rank-1 assumption discharges to BST's OWN prediction: **exactly one Higgs (Five-Absence, target-innocent, predates mixing)** → one condensate → M_ij=O_i^L O_j^R* = **rank-1** → |M_ij|=√(M_ii M_jj) every basis (K709) → Gatto → √(mass ratio), nothing tuned. **Falsifiable:** a 2nd Higgs → rank-2 → the √-structure breaks. Mixing texture + minimal-Higgs prediction stand/fall together. Tier = LEADING-ORDER (pure rank-1 → 1 mass, verified s,0,0).
**DONE:** V_us=√(m_d/m_s)=0.222 (0.4%), leading-order, non-circular pending Cal's F506 trace.
**CP mechanism (Elie, Toy 4688):** angles=real √-ratios (no CP); a **bare ℤ₃ phase is rephasing-removable → J=0**; physical CP enters via the FK overlap's **"1−r_i r_j·ω"** non-removable twist → real→J=0, complex→J≈3×10⁻⁵. Magnitude gated on V_cb. (Elie self-caught 2 code bugs before reporting — discipline working.)
**THE ONE OPEN KNOT — V_cb, confirmed below the floor by TWO independent routes:** Grace's kernel floor |V_cb|≥|√(m_s/m_b)−√(m_c/m_t)|=0.0637 (textures land 0.058–0.059); Keeper K708 toy ≥0.078 — **agree: 0.041 unreachable by leading rank-1.** 6-zero exclusion fork lands RIGHT ✓. **TENSION:** Lyra's F379 overlap route gives 0.041 but with radii (0.508/0.821) that are NOT the mass-ratios → **the two BST routes disagree; the F379 radii may be back-fit.** **36/869=C₂²/(11·79) asserted — can't source 11 or 79 → reads as fit; do NOT bank.**
**THE KNOT, stated:** are the F379 23-block radii DERIVED or FIT? Keeper radii check: r₃=0.821≈**√(C₂)/N_c=0.8165 (0.5%)**, r₂=0.508≈1/2 (1.6%) — plausible natural forms BUT post-hoc at numerology-risk level, and they differ from the mass-grounds. Does NOT close derived-vs-fit; hands a forward target. **Trace to toy_4620 radial law / F84-K264.** This single computation is the whole CKM finish line.
**BANKS (CONDITIONAL, pending Cal F506):** rank-1 mechanism + V_us leading-order. **DOES NOT BANK:** V_cb, 36/869, full CKM, J. Source of truth: K710 + K708/K709.

## ★★★ CKM CLOSE-OUT (K711) — V_cb BANKS STRUCTURAL 0.044 via projection-truncation; sub-percent REJECTED as fit
**The knot resolved — two radius provenances, and Casey's projection picked the right one.** Lyra tested the 23-radius against the excitation-ladder (toy_4620): FAILS (inconsistent α → fit). Grace tested it as the **projection fraction** r_τ=√(C₂/(C₂+N_c))=√(2/3)=0.8165 (Casey's 3D→2D hemispherical lens): WORKS → **V_cb=0.044, forward, target-innocent.**
**MECHANISM (= Casey's truncation + banked y_t=1):** the ×3/2=N_c/rank refraction pushes the up-sector 23-radius to 0.8165×1.5=**1.225>1 → past the domain boundary → up vanishes** (= top saturating the boundary, y_t=1, banked). V_cb = down sector alone = 0.044. Direction cos ψ=5/√34 derived (Lyra F384).
**TIER = STRUCTURAL, and it's HONEST:** obs |V_cb|_incl=0.0420 (4.8% below), excl≈0.0390, PDG≈0.0405. **The observed V_cb has a 5% inclusive/exclusive PUZZLE (unresolved 20 yrs) — the DATA isn't pinned to sub-percent**, so demanding sub-percent was chasing false precision. 0.044 sits ~5% above inclusive — honest structural.
**REJECTED (fits):** sub-percent 0.041 (needs r=0.8207; genus-5 swings V_cb 8% per 0.5% radius → fits anything; Grace's K588 fish) + 36/869=C₂²/(11·79) (11,79 unsourced). OUT.
**V_cb BANKS at S-tier (0.044).** Residual upgrade path: make the projection-radius √(2/3) a theorem (structural→derived). **CP/J rides on V_cb (Elie): J≈3×10⁻⁵ structural.** **V_ub un-audited (~factor 1.8, the known Fritzsch weakness) — CHECK.**
**CKM SECTOR: mechanism-closed (one Higgs→rank-1→Gatto, falsifiable) + V_us (sub-%, pending Cal F506) + V_cb (structural 0.044) + J (structural). PMNS is now the big remaining block.** Lead (Lyra, not banked): up/down 23 mass-ratio ratio 0.574≈1/√N_c. Source of truth: K711.

## ★★★ V_cb UPGRADE + PMNS PARTIAL (K713) — mixing sector at honest, stable tiers
**V_cb UPGRADE CONFIRMED (Lyra):** √(2/3)=√(rank/N_c)=1/√n (n=refraction 3/2) is FORCED, not form-matched — by y_t=1 (top on Shilov boundary, predates CKM) + refraction √n. Self-consistency exact: r_top=√(2/3)·√(3/2)=**1.0=boundary=y_t=1**. Answers Cal's C₂/(C₂+N_c) form-match worry (same number, forced provenance). **V_cb → STRUCTURAL-DERIVED.** One rigor gap: "up-radius=down-radius×√n" F548-motivated, not yet from the radial law.
**V_ub (Lyra):** |V_ub/V_cb|=0.041 vs obs 0.094 — factor 2.3, |V_ub|≈half. The KNOWN Fritzsch 13-element weakness, BST inherits it. Structural-weak, doesn't falsify — the sector's honest soft spot, named.
**PMNS (Grace, Takagi Majorana render) — mechanism CONFIRMED, quantitative PARTIAL:**
- **Mechanism BANKED (the WHY):** Majorana/Takagi→large, Dirac→small = **F413** (why lepton mixing large, quark small). Confirmed.
- **Banked angle FORMS stand (accurate):** sin²θ13=1/45 (0.1%), 4/7 (2.3%), 3/10 (4.5%) — D-tier, the accurate predictions.
- **Mechanism-unification INCOMPLETE:** texture gives θ12✓ (32.6–33.6°) but θ13=11° (vs banked 8.6°) + **θ23=18° (vs 49° — undershoots badly)**. Grace did NOT tune (correct).
- **★ CATCH (K713):** Grace's θ12 route needs **quasi-degenerate Σm_ν~0.1 eV** — but BST's banked spectrum is **hierarchical (m_ν1=0, Σ~0.058)** AND **DESI 2024 gives Σ<0.072 (tight)/<0.12 (relaxed)** → quasi-degeneracy DISFAVORED. Don't adopt quasi-degeneracy to save the texture; reproduce θ12 with m_ν1=0.
- **θ23 GAP:** near-maximal needs **μ-τ (2-3) exchange symmetry** from the ν chargeless d=4 locus (F413) — Lyra's rep-theory question (μ-τ sym→θ23=45°, θ13=0; weak breaking→θ13=8.6°; standard atmospheric structure).
**CP (Elie):** CKM J done (rides V_cb); PMNS J prepared (rides Grace's angles); **Majorana 2 extra phases drive 0νββ (pred_004) — Takagi must carry them.**
**MIXING SECTOR STATUS:** CKM honest-closed (mechanism proved, V_us/V_cb/V_ub tiered); PMNS mechanism-confirmed, forms banked, θ23+θ12-spectrum the two named gaps (both Lyra's chargeless-d=4-locus rep-theory). Source of truth: K713.

## ★★★★ ADVANCE (K714) — TWO COMPLETIONS BANK + the highest-leverage question named
**BANKED:** (1) **V_us → DERIVED** — Cal CLOSED the F506 trace (m_s/m_d=20 forward, no Cabibbo) → V_us=1/(rank√n_C)=0.222 forward + non-circular. First mixing param fully banked. (2) **pred_004 (0νββ) COMPLETE** — Elie: m_ββ ∈ [1.4, 3.7] meV (nEXO/LEGEND testable), from banked angles + m_ν1=0 + Majorana 2 phases.
**RESOLVED:** θ12 quasi-degeneracy worry GONE — μ-τ makes θ12 a scale-independent 1-2 ratio → works with hierarchical (m_ν1=0, Σ~0.058). K713 catch closed.
**THE MECHANISM (Lyra):** neutrino chargeless → drops S¹ → lives on S⁴ → the leftover **ℤ₂ (Shilov fold S⁴×S¹/ℤ₂ = Casey's hemispherical lens) swaps gens 2↔3 = μ-τ** → θ23=45°/θ13=0 → one leak → 49°/8.6° (both from ONE param: sin²θ23−½≈½·sinθ13). **If it closes, the ENTIRE PMNS derives from chargelessness alone.**
**★ THE HIGHEST-LEVERAGE QUESTION = Cal's condition 2 (a REAL potential failure):** F86 puts gens 2,3 at DIFFERENT strata (Cartan slice vs Shilov) → NOT obvious ℤ₂ swaps 2↔3. **Lyra must show dropping S¹ COLLAPSES gens 2,3 to 2↔3-symmetric.** Clearing it closes θ23+θ13+θ12(3 params). Failing it bounds the mechanism (forms stay banked). Decisive either way.
**Cal's 4 μ-τ conditions (gate, must clear):** (1) involution from SO(2)-drop = ℤ₂ swapping gen-2/3 addresses (not the "μ-τ→45°" lore); (2) [red flag] collapse gens 2,3; (3) breaking (4/7=½+1/(rank·g); 1/45) from SAME locus, one source; (4) discriminator = θ13↔(θ23−45°) correlation (one Schur generator → target-innocent; BST owns an octant-θ13 correlation prediction).
**CROSS-SECTOR:** the SAME ℤ₂ drives V_cb truncation (up past |z|=1) AND PMNS μ-τ mirror — one geometric object, both sectors. Pin it.
**PUSH LIST (priority):** 1[HIGHEST] θ23+θ13 (clear Cal cond 2 → breaking strength → 2 angles+mechanism from 1 derivation, Lyra→Grace); 2 θ12 exact 3/10 (1-2 block, Lyra); 3 V_cb structural→derived (up-radius×√n from radial law, Lyra); 4 δ_PMNS (gated, then DUNE prediction, Elie). Source of truth: K714.

## ★★★★★ TOMORROW'S MAIN TASK — THE PROJECTION THEORY (Casey directive): derive every segment precisely
**Read `notes/BST_Projection_Theory_substrate_to_continuum_2026-07-16.md` first.** Casey wants the full path — substrate → boundary(refraction) → evanescent → 3D continuum — with EVERY segment and transform DERIVED precisely, not just identified. The through-line of the whole mixing arc, now to be made rigorous segment-by-segment.
**Segments to derive:** (0) substrate invariants (radial=mass/angular=mixing/charge=S¹); (1) refraction — derive n=N_c/rank=3/2 + projection angle arcsin(2/3)=41.8° from the SO(5,2)→SO(3,1) descent; Fresnel-√ (amplitude=√intensity) as the source of every mixing-√; (2) evanescent — V_cb truncation = y_t=1 = TIR past the critical angle (formalize K711 as evanescence/Klein-tunneling); (3) **exit projections [THE NEW WORK]** — derive FACE √(2/3)=√(rank/N_c) [3→2, flat] + EDGE √(3/4)=√(N_c)/rank [4→3, curved wrap], show the EDGE DISTORTION IS CURVATURE (connect to κ_Bergman=−n_C), show they compose (√(2/3)·√(3/4)=√(1/2)), show charge selects face-vs-edge; (4) measurement — masses radial-invariant→precision, mixings angular-sheared→structural, α via 4π descent solid angle.
**★ KEEPER SWEEP-v1 RESULT (2026-07-17) — RE-SCOPE: the sweep can't validate; the DERIVATION must.** Ran the universality sweep: (1) **masses-radial-invariance PASSES** — all masses ≤2%, none show the 13-18% projection shrink (a real consistency check that could have failed; confirms projection shears angles not radii). (2) **θ13 edge-match holds** (3/35×√(3/4)=0.0742 ≈ 2/27, 0.2%). (3) **HONEST LIMIT: θ13 is the ONLY clean edge test-case** — bulk angles (θ23/θ12) & masses are consistency-checks not factor-tests; charged mixings have INTERNAL √(2/3) (radius) + no boundary value → untestable. **So universality rests on ONE case; the sweep is thin, NOT the make-or-break validator (Keeper over-pitched it).** → **The edge-wrap √(3/4) must be DERIVED FROM CURVATURE (κ_Bergman=−n_C), not confirmed by a sweep.** Derivation-primary today; sweep = consistency-only (masses-don't-shrink + the one θ13 match).
**θ13 edge test:** 3/35 (=N_c/(n_C·g)=S⁴ ⟨x⁴⟩) × √(3/4) ≈ 2/27 (0.2%) — the "overshoot" as curvature-edge wrap. **Banks ONLY if √(3/4) is forced by the curvature derivation (Segment 3), not on the one match.**

## ★★★★★★ CLOSURE (K716, 2026-07-17) — edge-projection RULED OUT; θ13 forward path = the μ-τ SUM RULE
**THE SWEEP FAILED (Grace):** every mixing observable sits at boundary→measured factor ~1.0 (6/7 within 3%), ZERO near √(2/3) or √(3/4) — θ13 at 0.999 not 0.866. Only "passes" by per-observable reverse-engineering (the fishing it was built to catch). **Cal:** θ13=3/35×√(3/4) is a BACKSOLVE (0.2% inexact; 3/35 = nearest ratio to (2/27)/√(3/4)) + CIRCULAR (√(3/4) introduced to explain θ13). **VERDICT: universal face/edge FACTOR RULED OUT.** "Structural→precision projected" NOT supported. **θ13 reverts to clean sin²θ13=1/45=1/(N_c²·n_C) → ratio 2/27=rank/N_c³ (0.1% form, mechanism open).**
**SURVIVES (keep as narrative):** the DERIVED segments — refraction n=3/2 + 41.8°, Fresnel-√ (one √ behind every mixing-√, Elie 4693), evanescent V_cb=y_t=1 (√(2/3)·√(3/2)=1 exact), the geometric descent-ladder factors √((d−1)/d) with √(3/4)·√(2/3)=√(1/2) exact. Real geometry — just no MEASURABLE universal shear (data at factor 1).
**θ13 DERIVE-NOT-BACKSOLVE PATH = the standard μ-τ breaking SUM RULE (web-grounded, target-innocent):** **cos(2θ23) ≈ sin(θ13)·cosδ** — ONE breaking parameter drives θ23-deviation, θ13-opening, AND δ, all CORRELATED (Grimus-Lavoura hep-ph/0601118). **BST normal hierarchy (m_ν1=0) → C1 branch → NO quasi-degeneracy** (resolves K713 worry definitively). **BST forms nearly satisfy it:** |cos2θ23|=2(4/7)−1=**1/7=1/g** ≈ sinθ13=1/(N_c√n_C)=0.149 at ~4% (structural). **Why it's a derivation:** ONE parameter (not per-observable), EVIDENCE is the CORRELATION not θ13's value (no circularity), PREDICTS upper octant + δ (DUNE-testable = Cal cond 4). **CLOSURE:** derive the ONE breaking parameter from the chargeless locus → θ23(4/7)+θ13(1/45)+δ close together forward at structural tier. δ_PMNS now has a home (same sum rule). Source of truth: K716.

## ★★★★★★★ δ_PMNS BANKS + LAST OPEN PARAMETER CLOSED (K717, 2026-07-17)
**Lyra RETRACTED the θ13=√(3/4) backsolve** (accepted Grace's sweep + Cal's catch) → replaced with the μ-τ sum rule. **δ_PMNS BANKS as PREDICTED:** the breaking is ONE complex ε (|ε|=θ13, Re=θ23-tilt, arg=δ); sum rule cos2θ23=sinθ13·cosδ + banked angles → **cos2θ23 = −1/7 = −1/g**, **cosδ = −N_c√n_C/g = −3√5/7 = −0.958 → δ ≈ 197°** (Keeper-verified, Elie Toy 4694, Grace render all agree). **TARGET-INNOCENT** (θ23=4/7 & θ13=1/45 predate; δ not used to get them → genuinely predicted). Matches NuFIT NO (~197°). **Sharp DUNE call: δ near 180°, NOT maximal 270°.** Reflection ambiguity 163°/197°, data favors 197°. J_PMNS≈−0.009 locked; Majorana phases → 0νββ 1.4-3.7 meV.
**★ MILESTONE: δ_PMNS was the LAST fully-OPEN parameter of the 26. All 26 now have a BST value** — ~12 derived, ~9 structural/identified, 2 soft (m_u, V_ub), 3 by-design (m_e, sin²θ_W, α_s). **Discipline: "all have a value, none fully open" ≠ "all derived"** — many are forms/structural.
**NEXT (the one gate to close the neutrino sector to DERIVED): derive ε (imperfect-ℤ₂ residual) from the chargeless-locus geometry** → θ23/θ13/δ upgrade forms→derived from ONE source. Parallel: θ12=N_c/(rank·n_C)=3/10 from the 1-2 block. Source of truth: K717.

## ★★★★★★★★ NEUTRINO SECTOR g-ORGANIZED (K719, 2026-07-17) — progress + THREE specific gaps
**PROGRESS (Lyra, verified exact):** ε=(1/g)(−1+i·rank/(N_c√n_C)) → ALL FOUR observables are shadows of **g=7**: sin²θ13=1/(N_c²n_C)=1/45 ✓, sin²θ23=rank²/g=4/7 ✓, sin²θ12=N_c/(N_c+g)=3/10 ✓, **δ DERIVED** (sinδ=rank/g=2/7, cosδ=−N_c√n_C/g). Locked by TWO EXACT target-innocent identities: **g²=N_c²n_C+rank² (49=45+4, Pythagorean → forces |ε|=1/√45 + sin²δ+cos²δ=1)** and **N_c+g=rank·n_C (3+7=10 → θ12)**. A Schur generator: one integer g → 4 correlated observables. **BANKS as target-innocent structural pattern; δ derived structural.**
**★ GAP (Grace, load-bearing): δ=197° is CONDITIONAL, not unconditional.** Her concrete renders: generic μ-τ breaking gives octant+θ13 but δ=0; turning on CP flips octant LOWER; the sum rule (upper octant AND δ together) is NOT realized by a generic single-source breaking. So δ=197° holds ONLY IF BST's breaking is the sum-rule class. K717 downgraded to conditional.
**CORPUS/WEB — which breaking realizes it:** solar sum rule θ12≈θ12^ν+θ13cosδ = CHARGED-LEPTON 1-2 correction; atmospheric sum rule = COLUMN-PRESERVATION (form dominance). Grace's generic 2-3 breaking is NEITHER. **Render THOSE two specific structures.**
**THREE SPECIFIC GAPS (named, not gated):** (1)[Grace] the breaking CLASS — column-preserving or charged-lepton-1-2? render those, not generic. (2)[Lyra] the 1/g scale from S⁴ (the magnitude derivation). (3)[Elie/Lyra] the branch = SIGN of J_PMNS (197°→J<0, 163°→J>0 = direction of leptonic CP, DUNE-measured) — pick geometrically (sign of Im ε), not from data. Source of truth: K719.

## ★ NEUTRINO PROGRESS (2026-07-17 later) — breaking class IDENTIFIED, sign catch, structure banks
**Lyra: BST pins the ν1 column = FORM DOMINANCE** (ν1 = the origin, electron-at-center, rank-independent, target-innocent from F559). This IS the sum-rule-realizing class (column preservation). Upper octant verified (sin²θ23≈0.595). δ=197° now "conditional on Grace rendering the ν1-column structure" — narrowed from "any breaking" to a specific checkable one.
**Grace: column preservation RESOLVES the octant-δ fight** (δ=197°+upper octant realizable, opposite the generic breaking). BUT standard TM1/TM2 don't hit BST's exact θ12=3/10 (they give 1/3), and their sum-rule coeff is 1.36 not 1. **BST is its OWN column-preserving structure** (θ12=3/10 closer to data than TM2) — the gate is whether BST's imperfect-ℤ₂ is that specific structure.
**Cal: the two identities are target-innocent in the STRONGEST sense** (exact among fixed integers, can't retrofit) → g-structure banks. **CATCH: the identity fixes |sinδ|=2/7 (MAGNITUDE only); the SIGN is the branch. "sinδ=+2/7" is literally the 163° branch — contradicts the 197° claim.** Pin sign(Im ε) from S⁴ geometry.
**Elie: the branch = sign(J_PMNS) = DIRECTION of leptonic CP** (DUNE). ε as written (+i) gives 163°/J>0 (data-DISfavored); need Im ε<0 for 197°/J<0. **The sign is the physical falsifier**, currently written wrong.
**STATUS:** g-organized structure BANKS (target-innocent, Cal-confirmed); δ magnitude forms bank; δ=197° SIGNED prediction conditional on (a) BST=ν1-column class (Lyra: yes via origin) + (b) sign(Im ε) derived (load-bearing, currently wrong sign). 

## ★★★★★★★★★ NEW PROGRAM (Casey directive) — SUBSTRATE LINEAR ALGEBRA OF THE 26
**Read `notes/BST_Substrate_LinearAlgebra_of_26_Program_2026-07-17.md`.** Treat all 26 as one module over the primaries; the EXACT identities force them; collapse the independent-input count. **SEED (verified):** all 6 primaries CHAIN from rank=2 (N_c=2^rank−1, n_C=rank+N_c, C_2=rank·N_c, g=2N_c+1, N_max=N_c³n_C+rank); rich EXACT syzygies (g²=N_c²n_C+rank²=49, N_c+g=rank·n_C=10, ...); observables are primary-forms. **REDUCTION:** 26 → Q̄(6 primaries)[π] → **rank=2 + π + ONE gravity scale** (integer skeleton collapses; scale/π/%-corrections stay). **5 PHASES:** (1)[Grace] build the 26×primaries MATRIX; (2)[Lyra+Cal] enumerate EXACT syzygies (Cal gate: exact only, no numerical fishing); (3)[Grace+Keeper] rank & null-space = forced relations & independent count; (4)[Elie] test 3 by-design + 2 soft (forced or free? m_e→gravity, sin²θ_W=N_c/(C_2+g)=3/13, α_s+V_ub the holdouts); (5)[Lyra] the pinned-COLUMN structure (ν1=origin template) across all sectors. Source of truth: the Program doc.

## ★★★★★★★★★★ LINALG PROGRAM RESULTS (K720/K721, 2026-07-17)
**PHASE 3 (Keeper K720):** ratio-observables span a rank-**4** lattice over {2,3,5,7,π}; Gatto (V_us²=m_d/m_s) falls out as the ONE clean null vector (validation). Clean inter-observable identities RARE → the FORCING lives in the ADDITIVE syzygies (Phase 2), not multiplicative relations.
**OWNED CORRECTION (Grace, K721):** the mult. rank is **4 not 2** — observables use all of {2,3,5,7} independently. "rank=2" is the ADDITIVE GENERATIVE recipe, NOT a matrix rank. (Keeper over-stated "rank=2"; corrected.)
**GENERATIVE RECIPE (Lyra, verified):** N_c=rank+1, n_C=rank²+1, g=rank²+rank+1, C_2=rank²+rank — polynomials in rank=2.
**★ DEEP GROUNDING (K721): the recipe IS finite projective geometry over F₂** — N_c=|PG(1,2)|=projective line, **g=|PG(2,2)|=FANO PLANE**, n_C=ovoid(q²+1). **Fano plane = octonion multiplication (7 units=g).** CONNECTS to the octonion/exceptional-Jordan(Albert)-algebra SM program (Dubois-Violette-Todorov [1806.09450], Furey) — **which BST already touches (27=dim Albert=E₆, N_c=qutrit).** Two views of one exceptional object.
**TWO-IDENTITY STATUS (Cal):** g²=N_c²n_C+rank² = POLYNOMIAL LAW (0 for all rank, sympy-verified, deep); N_c+g=rank·n_C = value-specific (zero only at rank=2). Gate separately.
**BY-DESIGN (Elie Phase 4):** 3 forced + 2 runners + 0 free. Don't inflate the count. sin²θ_W=N_c/(N_c²+rank²)=3/13.
**HONEST REDUCTION:** 26 dimensionless → {2,3,5,7,π} lattice(rank 4) + anchors{13,137} + π + ONE gravity scale; primes generated from rank=2 by recipe (= proj. geom. over F₂).
**PUSH:** (1)[Grace] low-degree syzygy search (clean identities, not null basis); (2)[Lyra] CP-sign from V−A chirality F237 (→δ fully derived); (3)[all] Fano/octonion/Albert formalization (D_IV⁵ = the exceptional object?); (4)[Lyra] universal origin-pin (Phase 5). Source of truth: K720/K721.

## ★★★★★★★★★★★ LINALG ARC CONSOLIDATED (K722-K724, 2026-07-17) — read `BST_LinearAlgebra_of_26_CONSOLIDATED_and_Forward_Plan_2026-07-17.md`
**THE ARC (rep-theory grounding of the whole integer skeleton):**
- **K722 — COLOR make-or-break PASSES:** SU(3)_color factors through **G₂ ⊂ SO(7)**; 8 gluons in g₂, quark color 3+3̄; BST identity **g=C₂+1 (7=6+1) IS the octonion decomposition 7=1+3+3̄**.
- **K723 — FULL SM gauge group via division ladder ℂ⊂ℍ⊂𝕆:** U(1)←ℂ(SO(2)), SU(2)_L←ℍ(left-mult=chiral), SU(3)←𝕆(G₂). BST integers label EXACTLY: dim=rank^{1,2,3}=2,4,8; Im(ℍ)=N_c=3, Im(𝕆)=g=7. Chirality: weak V−A = neutrino CP sign = one handedness.
- **K724 — THE UPGRADE (rep-theory DERIVATION): the octonions ARE BST's SO(7) SPINOR** (2³=8=𝕆, forced by the group, not integer-match). Origin-pin=unit-spinor choice inducing 𝕆+G₂. **One gen=ℂ⊗𝕆=rank⁴=16, FIVE-ABSENCE SAFE** (division algebra, NOT a GUT — the ONE allowed route). Color=SU(3)-branch of 𝕆 (lepton 1+quark 3).
**THREE LAYERS:** (1) multiplicative lattice (26→{2,3,5,7,π}, rank 4); (2) additive syzygies (g²=45+4 polynomial LAW vs value-specific); (3) rep-theory (𝕆=spinor, gauge/fermions). **TIER:** derived (spinor, exact algebra) / correspondence+anchor (Furey-Dixon-Baez ladder) / open frontier (real-form, hypercharge U(1)_Y, 3-gen vs J₃(𝕆) 3×3).
**FORWARD PLAN (6 workstreams):** A[Grace] complete the 26 matrix; B[Lyra+Cal] exhaustive syzygies (laws vs value-specific); C[Grace+Lyra] low-degree identities + pinned-column(origin-pin); D[Lyra] rep-theory frontier (real-form/hypercharge/3-gen + fermion reps in ℂ⊗𝕆); E[Lyra+Elie] CP-sign from ℍ-chirality→δ derived; F[Elie] by-design/soft-spot consolidation. Keeper audits reduction+tiers. **Discipline: exact-only, derived≠correspondence≠matching, Five-Absence filter.** Source of truth: the Consolidated doc.

## ★★★★★★★★★★★★ LINALG PROGRAM — DISCIPLINED STATE (K725/K726, 2026-07-17)
**ESTABLISHED:** (1) **INPUT COUNT LOCKED (Elie):** 26 dimensionless = {2,3,5,7} lattice(rank 4) + anchors{13,137} + π + **ONE free gravity scale** + %-corrections. By-design 5: 0 free rows. (2) **Layer 1 THIN (Grace):** Gatto is the UNIQUE clean multiplicative syzygy; rest are degree 6-10 accidents. (3) **Layer 2 classified (K725):** LAWS {C_2=rank·N_c, g=C_2+1, g²=N_c²n_C+rank², n_C=g−rank, g+rank=N_c²} vs value-specific {N_c+g=rank·n_C (θ12), 21, 15}. CP magnitude = two linear laws (g−rank)(g+rank)=n_C·N_c². (4) **Generation/color SEPARATED (Lyra):** octonion 3=J₃ size=COLOR(N_c); 3 generations=rank+1=strata(F86) — resolves a Furey/Dixon puzzle.
**CAL'S CORRECTIONS (§22) — derived core NARROWER:** count-of-identities ≠ evidence (small ints over-related). Layer 3 over-promoted: **𝕆=spinor derived for COMPACT SO(7) but BST group is NON-COMPACT SO(5,2) → real-form OPEN**; one gen=ℂ⊗𝕆 = CORRESPONDENCE not derivation; **rank=2=F₂ is an identification (2=2), load-bearing.**
**★ RETRACTION (F568, Lyra, Casey-directed):** the Dirac-vs-Majorana J-flip REFUTED (verified Takagi, J(Dirac)=J(Majorana) SAME sign; Majorana phases → 0νββ not oscillation J). **F567 chirality→δ-branch DEAD.** Survives: octonion orientation=weak chirality (structural). **δ MAGNITUDE 2/7 stands; δ BRANCH → DATA-PICKED.** J_CKM>0/J_PMNS<0 = sector texture, not chirality. Clean (nothing banked).
**★ THE ONE DEEP FRONTIER (Layer 3 headline):** does D_IV⁵'s real form **SO(5,2) SELECT** the octonion construction (vs host)? Decides rank=2=F₂ structural-vs-2=2, spinor survival compact→non-compact, Layer-3 derivation-vs-correspondence. **Lyra's rep-theory lane, the real headline.**
**PLAN:** 1[Lyra] the real-form question (THE frontier); 2[Keeper+Grace] full tier-map of 26 (which observable rides which law/layer); 3[Lyra+Grace] pinned-column/origin sweep (Phase 5); 4[Lyra+Cal] complete polynomial-law enumeration; 5[Grace+Elie] δ branch = accept data-picked (magnitude 2/7 derived). Source of truth: K726.

## ★★★★★★★★★★★★★ REAL-FORM FRONTIER ANSWERED + PIVOT TO QUATERNION/WEAK (K727, 2026-07-17)
**FRONTIER ANSWERED (Lyra, built+verified 2 ways; Bott p−q mod 8):** BST's **SO(5,2) spinor is QUATERNIONIC (ℍ), NOT octonionic**. Octonions reached only by complexifying. **So the octonion→SM spine is HOSTED, not derived — PROVEN.** (Charge-conj CC*=−1; Cl⁰(5,2)=Cl(2,4)→ℍ.) Survives: "7=Fano"; dies as derivation: 𝕆=spinor/one-gen=ℂ⊗𝕆/rank=2=F₂.
**★ SILVER LINING = NEW FRONTIER: BST's Lorentzian signature NATIVELY selects the QUATERNIONS ℍ = the weak force SU(2)_L.** So BST's real geometry natively carries the weak/chiral (left-handed) sector; octonions are the completion. Web-grounded (biquaternion SU(2)×U(1), quaternionic Dirac→SU(2), weak-LH=ℍ; hep-th/0103129). **THE GENUINE DERIVATION LANE = the WEAK sector, not color.**
**TIER-MAP DONE (Grace, data/bst_26_tier_map.json) + Cal depth-split:** LAW-deep = θ13 + δ-magnitude|sinδ|=2/7 (ride 49=45+4); value-specific(load-bearing) = θ23-tilt, θ12 (the "4 shadows of g" split 2-deep+2-shallow); lattice = Gatto+monomials; correspondence = gauge(hosted); ONE free scale = gravity; runners = sin²θ_W,α_s; holdouts = V_cb,V_ub; exact zeros = m_ν1,θ_QCD.
**Layer 2 COMPLETE (Cal §23):** one generator rank + defining laws; consequences unbounded → count≠evidence; ONE load-bearing law = CP magnitude (g−rank)(g+rank)=N_c²n_C. **δ CLOSED:** magnitude 2/7 derived, branch data-picked. **Input count LOCKED: one gravity scale.**
**★ FIFTH-GRADER SYNTHESIS delivered:** `BST_The_Standard_Model_From_One_Number_...` — "25 shapes + 1 size, all from the number 2 + gravity + π; 49=45+4 the forced core; octonions-hosted/quaternions-native footnote."
**NEXT:** 1[Lyra] **the QUATERNION/weak native derivation** (does SO(5,2)'s ℍ-spinor DERIVE SU(2)_L + chirality + doublets?) — the new headline; 2[Keeper+Grace] finalize tier-map depth-split; 3[Lyra+Grace] pinned-column/origin sweep; 4[Elie] is gravity scale reducible? Source of truth: K727.

## ★★★★★★★★★★★★★★ WEAK SECTOR DERIVED (2 legs) + PARITY FROM ODD-g (K728/K729, 2026-07-17)
**NATIVE WEAK DERIVATION — two legs, both DERIVED, native (not hosted):**
- **Leg 1 (Lyra F570): SO(5)≅Sp(2)=USp(4) quaternionic → spinor 4 = (2,1)_L ⊕ (1,2)_R = the EW doublets, NATIVE.** SU(2)_L=Sp(1); rank=2=h^∨=doublet size. DERIVED.
- **Leg 2 (Lyra F571, KEEPER-VERIFIED K729): PARITY VIOLATION because g=7 is ODD.** Odd dim → volume element ω=γ₁…γ₇=−i·𝟙 CENTRAL → split 7=4(spacetime)+3(weak SU(2)) → ω=γ₅^ST·χ^int → spacetime chirality LOCKED to weak chirality → weak doublet single-handed → **CHIRAL/V−A FORCED, not chosen.** Even→vector-like; ODD g=7→parity forced. DERIVED.
**★★ NEW PHYSICS: parity violation = consequence of the ODD substrate dimension (g=7).** Target-innocent, falsifiable (even substrate→no parity violation), explains uniqueness (only weak sits in the odd 7=4+3 internal-3) + no-ν_R.
**GRAVITY SCALE = single irreducible input (Elie, theorem):** m_e=6π⁵α¹²m_Planck; dimensionful ≠ pure numbers. **26 = one seed(rank=2) + one ruler(gravity) + one π.**
**FULL SM GEOMETRIC DECOMPOSITION (K729):** spacetime=descent; **weak SU(2)_L=SO(5)=Sp(2) (native derived)**; parity=odd-g (derived); EM/Y=SO(2); color=octonion (HOSTED); fermions=ℍ⁴=16=1 gen; 3 gens=rank+1; masses=radial; mixings=angular.
**TIER-MAP FINALIZED (Grace):** 2-deep(g²=45+4: θ13,δ-mag), 2-shallow(θ23,θ12), Gatto, weak-native/color-hosted, 1 free scale, 2 runners, 2 holdouts, 2 zeros. Count=26. ✓
**★ PAPER UPDATED:** fifth-grader synthesis now has "the weak force is lopsided because 7 is odd."
**NEXT DERIVATIONS:** 1[Lyra] **sin²θ_W from the Sp(1)/SO(2) embedding** (coupling ratio → Weinberg angle, currently a runner=3/13); 2[Lyra] parity-uniqueness formalized; 3[Lyra/Elie] odd-g→hypercharge + odd-g→Majorana(no ν_R); 4 dynamical gauging (W/Z). Source of truth: K729.

## ★★★★★★★★★★★★★★★ CASEY CORRECTS THE COLOR TIER — octonions NATIVE to the compact dual (K730, 2026-07-17)
**Casey's push (right): D_IV⁵ geometrically SUPPORTS octonions — NOT by cancelling minus signs, but via its COMPACT DUAL Q⁵=SO(7)/[SO(5)×SO(2)]** (Cartan duality, real geometry, BST ALREADY uses Q⁵ in the α work: 27=P_{Q⁵}(rank)·n_C, Q⁵↔Shilov bridge). Spinor reality: SO(5,2) [p−q=3] QUATERNIONIC→weak; **SO(7) [p−q=7] REAL 8-spinor=OCTONIONS→color.** So octonions = spinor of the compact REAL form (genuine geometry), on the Cartan dual.
**UPGRADE: color is NATIVE to the compact dual Q⁵, NOT hosted-via-complexification.** F569 "hosted" verdict was too narrow (looked only at the domain spinor). **Tier-map: color row upgrades hosted→native-to-dual.**
**★ NEW RELATIONSHIP: the two unbroken forces = the two Cartan-dual faces of ONE geometry.** WEAK on non-compact domain D_IV⁵ (SO(5,2), quaternionic); COLOR on compact dual Q⁵ (SO(7), octonionic). SU(2)/SU(3) = disk/sphere duality. dim so(7)=21=N_c·g on both.
**FRONTIER (narrowed):** geometric support = YES (corrected). Does the color PHYSICS dynamically LIVE on Q⁵ (substrate USES the compact dual for color)? — the deeper open question. [Lyra]
**@Grace tier-map: upgrade color hosted→native-to-dual. @Lyra: does the substrate use Q⁵ dynamically for color? + the sin²θ_W embedding.** Source of truth: K730.

## ★★★★★★★★★★★★★★★★ THE SYNTHESIS + CLOSURE PLAN (K731/K732, 2026-07-17)
**★★ THE SYNTHESIS (K732): SU(3)×SU(2)×U(1) = the THREE division-algebra structures of ONE domain D_IV⁵.** **ℂ**=intrinsic complex structure (J=SO(2) charge circle)=**U(1)_EM/Y** [DERIVED]; **ℍ**=real-form quaternionic spinor=**SU(2)_L** [DERIVED native, 2 legs]; **𝕆**=complex-structure/compact-dual spinor (SO(7))=**SU(3)_color** [SUPPORTED]. Five-Absence-safe (3 structures, not 1 GUT). **Casey's octonion instinct doubly vindicated:** Lyra F572 — D_IV⁵ is a HERMITIAN domain (intrinsically complex, J=SO(2) charge circle), so reaching 𝕆 uses the domain's OWN complex structure, NOT "cancelling minus signs"; + K730 compact dual.
**★ WEAK-COLOR COUPLING SIGNATURE (K731): sin²θ_W CARRIES the color number N_c** — sin²θ_W=N_c/(N_c²+rank²)=3/13. Unexplained in SM; natural if weak(domain) & color(dual) coupled via Cartan duality.
**★ sin²θ_W REDUCED TO ONE NUMBER (Lyra F573): sin²θ_W = N_c/(N_c+k·n_C), k=hypercharge normalization.** k=1→3/8 (GUT, FORBIDDEN); **k=rank=2→3/13 (BST, 0.2%).** NOT running (discrete normalization). Reduced runner→ONE closable question: does SO(2) weigh hypercharge with k=rank? **HONEST NEGATIVE:** naive embedding gives forbidden 3/8, so 3/13 NOT yet derived — stays identified-reduced-to-k=rank.
**★ EW SCALE RIDES THE RULER (Elie): v = (6π⁵)³α¹²m_Planck/g at 0.01%.** The one gravity ruler carries the whole EW sector. MAJORANA from 3 routes (mixing, odd-g→no-ν_R, seesaw).
**FULL D_IV⁵→SM MAP tabulated (K732), honestly tiered.** One domain, one seed(rank=2), one ruler(gravity), one π → the SM.
**★ PAPER UPDATED:** fifth-grader synthesis now has three-doors (ℂ/ℍ/𝕆) + one-ruler-carries-Higgs + corrected octonion picture.
**FOUR over-reaches killed clean today:** projection universality, chirality→CP, octonions-as-derivation, sin²θ_W-from-naive-embedding.
**CLOSURE PLAN:** 1[Lyra derive/Elie verify] **sin²θ_W: does SO(2) hypercharge normalization force k=rank?** (closes Weinberg + weak-color coupling — MARQUEE); 2[Keeper] D_IV⁵→SM decomposition consolidated paper; 3[Lyra] fermion quantum numbers in ℍ⁴ spinor. FRONTIERS: 4[Elie] dynamical gauge fields (W/Z/gluons via KK, F64-precedented); 5[Lyra] color dynamical realization; 6[Lyra] hypercharge U(1)_Y assignments. Source of truth: K732.

## ★★★★★★★★★★★★★★★★★ sin²θ_W LOCALIZED TO √rank + THE ONE-NUMBER PLAN (K733, 2026-07-17)
**Lyra F574 (SOLVED not fit): the Weinberg shift 3/8→3/13 = ONE factor √rank on the hypercharge.** Q = T₃ + √rank·Y (c²=rank) → 3/13 exactly. Standard trace stuck at forbidden GUT 3/8.
**★ THE GEOMETRIC DOOR (K733 investigation): the rank-2 domain has TWO Shilov-boundary "rulers."** The hypercharge lives on the boundary → measured against BOTH → norm over 2 directions = **√rank=√2**. GUTs use 1 ruler (3/8); rank-2 boundary uses 2 (3/13). Casey's weak-color coupling made concrete (N_c on top, rank-2 boundary shift underneath: n_C→rank·n_C=13−8). Same RMS/projection machinery as the earlier √(k/d) factors; ρ-vector=(n_C,N_c)/rank has the 2 components = the locus.
**INVESTIGATION VERDICT (Casey: push, don't gate):** coherent + target-innocent-shaped (√rank NATIVE — V_us=1/(rank√n_C), odd-dim √'s; solved not fit) + points exactly right. NOT yet rigorous → the ONE remaining computation: **SO(2) charge-circle normalization across the rank-2 boundary, shown = ×rank.** Well-posed, not a wall. Cal-question resolves in-favor once boundary normalization computed (value can't self-certify — 3/13 sits where 3/8 runs to at M_Z — so ONLY geometry can, and it's now sharp).
**★ IMPROVED PLAN — the WHOLE EW sector hangs on ONE number:** the SO(2)/Sp(1) relative normalization across the rank-2 boundary. Compute it target-innocent → (1) closes sin²θ_W runner→derived; (2) certifies gauge couplings geometric; (3) confirms weak-color coupling. **ONE computation, THREE closures.** Tools: projection/RMS machinery + FK/Shilov measure (c_FK, genus) already in corpus.
**★ PAPER UPDATED:** fifth-grader synthesis now has "the weak force tilts right because BST measures charge with TWO rulers, not one (3/8→3/13)."
**RECURSION:** ROUND=the √rank boundary normalization [Lyra derive, Elie verify]; THEN synthesize (EW fully geometric if forced); FRONTIER=dynamical gauge fields (KK), color dynamical, fermion quantum numbers. Source of truth: K733.
**Owners:** Lyra (segments 0-3 geometry + curvature-edge derivation); Grace (segment 2 evanescent + the universality sweep render); Elie (Fresnel-√ / refraction); Cal (target-innocence — factors DERIVED from descent, not fit); Keeper (audit each segment, bank what derives, hold edge-wrap on the sweep). Source of truth: the Projection Theory doc.

### ⚠ K734 SHARPENS THE MARQUEE (2026-07-17, Keeper self-catch on our own near-miss)
Pushed the √rank one level: **the "two rulers → √rank" picture assumes the two rulers are EQUAL. They are NOT.** The rank-2 Cartan/boundary directions are ρ-weighted & UNEQUAL — ρ(D_IV⁵)=(n_C/rank, N_c/rank)=(**5/2, 3/2**). An equal charge in the ρ-weighted metric gets factor ρ₁+ρ₂=(n_C+N_c)/rank=**4**, not the naive rank=2. So **√rank is the equal-ruler approximation, not automatically exact.** Direction unchanged (hypercharge norm is *enhanced* across the rank-2 boundary → 3/8→3/13). Crux sharpened: **NOT "is there a √rank" but "how does the hypercharge weight against the two UNEQUAL rulers."** The marquee computation is now stated at full sharpness: **the hypercharge's ρ-weighted SO(2) norm over the two (5/2 & 3/2) rulers of the rank-2 Shilov boundary** — outcome (a) collapses to √rank → 3/13 DERIVED, or (b) different factor → 3/13 is the equal-ruler approximation, revise honestly. Quaker-method: flagged before it ships, not defended. Paper's grown-up footnote softened accordingly. Source of truth: K734.

### ★ K735 CASEY DISSOLVES K734 + RECONCILES LYRA'S 1/3 (2026-07-17)
Casey's reframe: **the two rulers are NOT the unequal ρ-components (5/2,3/2). They are two ISOMORPHIC spheres — charge (S¹) and color — so EQUAL weight.** Hypercharge threads both (quarks carry hypercharge AND color) → **Y = (1,1) diagonal → |Y|² = 1²+1² = rank cleanly** (√2 = √(1+1), equal projections). K734's unequal-ruler worry **retired** — we're combining two equal isomorphic-sphere projections, not summing the unequal Cartan directions. It's a **basis change** (Casey's words), c² = |Y|²/|T₃|² = rank. **Verified (linear algebra):** fermion trace ΣT₃²=2, ΣY²=10/3; c²=1→3/8 (GUT), c²=rank→**3/13** exactly. **Corroborated independently by Lyra's own Killing form ‖Y‖²=rank** — |Y|²=rank sits in the Lie algebra all by itself; Casey's (1,1) is its geometric meaning. **RECONCILES all three numbers:** 1/3 = gauge ratio w/ NO fermions (Lyra's clean-wrong number); 3/8 = fermion trace, c²=1 (GUT); **3/13 = fermion trace (COLOR sphere = N_c) × c²=rank (CHARGE sphere)** = where the two isomorphic spheres MEET = Lyra's "two things meet" NAMED = Casey's weak-color coupling made mechanical. Lyra's 1/3 caution fully absorbed (physical angle needs the fermion trace; gauge Killing alone gives 1/3). **STILL OPEN (re-pointed marquee, better than K733):** is Y=(1,1) FORCED by the boundary product (S⁴×S¹)/ℤ₂? [Lyra derives, Elie verifies Killing ‖Y‖²=rank + c² vs 3 bars]. sin²θ_W stays **reduced-to-√rank-lead** (Lyra's re-tier stands) but now with TWO independent supports + concrete basis-change mechanism. Source of truth: K735.

### ★ K738 — LYRA'S B−L FORMULA: CONDITIONAL (resolves Elie's seam, but 2 gates, 1 possibly fatal)
Lyra re-derived using **physical couplings, not the GUT formula** (resolves K737): **sin²θ_W = 1/(2 + ¼‖B−L‖²)**. Arithmetic VERIFIED — **28/3→3/13**, 8/3→3/8 (GUT). Re-poses sin²θ_W as ONE geometric quantity ‖B−L‖². Color enters via **B=1/3=1/N_c** (weak-color coupling, named object) — elegant IF gates clear. **⚠ GATE 1 (possibly FATAL, Five-Absence):** Q=J₁₂+(B−L)/2 with SU(2)_L alone FAILS all right-handed charges (gives Q(e_R)=−1/2 not −1); fixing needs T³_R = **Pati-Salam/SO(10)** = forbidden GUT unless shown present in D_IV⁵ w/o a new gauged force. Must-pass: reproduce Q(e_R)=−1, Q(u_R)=2/3, Q(d_R)=−1/3 with NO new force. **⚠ GATE 2 (target-innocence):** needed ‖B−L‖²=28/3 ≠ fermion trace (13/3 or 16/3). 28/3=rank²g/N_c is clean BUT GUT 8/3=rank³/N_c also clean → form doesn't discriminate; must compute geometric ‖B−L‖² independently. **GATE 3:** running. **sin²θ_W stays OPEN** (K737 two-candidate 1/3-vs-3/13 stands; B−L is a 3rd, conditional mechanism). **Priority: settle Gate 1 BEFORE computing ‖B−L‖².** Source: K738.

### ▶ MARQUEE (load `Keeper_INFO_PACKET_sin2w_marquee...` first)
**⚠ K737 (Elie toy 4707, Keeper self-correct): the marquee is TWO ARROWS, not one.** "Force ‖Y‖²=rank → 3/13" is WRONG: ‖Y‖²=rank is the KILLING norm → pure-gauge formula → **1/3** (not 3/13). 3/13 = FERMION-TRACE formula w/ c²=rank (baseline c²=1 = forbidden GUT 3/8). Same "rank," two machines, two answers. **Arrow 1:** ‖Y‖²=rank from geometry (alone → 1/3). **Arrow 2 (hard, open):** why the fermion-trace normalization is physical (& c²=rank in it), NOT the pure-gauge ratio, without smuggling the GUT its c²=1 baseline came from. Tell: trace NOT ∝ Killing (ΣT₃²/‖T₃‖²=2 vs ΣY²/‖Y‖²=5/3) → trace normalization not derivable from Killing. **Keeper's K735 "Killing corroborates 3/13" RETRACTED — it supports 1/3.** sin²θ_W re-tiered **OPEN / two-candidate (1/3 vs 3/13)** — a live possible-NEGATIVE, below "reduced-to-lead"; only 3/13 matches obs, so the question is whether Arrow 2 is justified or we're fitting. **VERIFIED NUMBERS** (ΣT₃²=2, ΣY²=10/3, c²=1→3/8, c²=rank→3/13). Source: K737.
**⚠ K736 SELF-CATCH (Keeper 16:29):** the "two SO(2) circles, one per Cartan-dual face" identification I put in K735/packet is **RETRACTED** — Cartan duality shares the SAME K (one SO(2), not one-per-face); I conflated K730/K731's "two faces" with Casey's "two spheres." **Number c²=rank + Killing support UNTOUCHED; only the geometric source was flawed.** WHERE the two isomorphic spheres live is **OPEN (Lyra's rep-theory)** — candidates: (1) rank-2 polydisk's two S¹'s [but ρ-weights 5/2,3/2 unequal — the K734 tension, must check not assume]; (2) SO(2) charge circle + a U(1) in the compact-dual color torus (SO(7)⊃G₂⊃SU(3); color is NOT in SO(5)) — Keeper's best guess, still a guess; (3) other. Do NOT bank any candidate. **3 bars:** target-innocence (the 2 must come from geometry, not back-solved) / independent Killing recompute / Five-Absence (not a GUT — 3/13≠3/8). Owners: Lyra (derive the source + (1,1) forcing), Elie (verify vs 3 bars), Grace (render-on-landing, staged), Cal (target-innocence at landing), Keeper (audit + synthesize). Source of truth: K736 + INFO PACKET (corrected).
