---
title: "BST Working Paper — Volume 2: The Framework — Chapter 2: Standard Model"
volume: 2
volume_title: "The Framework"
chapter: 2
chapter_topic: "Standard Model"
parent: "./INDEX.md"
...
*[Corrected in place 09:14 2026-09-14 (Grace, R144 G2, on Lyra L1 and K1687): this identification is RETIRED. $J_{56}$ generates the $\mathrm{SO}(2)$ centre of $K$, which is the clock (Lecture 4, T2629); hypercharge lives in the Cartan of the internal $\mathrm{SO}(5)$ factor, not on the centre (K1687; K1689/T2346; Lecture 5). The Killing-norm ratio and $\sin^2\theta_W = 1/3$ below fall with it. The sixth site of the K1687 mislabel; the computation is kept as the May record.]*
...
The standard harmonic-oscillator shell model reproduces all seven observed magic numbers once given a spin-orbit strength; BST's contribution is a *consistent factorization* of that strength as $\kappa_{ls} = C_2/n_C = 6/5$ — a real consistency, not a unique forcing of the magic numbers (the per-number closed forms are post-hoc, per Cal's K601/#286 re-tier). The durable predictive result is the 8th magic number, 184, testable in superheavy element experiments.
...
**Factor 1: the Bergman kernel power — corrected 2026-09-11.** The Bergman kernel of $D_{IV}^5$ is $K(z,w) \propto N(z,w)^{-p}$ with $p$ the genus, and the genus of the Lie ball in five dimensions is $5$ (Xiao–Yuan; pinned to the page 2026-09-09, and checked by Monte Carlo on the domain in Round 130). The May text wrote the power as $n_C + 1 = 6$; that is the *unit ball's* exponent — Elie's toy 5752 (Round 142) measures $6.000$ on $B^5$ and $5.0003 \pm 0.0006$ on the Lie ball in the same run, with the reproducing property holding at $p = 5$ and failing at $p = 6$ by $60$–$80\sigma$ — a rank-one number in a rank-two slot, the same species as T1918. The arithmetic below that used $6$ as "the Bergman power" is an identification whose label is now wrong; Grace's 29-row genus sweep (R140) lists the affected rows. It controls the weight of every mode on the domain and appears throughout the BST structure (Wyler formula for $\alpha$, fermion mass ratios, $\Lambda$ derivation). It counts the power of the volume form in the Bergman measure.
...
- **Derived:** the skeleton (T2519); flavour universality as the partial-isometry condition; $\lambda = 1/\sqrt{20}$, blind (T2530 on T2529); CP existence, both sectors (T2547, T2536); the *order* of the corner (K1808/K1810); $\theta_{23}$ maximal (T2534).
- **Identified:** $\theta_{12} = 3/10$; $\theta_{13} = 1/45$ (K1260, 2026-08-07, both forms — a ruling the tier map never absorbed until Cal Section 959; Elie's sibling count N = 4 at the row's tolerance); the $4/7$ departure of $\theta_{23}$; $|V_{cb}|$'s position.
- **Fired and lost, certified:** five sealed series for the corner's value (K1800/E6).
- **Open** (the register's word; "input" is reserved for the two dimensionful scales): the corner's value; $\delta_{CP}$'s magnitude; $A$ — taken from measurement where used.
...
| $\|V_{cb}\|$ | $A\lambda^2$: **IDENTIFIED (the register's word; position $A\lambda^2$, value input)** (the old 0.044 retired at $+9.2\sigma$ vs current exclusive $(39.77 \pm 0.46)\times 10^{-3}$; the inclusive/exclusive split is $>3\sigma$ and unresolved) | — | see left | — |
