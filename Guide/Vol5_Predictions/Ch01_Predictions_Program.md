...
To regenerate a number without trusting us: `python3 play/verify_bst.py` runs the registry's verification set; `python3 play/toy_bst_explorer.py verify T187` checks one theorem; `bash play/reproduce_A9_run1.sh` takes A9's run 1 from the catalogue download to the letter, under the frozen protocol (Keeper K1917). Every toy named below is in `play/` with its pre-registration in `notes/`.
...
**A6 — the lightest neutrino is massless.** The bet: m₁ = 0, reached by two routes — the ν = 0 Wallach point (F619) and the k = 0 no-Higgs channel (toy 5461) — one bank until they are reconciled. Current: cosmology bounds Σm_ν from above; m₁ = 0 under normal ordering sets the 58.2 meV floor from below (T2563 — the same number N7 carries). Kill: a measured m_lightest > 0. Owner: Lyra, with Elie on the channel route.

**A7 — Koide's Q = 2/3.** Conditional-forced (K1749; the current statement is K1830's floor ruling); toy 5100 region-match. Current: 0.6666605 on PDG masses. Kill: an improved τ mass moving Q off 2/3 beyond the combined errors. It sits here, among the live rows, and not among the floors of Section 3, because its kill is a measurement, not a theorem: the floor ruling fixed the claim's wording, and the row waits on data. Owner: Elie.
...
*The protocol.* Pre-registered v1.5.2, SHA256 8013d959…, Cal Section 974, verified from the files by Keeper (K1909); the target excluded from the run's inputs. *Run 1.* Quaia (G < 20.5 primary, G < 20.0 alongside), the published selection function as fallback weights: **Landing C — not decidable — on both samples.** Elie's record (step vi), Keeper's instrument (vii, K1910), Cal's cold read (viii, Section 976), Keeper's certification (ix, K1911, 2026-09-17). The reason is on the run's own face: the protocol's profile-alternative trigger fired at +7.5σ and +5.0σ against the pre-registered bar of 2 σ_β (protocol v1.5.2, Section 4.4), and the per-bin residual test failed in three of four non-null bins. The per-bin moments rise with redshift, which a boost does not do and clustering does not do; under the fallback weighting that is a residual selection systematic, caught three ways with the target read last.
...
**E3 — the commit-Boltzmann mass carrier.** "Commit-Boltzmann": a single-temperature Boltzmann weight e^(−E/T) on the three strata's commit energies E = ν², read as the lepton mass ratios; "anti-alignment" (E2): the mass ladder and the commit-energy ladder run in opposite directions. The bet: a parameter-free 16/9 against PDG, with the fail line frozen at 5.00 %. Measured 1.88901 (PDG 2024) against 1.77778: a 5.90 % miss, robust across the full m_τ band (toy 5499, 8 of 8; Keeper-certified 2026-08-25). Died: the single-temperature Boltzmann reading of the anti-alignment — the simplest carrier, eliminated with a number. Survived: the anti-alignment order itself, still standing and still needing a carrier.
...
**F2 — the thermostat closes as a structure; the exponent is fixed, and at that value the form is de Sitter's.** Λ/Λ_P = (t_K/t_P)^{2p/(2−p)}, the response coefficient entering legitimately at a pre-committed fork; the mismatch power p was the one free thing. It is fixed: p = 1 by the Peirce weight — the residual scales under the Jordan quadratic representation with weights 2, 1, 0 on the time line, the spatial three-space and the dropped direction, and the form's domain (0, 2) contains exactly the middle one (Lyra, 2026-09-21; K1919). At p = 1 the exponent is 2, and with the tick defined as the time to begin a commitment — Landauer's bit at the horizon temperature — the form reads Λ/Λ_P = (H t_P)², which is de Sitter's Λ ∝ H²: the Friedmann relation Λ = 3H²Ω_Λ in commitment language, a consistency of the form with general relativity, not a value of Λ (K1919 Sections 6–7). The value limb is closed: it would require deriving H from the five integers, and no tick in the corpus does; H is measured because it changes. T2405's α^36 tick was fitted to a number that was never a time and is re-labelled (K1919 Section 1). The open object is the ledger's filling law — how the committed count's fraction of the horizon count grows with the scale factor, equivalently δ in S ∝ A^δ; a constant fraction gives w = 0 and is dead on sight, δ = 1 exactly is ΛCDM. The conditional cap (K1057 lineage) stands on the form.
...
- **Open / input:** the CKM corner's value (A8, E6); the ledger's filling law (F2's door; p itself is fixed at 1); δ_CP's live claim (C1).
...
*Last accuracy-synced: 2026-09-22 / register v0.21 / K1916 reader's notes applied / K1919 Sections 6–7 (F2).*
...
- **"Dark energy $w \neq -1$ — Prediction confirmed in direction" (43.7 Summary row; 43.5 row 6) — withdrawn as a confirmation (Keeper, 2026-09-25, on K1921).** The register's direction is A1's: w relaxes to −1 from above, monotone, no crossing (F799, T2559 — in CPL, $w_a > 0$). DESI DR2's best fit has $w_a < 0$ and a crossing: that is a tension with the register's row, not a confirmation of it. The magnitude is not "open" in the May sense either — it waits on the ledger's filling law (F2's door), whose direction is to be printed blind.
...
*Read before the table: the ✓ column is a match column, not a tier column, and "Established" is the May program's word. A row here can be −2σ on the register (A2 above) and ✓ here; the register's sentence governs. Tier words for the 26 primaries are on `data/bst_26_tier_table_generated.md` only.*
