...
**Protons.** *[Re-scoped 2026-08-22, Grace — the (A2)/#108 correction reaches here: separate two things this passage conflated. **(1) The winding STRUCTURE is geometric:** the three wall representations of $\mathfrak{so}(7)_2$ have fractional conformal weights $h = N_c/g = 3/7$, $n_C/g = 5/7$, $C_2/g = 6/7$; physical (asymptotic) states require closed orbits on $Q^5$ under the SO(2) fiber action, and a $\mathbb{Z}_3$-incomplete winding cannot close. This is the geometric content, and it is the same statement as the (A1) boundary theorem (no $\lambda_2 > 0$ K-type reaches the Šilov boundary — a two-row-sector fact, no "colour" word). **(2) The SU(3) confining DYNAMICS is IMPORTED, NOT derived from topology:** per #108, SU(3) (and its centre $\mathbb{Z}_3$) is not in the geometry's internal algebra $A_F = \mathbb{C}\oplus\mathbb{H}\oplus\mathbb{R}$; and no geometry-symmetry (SO(5)-equivariant) operator can confine the frame-dependent colour triplet (Elie 5433/5434). So the earlier claim "colour confinement is topological, not dynamic" OVERCLAIMED — the triplet-confining force is the imported gauge dynamics. The geometry gives the winding structure + the (A1) boundary; the confining dynamics is imported.]*

The winding structure is enforced by the topology of $Q^5$; the primality of $g = 7$ makes the winding-completeness sharp — no intermediate closure points exist (if $g$ were composite, partial windings could close). This geometric constraint does not require a thermodynamic arrow. But it is the *necessary geometric condition* on asymptotic states, not the confining force itself.

**Theorem (Proton Persistence — restated to what is derived).** *The proton is **topologically STABLE**: its total winding $3 \times (3/7) = 9/7$ is a conserved topological charge (winding number, $\pi_2$), so the proton does not decay — this is the Five-Absence "no proton decay" prediction (no outside-winding / $D_{IV}^5$ irreducibility), and it IS geometric. Distinct from it: colour CONFINEMENT (why the constituent quarks are not free) rides on the imported SU(3) gauge dynamics, not on topology. Persistence through interstasis follows from the topological STABILITY (winding conservation), which stands.*
...
**Bound nuclei.** Bound nucleons share the proton's topological STABILITY (winding-number conservation); the residual strong force that binds them is imported strong dynamics (as is colour confinement, #108), not derived from the $\mathbb{Z}_3$ winding structure. What is geometric is the *stability* of the nucleon windings; bound nuclei persist through interstasis on that stability.
...
| Proton ($p$) | Winding-number STABILITY ($g=7$ prime; no proton decay, Five-Absence) | **Absolute** | Topological stability (colour-confinement *dynamics* is imported SU(3), #108) |
