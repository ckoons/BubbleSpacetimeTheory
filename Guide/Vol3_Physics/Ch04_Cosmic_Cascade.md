...
*(Tier note, honest as of 2026-08-14.* Two claims live in this paragraph and only one is settled. **Given a four-dimensional manifold**, the Einstein equation as an equation of state is Jacobson's theorem, and BST supplies the microscopic degrees of freedom — that part is not a conjecture. But **the emergence of the smooth 4D manifold itself from the discrete commitment order** — the continuum limit of the causal set — is a genuine open problem, open for every discrete approach to gravity, not only BST. BST is better-positioned than most (its order is *forced*, not randomly grown; it escapes the Kleitman–Rothschild pancake; its tangent signature is (3,1)), but the limit is not yet solved. The 5→4 reduction sits at framework tier — a structural posit, exactly as general relativity posits its manifold — with the continuum limit named as the open frontier. This ontological picture is sound; the derivation of the manifold from the order is honestly unfinished.)*
