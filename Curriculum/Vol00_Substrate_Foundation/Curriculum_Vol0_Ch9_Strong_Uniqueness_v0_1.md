...
**What changed since this chapter was written (2026-09-11, Keeper).** In September 2026 we audited the eleven-criterion theorem below and its null-model probability, and we no longer stand behind them (K1889, Cal Sections 939–946). Four of the criteria were satisfied by construction and could not have failed; several were the same criterion counted twice, because at complex dimension five the classification leaves three candidates and the rank criterion then has one to choose from; and a probability was the wrong instrument where an enumeration was available. What survived is smaller and better: **among all irreducible bounded symmetric domains of rank at least two, $D_{IV}^5$ is the unique one with characteristic multiplicity $a = 3$** — a theorem read off Cartan's classification and verified three independent ways. The multiplicity is *identified* with the number of quark colours; that identification is the program's one dimensionless input, and we have proved the geometry cannot supply a mechanism for it (the group it carries on the relevant space is $U(1)\cdot SO(3)$, dimension four, not $SU(3)$). So "forced with zero free parameters" is retired; the honest sentence is "selected by one measured integer, named." The current treatment is Lecture 2 of the Spine (the Spine, `Curriculum/Spine_DIV5_QM_GR_SM/`). The text below is kept as the May record; read it knowing which of its criteria fell.
