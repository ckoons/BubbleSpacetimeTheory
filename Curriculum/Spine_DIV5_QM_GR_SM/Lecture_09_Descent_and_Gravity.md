...
What that honesty buys is a falsifier in *shape*, Cal's: **$\varepsilon = 0$ frame agreement** — if the descent is induced consistently, the frame picked out by radiation and the frame picked out by matter must coincide, and an off-slice baseline $\varepsilon \neq 0$ refutes the picture. We state it at its current strength: **structural until the observable is named** — which frame from radiation (the CMB dipole?), which from matter (the local flow?), measured how, against what current bound. A falsifier without an instrument is a sentence; the register carries it as a Section-D marker, not a live row (Grace, v0.7, 2026-09-13), and names the candidate observable so the next writer does not start from zero: the **Ellis–Baldwin test** — the dipole of distant matter (quasars, radio sources) against the CMB dipole. The primary already reads strongly: Secrest et al., *ApJL* **937** (2022) L31 find the matter dipoles exceed the CMB-frame kinematic expectation at $5.1\sigma$ jointly in amplitude, with the directions agreeing. So the map decides the verdict before any new measurement does — if the program's $\varepsilon$ is the amplitude mismatch, the literature reads as a *fire*; if it is the direction, as *agreement*. Lyra and Cal own that map; until it is written, this is the most consequential unnamed observable in the Spine. An earlier draft of this paragraph called it "the one genuine laboratory-shaped falsifier in this lecture"; Cal's second pass is right that it is not yet.
