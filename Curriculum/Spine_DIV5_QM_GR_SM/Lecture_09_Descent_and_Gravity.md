...
What that honesty buys is a falsifier in *shape*, Cal's: **$\varepsilon = 0$ frame agreement** — if the descent is induced consistently, the frame picked out by radiation and the frame picked out by matter must coincide, and an off-slice baseline $\varepsilon \neq 0$ refutes the picture. For three weeks $\varepsilon$ had a name, a value and no host: three corpus sentences carried it and none put it in a formula (Lyra, Round 143 — the finding came first). It has one now, ruled on the rows as written (Cal, Section 957). After the descent a rest frame is a unit timelike vector — a point of $\mathrm{SO}_0(3,1)/\mathrm{SO}(3)$, exactly the velocity T2565 says the geometry cannot supply. Radiation's frame is where the photon flux has no dipole; matter's frame is where distant source counts have no dipole, read through Ellis–Baldwin's $D = [2 + x(1+\alpha)]\beta$. **The program's $\varepsilon$ is the rapidity between those two rest frames, each read as the $\ell = 1$ moment of a sky scalar on the induced slice; $\varepsilon = 0$ means one boost, not two.** That is the only reading P3 and T2565 license. The observable is the **Ellis–Baldwin test** (1984): the dipole of distant matter — quasars, radio sources — against the CMB dipole. The primary, pinned from the PDF: Secrest et al., *ApJL* **937** (2022) L31 find amplitudes about $3\times$ and $2\times$ the kinematic expectation and directions $45°$ and $26°$ off the CMB dipole — outside the $95\%$ positional uncertainties — a joint $5.1\sigma$ against "purely kinematic at 370 km/s," and the authors' own conclusion is intrinsic anisotropy, $48°$ away. (An earlier draft of this paragraph said "directions agreeing"; it was wrong, from a brief rather than the paper.) **Under the ruled reading the measurement is not decidable:** reading the matter frame off the dipole presupposes the dipole is purely kinematic, the paper itself rejects that, and that rejection is precisely the escape hatch Cal's pre-registration of 2026-08-21 named before the data. The amplitude excess *would* fire at $5.1\sigma$ — on a reading no row makes; it falsifies the cosmological principle, which is Secrest's title, not our row. The decider is a different measurement: the redshift dependence of the dipole. So the register carries $\varepsilon$ as a live row in A1's shape — observable, current state, presupposition on its face, kill condition a confirmed kinematic mismatch surviving the hatch (A9, registered 2026-09-14 — falsifier register v0.8; ruling Cal Section 957). An earlier draft called this "the one genuine laboratory-shaped falsifier in this lecture"; now it is one, and it is not yet fired.

**Where that stands as this is written (2026-09-14, 11:26).** Prior art, found the day the test was registered and not after: Wu & Xia, "Measuring peculiar velocity and tomographic redshift dipole with DESI DR1 catalogs," arXiv:2608.30914 (31 Aug 2026), measure the boost by the *redshift-dipole* method — the Doppler modulation of observed redshifts — and report for the high-redshift quasar sample $v = 357.95^{+55.05}_{-48.47}$ km/s, in agreement with the CMB-inferred $369.82 \pm 0.11$. That is P1's Landing A on a different catalogue by a different estimator, two weeks before our file; so the test is registered as a **replication with an independent estimator**, not a first test (K1898). The pre-registration (Cal, frozen v1.1, SHA256 `92a7ebb3…`; gate opened K1899) fixes, before any catalogue is opened, the catalogue (Quaia; Storey-Fisher et al. 2024, *ApJ* **964**, 69), the mask, the bins, the per-bin factor $f_i = 2 + x_i(1+\alpha_i) + B_i$ with von Hausegger–Dalang's observed-redshift boundary term, and a three-way partition of every outcome written as code — **A** one boost, **B** a discrepancy that survives the hatch (the row fires), **C** no decision at this depth. The two channels are the two factors of a record (Lecture 4, Theorem B): the count dipole is the Rac factor's $\ell = 1$ moment, the redshift dipole the clock factor's. The one thing the synthetic controls taught before the sky was opened is that you must *window one factor and read the other*: a sample windowed on observed redshift and then read on redshift cancels its own boost at first order — the window moves with what it reads, the same mechanism as $B_i$ — so the redshift channel is windowed on flux and the two channels are fitted for one shared boost (Elie 5759; K1900; Lyra R148). At Quaia's depth the count channel alone cannot decide (its synthetic $\sigma \approx 443$ km/s against the pre-registered 185 km/s bar); the joint fit's expected power, computed before its own run, is near 160 km/s (Elie 5760, pre-registration). **No catalogue has been opened as this paragraph is written. It will be rewritten and dated when the landing is certified, whichever letter it is.**
...
A confirmed kinematic boost mismatch between the radiation and matter rest frames that survives the pre-registered hatch — the redshift dependence of the matter dipole is the decider (A9). A forced $p$ outside $(0, 2)$. A $G$–$m_e$ relation off $0.065\%$ once $\alpha$'s running is accounted at the stated scale. A native spin-2 dynamics exhibited without breaking equivariance or antisymmetry — which the theorems say cannot happen, so an exhibit would be a discovery about the theorems.
