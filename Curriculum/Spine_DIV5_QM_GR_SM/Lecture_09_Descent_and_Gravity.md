...
Start from the isometry group $\mathrm{SO}(5,2)$ and ask what a Lorentzian four-space inside the object would have to be. The answer is constrained by the Jordan structure. The step down is a **single idempotent**: a projection $P$ with $P^2 = P$, applied once — there is no second step, because applying it again does nothing. It has **codimension one** — it drops exactly one dimension. And the signature of what it leaves is **$(3,1)$**: three directions of one sign, one of the other, read off the long root of the root system (T2543/T2545). The three spatial directions are the middle Peirce space of Lecture 2 — the *real* three-dimensional $\mathrm{SO}(3)$ vector, irreducible by Jordan structure theory — and the one time direction is the centre's.

That is the structure, and it is derived. One repair rides with it: T2545's argument for the $(3,1)$ count was first written from a real-space premise that turned out not to be the right ground; the conclusion is unchanged, and the reason is now the irreducibility of the real vector with a Frobenius–Schur argument that it is not any $SU(3)$ fundamental (Grace's fix, Round 141).
...
The physical Hilbert space is $H^2(D_{IV}^5)$ — the holomorphic discrete series, on which $G = \mathrm{SO}_0(5,2)$ acts; that was the lever that decided the question between the Bergman and Hardy spaces (W1). On $H^2$ directly, locality and the modular structure carry over *verbatim*: the Tomita–Takesaki modular theory, the Bisognano–Wichmann relation between the modular group and boosts, and the transport of locality along it — standard theorems, cited and not claimed. The Szegő projection onto the boundary is exact (T349). So **the foundation clears** — this is the cell the program calls G3.
