...
Then two of us computed the exponents independently, numbers unseen (K1827/K1828; toys 5497/5498 and Grace's cross half). The results were identical to the digit: $E = (0,\ 9/4,\ 25/4) = \nu^2$ for the three strata, under both weight conventions — the squares of $0$, $3/2$ and $5/2$, the three named points on the weight ladder (the Wallach floor, the singleton weight, the Hardy point), which is why the exponents are what they are (Lyra, Round 142). And that is the **exact strict reverse** of the frozen order. The mechanism did not miss by a little; it produced the opposite of what it had to produce, on first contact with numbers, after surviving every prior audit.
...
**Koide's relation** (K1830). $Q = 2/3$ for the charged leptons is one of the prettiest empirical facts in the sector, and the program's overlap-norm route to it is closed permanently: the norms at the forced addresses are *uniformly under-hierarchical* — the relation demands a max/min amplitude ratio of $22.96$, and every finite form delivers between $1.79$ and $2.78$, all eight evaluations missing high (Elie 5454). Reopens only on a non-overlap-norm object class. (Two statements about Koide that look opposed and are not — Keeper's ruling on Cal's second pass: the $45°$ is free *as evidence for the mechanism*, so matching it proves nothing about the program's source; and the program's projection-invariant $Q = 2/3$ is conditional-forced (K1749; K1830), so the falsifier register keeps it live as row A7 — an improved $\tau$ mass moving $Q$ off $2/3$ beyond combined errors would kill the identified form. A7 falsifies the *form*; it is not evidence for the *source*.) One further honesty: the $45°$ in Koide's parametrisation (which is $Q = 2/3$ itself) is *free* — any right-shaped amplitude yields it — so matching it proves nothing; the only informative angle is the $\sim 12.7°$ that says *which* masses, and that is read off the data.
...
- **Derived:** the count of strata, as the rank read twice; $m_1 = 0$; the $\nu_R$ degeneracy.
- **Identified:** that the three strata *are* the three generations (F588).
...
A fourth generation would falsify the count outright, and it is excluded to high confidence by the $Z$ width — a falsifier of exactly the kind we prefer, sharp and not ours to tune (no register row; the exclusion is standard). A measured lightest-neutrino mass above zero breaks the corollary (register A6). An improved $\tau$ mass moving Koide's $Q$ off $2/3$ kills the identified form (register A7). The floors cannot be falsified; they can be *opened*, and each door above says by what.
