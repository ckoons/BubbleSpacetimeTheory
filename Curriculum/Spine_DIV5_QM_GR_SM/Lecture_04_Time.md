...
- **The record space and the particle space — two theorems and an identification (Lyra L2, Cal Section 960, 2026-09-14):** (A) there is no nonzero $G$-equivariant map in either direction between $H^2$ (weight $5/2$) and any tensor product of singletons — their lowest weights are $\geq 3$, and Schur finishes it; the composites do not sit inside the record space. (B) As $K$-modules, $H^2 \cong \mathrm{Rac} \otimes C$, with $C$ the odd-charge Hardy space of the time circle: the $j$-th winding sector is a Rac carrying clock charge $1 + 2j$ — a record is a singleton with a clock reading; the Rac is what the write chain sees (hence its $j$-blindness), the clock's charge is an integer and invisible to the double cover, so the $-1$ per turn is the Rac's alone; and the Di has no image in $H^2$ (the clause Cal required). The relation is a $K$-equivariant factorisation, not a $G$-equivariant embedding — it exists for an observer at a point and for nobody else, frame-relative in exactly T2565's sense. "Particles $=$ the Rac factor" is an *identification*, carried as a note (row candidate to Grace on Cal's word).
