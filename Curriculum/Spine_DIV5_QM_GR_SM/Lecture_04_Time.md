...
**The arrow is that $J$ is bounded below.** $\operatorname{spec} J \geq E_0 = 3/2 > 0$, so $e^{-\tau J}$ is a contraction, defined for $\tau \geq 0$ and not for $\tau < 0$; the flow has no inverse. That is the arrow of time — not an added postulate, but the statement that the energy operator has a ground state and no ceiling. Energy generates, time counts.
