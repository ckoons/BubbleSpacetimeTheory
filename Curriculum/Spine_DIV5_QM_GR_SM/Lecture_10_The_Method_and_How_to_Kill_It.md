...
**The live falsifiers — the register's Section A, reproduced** (v0.7, 2026-09-13), each with the lecture that owns it and the measurement that fires it: **A1** a genuine stochastic-Λ wobble or phantom crossing — forbidden from the mean and the variance (Lecture 9); **A2** CKM first-row unitarity resolving against unity — the direct sum confirmed short (Lecture 7); **A3** a confirmed sterile neutrino (Lecture 5); **A4** a right-handed $W$ or gauged $B-L$ (Lecture 5); **A5** neutrinos proven Dirac (Lecture 5); **A6** a nonzero lightest-neutrino mass (Lecture 6); **A7** Koide's $Q$ moved off $2/3$ by an improved $\tau$ mass (Lecture 6). **A8** the order of the mixing corner — an order-one $|V_{ub}|/|V_{cb}|$ would falsify the position (Lecture 7; registered 2026-09-13). **Named in the lectures and carried as a Section-D marker, not a live row:** $\varepsilon \neq 0$ frame disagreement — until its observable is pinned; the candidate is the Ellis–Baldwin test, and the literature already reads one way or the other on it (Lecture 9); a forced $p$ outside $(0,2)$ (Lecture 9; the form's own kill condition, F-section); a fourth generation (Lecture 6; standard exclusion, no row); a free coloured state at a detector (Lecture 5; the (A1) theorem's kinematic exposure, no row). A summary lecture is where a reader expects the register reproduced; an earlier draft of this paragraph was four rows off in each direction.
