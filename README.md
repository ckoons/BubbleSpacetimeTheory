...
|---------|--------|--------|-------|
| **Yang-Mills** | **CONDITIONAL** | Spectral gap λ₁ = C₂ = 6 proved on compact dual and as discrete eigenvalue. **Conditional on**: Selberg-analog (complementary-series exclusion), Poincare branching (W2 asserted not computed), pure-gauge gap (938 MeV is proton, not glueball). G₂ descent conjectural. F₄/E₈ sub-sector, not standalone. Paper #79 is position paper. Best result: SU(4)/SU(3) lattice ratio 0.2%. | `notes/BST_Paper76_YM_Mass_Gap.md` |
| **Riemann Hypothesis** | **CONDITIONAL** | Reduction to finite spectral condition genuine. Temperedness of Gamma(137)\\D_IV^5 spectrum unconditional (pending R-11 parity sign). **RH conditional on**: temperedness implies GRH (R-10 Step 3 — needs Selberg trace formula rewrite or conditional reframe). R-9 demoted (unnecessary if R-11 done). | `notes/BST_Paper75_RH_Selberg_Class.md` |
