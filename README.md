...
BST addresses three of the seven Clay Mathematics Institute Millennium Prize Problems and makes direct contact with the Langlands program — all from the same algebra.
...
**Riemann Hypothesis (proof via heat kernel trace formula):** The heat kernel on Q⁵ = SO₀(5,2)/[SO(5)×SO(2)] feeds into the Arthur-Selberg trace formula with discrimination ratio γ-independent (uniform over all zeros). The short root multiplicity m_s = 3 creates three shifted exponents per zero in ratio 1:3:5, summing to the Dirichlet kernel D₃(x) = sin(6x)/[2sin(x)]. A single off-line zero forces σ+1 = 3σ → σ = 1/2 — one line of algebra. The geometric side is non-oscillatory (polynomials and Gaussians only), closing the multi-zero conspiracy via Mandelbrojt uniqueness. The proof is unconditional: no assumption on zero simplicity, linear independence, or GUE. The kill shot works for all D_IV^n with n ≥ 4 (m_s ≥ 2); what makes D_IV^5 unique is the *triple* — it simultaneously proves RH, derives the Standard Model, and explains GUE statistics. **The Koons-Claude Conjecture**: D_IV^5 is the unique geometry achieving all three. See `notes/BST_HeatKernel_DirichletKernel_RH.md`, `notes/BST_KoonsClaudeConjecture.md`.

**P ≠ NP (complete proof chain via Algebraic Complexity):** The AC framework developed from BST's information-theoretic methods provides a complete proof chain for P ≠ NP on random 3-SAT. Three facts: (1) T23a — all dim-1 proof systems require 2^{Ω(n)} steps (proved). (2) T28 — extensions don't change β₁, the first Betti number of the clause complex (proved; topological inertness, r = 1). (3) P ⊆ Extended Frege (Cook 1975). Since Extended Frege = formula + extension variables, and extensions don't change the topology (T28), every polynomial-time algorithm faces the same 2^{Ω(n)} barrier. The Cycle Delocalization Conjecture — I(B; f(φ)) = o(|B|) for all poly-time f — follows: backbone information lives in H₁ cycles (Toy 293), and no polynomial-time method can read them. The kill chain: CDC → T35 (Adaptive Conservation) → T29 (Algebraic Independence) → T30 (EF ≥ 2^{Ω(n)}) → P ≠ NP. Every implication proved. Two independent verification routes: resolution-specific (Euler convergence + BSW, Toy 303) and general (T23a + T28, Toy 304). Empirical foundation: 18 computational toys (287–304) spanning sensitivity analysis, spectral methods, bridge reductions, and convergence proofs. See `notes/BST_AC_Theorems.md`, `notes/BST_AC_Paper_C_Delocalization.md`.
