...
- **General route (conditional, Toy 304):** Three facts — (1) T23a: all dim-1 proof systems require 2^{Ω(n)} steps (proved). (2) T28: extensions don't change β₁ (proved; topological inertness, r = 1). (3) P ⊆ Extended Frege (Cook 1975). The conditional step: does β₁ preservation under extensions imply the same proof complexity barrier for EF? This is a novel claim — T28 shows extensions don't *kill* original cycles, but does not prove EF cannot *indirectly* resolve linking structure through auxiliary reasoning. Status: conditional on topological closure.

The kill chain: CDC → T35 → T29 → T30 → P ≠ NP. Every implication in the chain is proved; CDC itself is conditional for all P. Empirical foundation: 18 computational toys (287–304). See `notes/BST_AC_Theorems.md`, `notes/BST_AC_Paper_C_Delocalization.md`.
...
| Riemann Hypothesis | Heat kernel trace formula on Q⁵; σ+1=3σ → σ=1/2; works for all D_IV^n with n≥4 | proof (95%) |
