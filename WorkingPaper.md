...
| 22 | $a_4$ spherically exact at $n = 5$ | Non-spherical reps contribute $O(t^n)$; $a_4$ clean iff $n \geq 5$ | Spectral | Toy 250 |

The conditions span six equation types — linear, quadratic, factorial, exponential, variational, and numerical — across seven mathematical disciplines. Fifteen are analytic (closed-form equations with $n = 5$ as unique physical root). Seven are structural (group-theoretic, computational, or spectral, verified exhaustively). The 21st and 22nd are qualitatively different and intertwined: the fourth Seeley-DeWitt coefficient $a_4(Q^5) = 2671/18 = 147 + 25/18$ — a Riemannian curvature invariant computable as a spectral inner product $\langle w_4 | d \rangle$ — approximates the fiber packing number $N_c g^2 = 147$ to within $25/18 = n_C^2/(2N_c^2)$, a correction expressible in BST integers. The ratio $a_4 / N_c g^2$ crosses unity uniquely at $n = 5$: for $n = 3, 4, 6, 7$ the ratio is $0.21, 0.45, 2.10, 4.03$ respectively. Moreover, $n = 5$ is the smallest value where $a_4$ is exact from spherical representations alone: non-spherical representations contribute $O(t^n)$ to the rescaled heat trace, so $a_0$ through $a_{n-1}$ are purely spherical. The crossing is clean precisely because $n = 5$ is where the spherical spectrum becomes sufficient. No two conditions share the same proof technique. The probability that twenty-two independent conditions accidentally select the same integer is negligible.
