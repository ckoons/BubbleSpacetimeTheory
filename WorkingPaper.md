...
The 21st condition is now exact: $a_4(n)$ is a degree-8 polynomial with rational coefficients (Toy 256, mpmath 60-digit cascade extraction + Lagrange interpolation over $n = 3, \ldots, 12$). The fourth Seeley-DeWitt coefficient $a_4(Q^5) = 2671/18 = 147 + 25/18 = N_c g^2 + n_C^2/(2N_c^2)$ — a Riemannian curvature invariant computable as a spectral inner product $\langle w_4 | d \rangle$ — equals the fiber packing number $N_c g^2 = 147$ plus a rational correction expressible entirely in BST integers. The ratio $a_4 / N_c g^2$ crosses unity uniquely at $n = 5$: for $n = 3, 4, 6, 7$ the ratio is $0.21, 0.45, 2.10, 4.03$ respectively. The degree pattern $\deg a_k(n) = 2k$ (from $R^k$ with $R \sim n^2$) is confirmed for $k = 1, \ldots, 5$, with leading coefficients $c_{2k} = 1/(3^k \cdot k!)$ — proved for all $k = 1, \ldots, 5$ (Toys 256, 257d). The $a_5(n)$ degree-10 polynomial has all 11 rational coefficients determined exactly, with $c_{10} = 1/29160 = 1/(3^5 \cdot 5!)$ and $c_9 = -1/14580 = -2c_{10}$. All coefficient denominators have prime support $\subseteq \{2, 3, 5, 7, 11\}$.

**Sub-leading ratio theorem** (proved $k = 1, \ldots, 5$): $c_{2k-1}/c_{2k} = -\binom{k}{2}/5 = -k(k-1)/10$. The top two terms of $a_k(n)$ are $n^{2k-1}(n - k(k-1)/10)/(3^k \cdot k!)$. The sequence of sub-leading factors is $0, -1/5, -3/5, -6/5, -2$ — **triangular numbers** $\binom{k}{2}$ divided by $n_C = 5$. The 10 in the denominator is $\dim_{\mathbb{R}}(Q^5)$. **Constant term theorem** (proved $k = 1, \ldots, 5$): $c_0(a_k) = (-1)^k/(2 \cdot k!)$. The polynomial is controlled by two independent structures: **Bernoulli flow** (heat equation discretization, von Staudt-Clausen denominators) and **curvature boundary** (binomial counting of Ricci corrections, sub-leading numerators). Force and boundary condition meeting in one polynomial.
