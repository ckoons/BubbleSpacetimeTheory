...
The Three Theorems and sub-leading ratio theorem are now verified through **$k = 1, \ldots, 11$** — eleven consecutive levels, extending from six. The sub-leading ratio sequence $c_{2k-1}/c_{2k} = -k(k-1)/10$ produces integer values at the **speaking pairs** $k \equiv 0, 1 \pmod{5}$: the ratios $-2, -3, -9, -11$ at $k = 5, 6, 10, 11$ connect to BST integers ($-2$, $-N_c$, $-N_c^2$, $-\dim K_5$). Predictions: $k = 15, 16$ give $-21 = -\dim \text{SO}(7)$, $-24 = -\dim \text{SU}(5)$ — the heat kernel polynomials encode gauge group dimensions through their sub-leading structure.

The 24th condition is **arithmetic tameness** (T538): at $n = n_C = 5$, every prime in the denominators of $a_k(5)$ for $k = 1, \ldots, 12$ is a cumulative Bernoulli (von Staudt-Clausen) prime. Zero polynomial-factor primes appear. The column rule at $n = 5$ only cancels — it never adds. The polynomial-factor primes that appear at other dimensions (66569 at $n = 10$, up to 26 million at $n$ near 33) emerge from spectral sum aggregation (T539), not from any individual eigenvalue or representation dimension. The BST dimension is the one where curvature has zero arithmetic overhead: all primes come from number theory, and some get removed by the geometry. Among all $Q^n$, this tameness property is specific to $n = 5$ (Toy 615, verified $k = 1, \ldots, 12$). Full treatment in the companion paper (BST Arithmetic Triangle, Paper #9).

The Harish-Chandra c-function for $\text{SO}_0(n,2)$ — the same finite Gamma-ratio product used in the Riemann Hypothesis proof (§30.7, Route A, Lemma 5.6) — is predicted to unify both prime sources (T533 revised, T536): the Bernoulli primes from Gamma asymptotics, the polynomial-factor primes from the root multiplicity $m_s = n - 2$ entering Gamma arguments. One finite function, already in the toolkit, explaining all the primes.
