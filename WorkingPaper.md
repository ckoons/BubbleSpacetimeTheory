...
- **Irreducible Complexity = $\ln 2$.** The modular tensor category of the BST quantum group ($\mathfrak{so}(7)$ at level 2) has total quantum dimension $D^2 = 4$ with exactly 4 non-wall primaries, each of quantum dimension 1. This gives $D = 2 = r$ (the rank of $D_{IV}^5$ as symmetric space). The topological entanglement entropy $\gamma = \ln D = \ln 2$ is the irreducible complexity of the universe — the minimum information cost of the substrate's existence, measured in nats. This is universal: $D^2 = 4$ for ALL $B_N$ at level 2 regardless of rank, a theorem about B-type quantum groups, not a numerical coincidence. The Gödel Limit ($19.1\%$) is the upper bound on self-knowledge; $\gamma = \ln 2$ is the lower bound on existence cost. Between them: one bit to exist, one-fifth to know. See `notes/BST_IrreducibleComplexity_Ln2.md` and `notes/BST_CodeMachine_Inevitability.md` §10.
...
- **The spectral zeta function — poles, residues, and the Odd-Zeta Parity Theorem.** $\zeta_\Delta(s) = \sum d_k/\lambda_k^s$ converges for $\operatorname{Re}(s) > 3$ and has simple poles at $s = 5, 4, 3, 2, 1$ with residues proportional to Seeley–De Witt coefficients $A_0, \ldots, A_4$. The $s = 3$ pole has logarithmic divergence coefficient exactly $1/60 = 2/5! = 1/|A_5|$ (verified to 8 figures). **Odd-Zeta Parity Theorem**: at convergent integer values, $\zeta_\Delta(s)$ is a linear combination of ONLY odd Riemann zeta values $\zeta(3), \zeta(5), \ldots$ — even values $\zeta(2), \zeta(4), \ldots$ cancel identically by the anti-symmetry $f_s(-5-k) = -f_s(k)$. **Exact closed forms**: $\zeta_\Delta(4) = (101/18750)\zeta(3) + 349/1875000$ where $18750 = C_2 \times n_C^{n_C}$; $\zeta_\Delta(5) = (49/187500)\zeta(3) + (2/3125)\zeta(5) - 709/58593750$ where $3125 = n_C^{n_C}$; the coefficient of $\zeta(5)$ in $\zeta_\Delta(6)$ is $d_3/(C_2 \times n_C^{C_2})$ — BST integers in every denominator. This parallels and proves the conjecture (Francis Brown) that only odd zeta values appear in Feynman integrals. See `notes/BST_SpectralZeta_PoleStructure.md`.

- **The effective spectral dimension is 6 = C₂ — the Grand Identity.** $Q^5$ has real dimension $d = 10$ but its heat trace $Z(t) \sim t^{-3}$, not $t^{-5}$. The effective spectral dimension is $d_{\text{eff}} = 6 = C_2$. **Grand Identity**: $d_{\text{eff}}(Q^n) = \lambda_1(Q^n) = \chi(Q^n) = C_2(\text{fund}) = n+1$ — four independently defined quantities are the same number. For $Q^5$: all four equal 6. The ratio $d_{\text{eff}}/d = 6/10 = 3/5 = c_5/c_1 = N_c/n_C$ holds universally for all $Q^n$ ($d_{\text{eff}}/d = c_n/c_1$, verified $n = 3, 5, 7, 9$). The **fill fraction** follows immediately: $f = d_{\text{eff}}/(d \cdot \pi) = 6/(10\pi) = 3/(5\pi)$ — three lines, no structural identification needed. The decomposition $10 = 6 + 4$ splits into spectral dimensions (6 = mass gap) and hidden dimensions (4 = physical spacetime from $B_2$ root multiplicities) — the **same** partition as string theory but with **opposite roles**. Only $n_C = 5$ gives $d - d_{\text{eff}} = n - 1 = 4$ spacetime dimensions (8th uniqueness proof). The $a_3$ Seeley–De Witt coefficient is computed exactly: $a_3 = 437/4500 = 19 \times 23 / (N_c^2 \times n_C^3 \times 4)$ (corrected March 16 — Vassilevich's $c_4$ coefficient is wrong, proved by S² test; 3 independent methods converge exactly). The Golay prime 23 and dark energy denominator 19 survive in the cleaner factorization. Also proved: $c_n(Q^n) = (n+1)/2$ for all odd $n$ — the top Chern class IS the color number. See `notes/BST_EffectiveSpectralDimension.md`.

- **Zonal spectral coefficients — $r_5 = 137/11 = N_{\max}/c_2$.** The heat trace expansion $t^3 Z_0(t) = (1/60)[1 + r_1 t + r_2 t^2 + \cdots]$ encodes BST integers at every level: $r_1 = 5 = n_C$ (exact), $r_2 = 12 = 2C_2$ (exact), $r_5 = 137/11 = N_{\max}/c_2$ (exact). The number 137 emerges as a prime from four Bernoulli-weighted terms and appears **only** for $Q^5$ among all quadrics tested ($Q^3, Q^7, Q^9$ give different numerators). The decomposition $137 = n_C^3 + 2C_2 = r_1^3 + r_2$ connects it to the first two coefficients. The Weyl vector $\rho = (n_C/2, N_c/2) = (5/2, 3/2)$ gives $|\rho|^2 = 17/2$; the factor 17 threads through $r_3 = 1139/63 = 17 \times 67/63$ and $r_4 = 833/45 = 17 \times 49/45$. The degeneracy polynomial derivatives $d'''(0) = C_2 = 6$ and $d^{(5)}(0) = r = 2$. **Structure theorem**: for $k \geq 3$, all $r_k$ come from Euler–Maclaurin boundary corrections (integral contributes nothing beyond $r_2$). See `notes/BST_ZonalSpectralCoefficients.md`.

- **$H_5 = 137/60$ — the spectral origin of $\alpha$.** The fifth harmonic number $H_5 = 1 + 1/2 + 1/3 + 1/4 + 1/5 = 137/60$. The numerator is $N_{\max} = 137 = \lfloor 1/\alpha \rfloor$. The denominator is $60 = n_C!/2 = |A_5|$ — the same 60 from the 1/60 theorem. Every harmonic number through $H_5$ has BST content: $H_2 = N_c/r = 3/2$, $H_3 = c_2/C_2 = 11/6$, $H_4 = c_1^2/(2C_2) = 25/12$, $H_5 = N_{\max}/(n_C!/2) = 137/60$. 137 is prime, so $H_5$ doesn't simplify. New derivation route: $N_{\max} = \operatorname{numer}(H_{n_C})$ gives the integer part of $1/\alpha$ from the dimension alone — no Lie theory required. Works for the baby case too: $H_3 = 11/6$, $\operatorname{numer}(H_3) = 11 \approx 1/\alpha(n_C = 3)$. The spectral zeta's $s = 3$ pole encodes both numbers: divergent part gives $1/60$ (denominator), finite part involves $H_5$ carrying 137 (numerator). See `notes/BST_HarmonicNumber_AlphaOrigin.md`.
