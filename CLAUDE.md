...
**SP-31 Substrate-Native Physics Formalism Program (NEW, HIGH PRIORITY, Casey directive Wednesday EOD)**: Derive full Hilbert Space and every physics object physicists understand (besides numbers) from D_IV⁵. **Believability + Provability dual-axis**. 41 sub-items across 8 phases. Tier-1: Hilbert space spec (SP-31-1) + operator zoo completion charge/chirality/number/parity/Hamiltonian/time (SP-31-6) + per-conservation-law theorems 12-15 incl T and C currently NOT EXPLICIT (SP-31-18) + per-integer theorems consolidation n_C/g (SP-31-39) + Strong-Uniqueness v0.6+ extension (SP-31-41). Phases 2-8: time evolution + measurement + decoherence + spin-statistics + gauge connections + Higgs + per-BC theorems + QFT formalism + S-matrix + RG flow + anomalies. Strong-Uniqueness Theorem v1.0 (D_IV⁵ + all consequences uniquely forced) is target endpoint. Anchors and extends Tasks #209 + #228 + #218 + #235 + #266. Filed `notes/SP31_Substrate_Native_Physics_Formalism_Program.md`. Tier-1 starts Thursday; Phases 2-8 multi-month to multi-year. **Strategic framing shift**: from "BST derives 191 constants" to "BST derives the full physics formalism from D_IV⁵; constants are downstream consequences" — the framing that wins physicist conversation.
