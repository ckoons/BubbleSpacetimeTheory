...
TAG = (f'[{KEY} — {ts} {today}, Grace R133 G1 on Cal C1 ({cw}); Keeper CI_BOARD_ROUND_133 L2; Lyra L1/L2 (dilation); Elie E1–E3: '
       'LOST (~10¹²² bits of spatial coordinates) is RE-TIERED from "no structural anchor / destroyed" to "TRACED OUT into the reset\'s Stinespring auxiliary on the ANGULAR sector k — NOT K1860\'s beyond-band sector, which is the WINDING grading m (Cal §916: two sectors, one phrase) — not destroyed"; '
...
if KEY not in t:
    t = t.replace('| LOST | Specific galaxy positions, cluster arrangements, void geometry, all spatial coordinates | Eigenvalue configurations of emergent space; no structural anchor | ~10¹²² bits |',
                  '| LOST → TRACED OUT (R133 re-tier, held until Cal C1: ' + cw + ') | Specific galaxy positions, cluster arrangements, void geometry, all spatial coordinates | Eigenvalue configurations of emergent space; no structural anchor for the new cycle — traced out into the reset\'s Stinespring auxiliary (angular sector k), not destroyed; NOT identified with K1860\'s beyond-band windings (m-sector, Cal §916) nor with the Bekenstein 10¹²² of the cosmic-horizon SURFACE without a separate claim; re-entry is the can-fail | ~10¹²² bits (SIZED on the cosmic horizon; the identification with the auxiliary\'s share is OPEN) |')
...
for coll in ('theorems', 'nodes'):
    for x in gd[coll]:
        if x['tid'] == 1292 and KEY not in x['status']: x['status'] += f' — {KEY} {ts} {today} (Cal C1 {cw}): LOST = traced out into the reset\'s angular-sector auxiliary (not K1860\'s m-band exterior), not destroyed; RECONSTRUCTED = re-entered; re-entry is the can-fail'; c += 1
