#!/usr/bin/env python3
"""
Toy 1395 -- Heat Kernel Cascade Extraction: a_17 through a_20
==============================================================
Uses 38 dps=1600 checkpoints (n=3..40) from Toy 671b to extract
heat kernel coefficients beyond the confirmed k=16.

Target: verify Toy 632 predictions
  - ratio(20) = -38 = -2 x 19 (speaking pair 4)
  - ratio(21) = -42 = -C_2 x g (needs Phase C data)

Column rule (C=1, D=0): c_top = 1/(3^k * k!), c_sub/c_top = -k(k-1)/10

INV-6c: Check a_17-a_20 for column rule and speaking pair extension.
...
# ═══════════════════════════════════════════════════════════════════
# EXTRACTION FUNCTIONS (from Toy 671b)
# ═══════════════════════════════════════════════════════════════════
...
def main():
    ...
    print("=" * 70)
    print(f"Toy 1395 -- Heat Cascade Extraction: a_1 through a_{TARGET_K}")
    print(f"  dps={DPS}, checkpoints n=3..40 (38 dimensions)")
    print("=" * 70)

    # Step 1: Load all checkpoints
    print(f"\n--- Loading {DPS}-digit checkpoints ---\n")
    ALL_DIMS = list(range(3, 41))
    trace_data = {}
    for n in ALL_DIMS:
        ...
        if KNOWN_POLYS.get(k):
            ...
            if len(p) > deg:
                ...
                if is_integer:
                    ...
                    # Check against formula: should be -k(k-1)/10
                    expected = -k * (k - 1) // 10
    ...
    # T1: All levels through k=16 should match known values
    ...
    # T5: a_17 extracted
    t5_pass = max_confirmed >= 17
    score.append(("T5", f"a_17 extracted: {'YES' if t5_pass else 'NO'}", t5_pass))

    # T6: a_18 extracted
    t6_pass = max_confirmed >= 18
    score.append(("T6", f"a_18 extracted: {'YES' if t6_pass else 'NO'}", t6_pass))

    # T7: a_19 extracted
    t7_pass = max_confirmed >= 19
    score.append(("T7", f"a_19 extracted: {'YES' if t7_pass else 'NO'}", t7_pass))

    # T8: a_20 extracted
    t8_pass = max_confirmed >= 20
    score.append(("T8", f"a_20 extracted: {'YES' if t8_pass else 'NO'}", t8_pass))

    # T9: Speaking pair 4 (k=20 ratio = -38)
    t9_pass = speaking_pairs.get(20) == -38
    score.append(("T9", f"Speaking pair 4 (k=20 ratio -38): {'CONFIRMED' if t9_pass else 'PENDING'}", t9_pass))

    # T10: Full speaking pair table
    print(f"\n  Speaking pair analysis:")
    print(f"  {'k':>3}  {'ratio':>6}  {'formula':>8}  {'BST reading'}")
    print(f"  {'---':>3}  {'------':>6}  {'--------':>8}  {'───────────'}")
    for k in sorted(speaking_pairs.keys()):
        ...
        elif r == -9: reading = "-N_c^2"
        elif r == -11: reading = "-(2n_C+1)"
        elif r == -21: reading = "-C(g,2)"
        elif r == -24: reading = "-dim SU(5)"
        elif r == -38: reading = "-2 x 19"
        elif r == -42: reading = "-C_2 x g"
        print(f"  {k:>3}  {r:>6}  {formula:>8}  {reading}")

    all_sp_match = all(speaking_pairs.get(k) == -k*(k-1)//10
                       for k in speaking_pairs if k*(k-1) % 10 == 0)
    t10_pass = len(speaking_pairs) >= 6 and all_sp_match
    score.append(("T10", f"All speaking pairs match formula: {all_sp_match}", t10_pass))
    ...
    results_path = os.path.join(CKPT_DIR, "cascade_results.json")
