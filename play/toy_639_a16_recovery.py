#!/usr/bin/env python3
"""
Toy 639 — a₁₆ Polynomial Recovery: Push to ELEVEN Consecutive Levels
=====================================================================
Extends Toy 622's pipeline to recover the 16th Seeley-DeWitt coefficient
a₁₆(n) as a polynomial in n of degree 32.

Key parameters:
  - Degree 32, Three Theorems constrained → 30 unknowns
  - 33 checkpoint dimensions (n=3..35) → 3 verification points (TIGHT)
  - QUIET predicted: 2k+1 = 33 = 3×11 NOT PRIME → no new prime enters
  - c_top = 1/(3¹⁶ · 16!), ratio = -24 = -dim(SU(5)) = -(n_C²-1), c_const = +1/(2·16!)
  - VSC: (p-1)|32 for p ∈ {2,3,5,7,11,17}. No new prime.

CRITICAL TEST — Two predictions at once:
  1. No new prime enters denominator (QUIET level prediction)
  2. Sub-leading ratio = -24 = -dim(SU(5)) = -(n_C²-1)
     SU(5) GUT group dimension readout (Grace Prediction #12 sequel)
...
Pipeline:
  ...
  7. Validate: Three Theorems + quiet level + eleven-level migration

If successful: ELEVEN consecutive levels (k=6..16) with exact polynomials.
The SU(5) readout level. Tests column rule at 2k+1=33.
...
def main():
    ...
    print("╔══════════════════════════════════════════════════════════════════╗")
    print("║  Toy 639 — a₁₆ Polynomial Recovery                            ║")
    print("║  ELEVEN Consecutive Levels: k=6..16                            ║")
    print("║  QUIET predicted (no new prime enters at 2k+1=33=3×11)        ║")
    print("║  Speaking Pair: ratio = -24 = -dim(SU(5)) = -(n_C²-1)        ║")
    print("╚══════════════════════════════════════════════════════════════════╝")
    ...
    TARGET_K = 16
    TARGET_DEG = 2 * TARGET_K  # 32
    N_NEEDED = TARGET_DEG - 2  # 30
    ...
    if ak_poly:
        ...
        quiet = max_den_p <= 31
        print(f"\n  QUIET level prediction (2k+1=33=3×11 is composite → no new prime):")
        if quiet and max_den_p <= 31:
            print(f"    → ✓ CONFIRMED: no new prime entered (max prime = {max_den_p})")
        else:
            print(f"    → ✗ SURPRISE: new prime entered (max prime = {max_den_p}, expected ≤31)")

        score(f"a_{TARGET_K} QUIET level (no new prime)", max_den_p <= 31,
              f"max prime in den = {max_den_p}")

        # Speaking pair test: ratio = -24 = -dim(SU(5)) = -(n_C²-1)
        ratio_val = ak_poly[deg-1] / ak_poly[deg] if ak_poly[deg] != 0 else None
        print(f"\n  Speaking pair test:")
        print(f"    Sub-leading ratio c_{{31}}/c_{{32}} = {ratio_val}")
        print(f"    Expected: -C(16,2)/5 = -24 = -dim(SU(5)) = -(n_C²-1)")
        if ratio_val == Fraction(-24, 1):
            print(f"    → ✓ CONFIRMED: ratio = -24 = dim(SU(5))")
        elif ratio_val == Fraction(-24):
            print(f"    → ✓ CONFIRMED: ratio = -24 = dim(SU(5))")
        else:
            print(f"    → ✗ ratio = {ratio_val}, expected -24")
        score("Speaking pair ratio = -24 = dim(SU(5))", ratio_val == Fraction(-24),
              f"ratio = {ratio_val}")
        ...
        # Prime migration summary: k=6..16
        ...
        for kk in range(6, TARGET_K + 1):
            ...
            if v5 is not None:
                ...
                if kk == TARGET_K:
                    entry = "QUIET" if mp <= 31 else ("★ NEW" if mp > 31 else "QUIET")
                print(f"    a_{kk:>2}(5): den max prime = {mp:>2}  {entry}  "
                      f"den = {v5.denominator}")

        # VSC analysis for k=16
    ...
    if ak_poly:
        ...
        print(f"  ELEVEN consecutive levels (k=6..16) now exact.")
        print(f"  SU(5) GUT group dimension = 24 tested.")
    else:
    ...
    if FAIL == 0:
        print(f"\n  ALL PASS — a_{TARGET_K} recovered. QUIET level + ratio -24 confirmed.")
    else:
