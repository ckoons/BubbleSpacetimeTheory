#!/usr/bin/env python3
"""
Toy 278 — Enhanced Cascade to a₁₂: Breaking the Wall
=====================================================
Extends the Seeley-DeWitt heat kernel cascade through a₁₂(n) on Q^n = D_IV^n.

Key improvements over Toy 277:
  (1) P_MAX = 1000 (from 750) — more eigenvalues for better convergence
  (2) dps = 400 (from 300) — more headroom for deep cascade
  (3) N_PTS = 48 (from 32) — better Neville extrapolation
  (4) Range n=3..27 (from n=3..25) — 25 data points for degree-24 polynomial
  (5) Robust polynomial recovery with leave-one-out validation
  (6) Pipeline through a₁₁ (Golay prime 23 prediction) and a₁₂
...
mpmath.mp.dps = 400  # 400 decimal digits for 12-deep cascade
...
def main():
    ...
    for n in ALL_RANGE:
        ...
        frac_cf, _ = identify_rational_cf(a12, max_den=500000000000000,
                                           tol=1e-5, max_prime=A12_MAX_PRIME)
        if frac_cf:
            a12_rats[n] = frac_cf
            a12_clean[n] = frac_cf
        else:
            frac_any, _ = identify_rational_cf(a12, max_den=500000000000000, tol=1e-5)
            if frac_any:
                a12_rats[n] = frac_any
