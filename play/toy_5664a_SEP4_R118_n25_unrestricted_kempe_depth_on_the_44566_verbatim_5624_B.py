...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print(f"Toy 5664a (=5624 at n=25, prereg 49670a3b) — unrestricted Kempe depth [{mode}] MAXD={MAXD} CAP={CAP}")
    print("=" * 70)
    t0 = time.time()
    graphs = {}
    ...
    if mode == 'A':
        ...
        kills = [eval(k) for k in json.load(open(os.path.join(HERE, '.in_frame_one_word_n25.json')))['kills']]
    ...
    json.dump(rows, open(os.path.join(HERE, f'.unrestricted_depth_{mode}_n25.json'), 'w'))
