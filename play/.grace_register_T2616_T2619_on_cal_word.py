...
ROWS = {
 'T2616': dict(
  name='Re s = 1/2 is the Mellin-unitarity axis of the dilation group (the critical LINE, not the points on it)',
  ...
  plain='The Mellin transform is an isometry on exactly one vertical line, Re s = 1/2, because that is where the dilation group x -> ax acts unitarily. The functional equation s <-> 1 - s is the reflection in that line, so zeros of any L-function with the functional equation come in pairs symmetric about it. This pins the LINE the zeros are symmetric about; it says nothing about whether they sit ON it (that is RH).',
  proofs=['Mellin–Plancherel', 'Nyman–Beurling'],
  registry=f'| T2616 | RE s = ½ IS THE MELLIN-UNITARITY AXIS (harvest advance 4; K1862 seam i; Round 120 G6; Cal {calword}): the Mellin transform (Mf)(s) = ∫ f(x) x^s dx/x is a unitary isomorphism L²(ℝ₊, dx/x) → L²(c + iℝ) exactly for c = ½ (Plancherel; the half-density weight), i.e. Re s = ½ is the axis on which the dilation group ℝ₊ acts unitarily — the self-dual point of the rank-one dilation sector\'s principal series inside SO₀(5,2)\'s (F694/F988). Consequence: the functional-equation involution s ↔ 1 − s is the reflection in this axis, so the zero set of any L-function with that functional equation is SYMMETRIC ABOUT Re s = ½. SCOPE (K1508 ceiling, verbatim in substance): this pins the LINE, not the POINTS; RH is the strictly stronger statement that the zeros lie on the axis; nothing here touches Weil positivity. Target-innocent: the axis is fixed by unitarity before ζ is named. Inputs: T2562 (the derived dilation operator; operator class = coordinate shifts on a Hardy space, K1862 C.2), F988 (Lyra: s ↔ 1 − s is a frame reflection, not time-reversal). NO edge to K21 (retracted; F988\'s firewall corrected 2026-09-06). | DERIVED (classical; the corpus\'s own exercise of the frame: K1862-B Gram instrument, Báez-Duarte rate within 2 %) | graph-node (number theory / RH row) | — | {today} |',
  edges=[(2562, 2616, 'derived', 'the derived dilation operator (operator class: Hardy-space shifts) is the input; the axis is its unitarity axis')]),
 'T2617': dict(
  ...
  status_long=f'PROVED (K1862-C Lemma 1: Hensel-lifting count, validated by convolution at p = 3, 5, 7, e ≤ 4 and p = 11, 13 to N = 300 by Elie 5693/5694; family rule swept by Grace 5698: exact constancy of the definite/indefinite ratio at n = 2, 4, 6, N-dependence at n = 3, 5, 7; Cal\'s word {calword})',
