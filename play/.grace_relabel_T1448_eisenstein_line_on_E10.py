...
if mode == 'B' and not value: value = 'C = −0.10132118·∫h (K1869 §1 H4)'
...
for coll in ('theorems', 'nodes'):
    for t in gd[coll]:
        if t['tid'] in (1448, 1450, 1451, 1461) and 'G18' not in str(t['status']):
            t['status'] = str(t['status']) + (SUF if t['tid'] == 1448 else ' — CONDITIONAL on T1448\'s Eisenstein line (K1869 §2, ' + stamp + ' ' + today + '): the E term is matched not derived; identity/curvature/hyperbolic/mixed terms untouched'); c += 1
