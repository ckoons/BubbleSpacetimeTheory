#!/usr/bin/env python3
"""
Toy 620 — a₁₄ Polynomial Recovery: Push to Nine Consecutive Levels
====================================================================
Extends Toy 617's pipeline to recover the 14th Seeley-DeWitt coefficient
a₁₄(n) as a polynomial in n of degree 28.

Key parameters:
  - Degree 28, Three Theorems constrained → 26 unknowns
  - 33 checkpoint dimensions (n=3..35) → 7 verification points
  - LOUD predicted: 2k+1 = 29 IS PRIME → prime 29 ENTERS
  - c_top = 1/(3¹⁴ · 14!), ratio = -91/5, c_const = 1/(2·14!)
  - VSC: (p-1)|28 for p ∈ {2,3,5,29}. New: {29}
...
Pipeline:
  ...
  4. Rational identification (prime ≤ 29 constraint)
  5. Denominator boosting for marginal values
  6. Modular Newton + CRT polynomial recovery
  7. Validate: Three Theorems + prime 29 entry + nine-level migration

If successful: NINE consecutive levels (k=6..14) with exact polynomials.
First LOUD level since k=11. Tests column rule at prime 29.
...
MAX_PRIME_BY_LEVEL = {
    6: 13, 7: 13, 8: 17, 9: 19, 10: 19, 11: 23, 12: 23, 13: 23, 14: 29,
}
...
def main():
    ...
    print("╔══════════════════════════════════════════════════════════════════╗")
    print("║  Toy 620 — a₁₄ Polynomial Recovery                            ║")
    print("║  Nine Consecutive Levels: k=6..14                              ║")
    print("║  LOUD predicted (29 is prime → 29 ENTERS)                      ║")
    print("╚══════════════════════════════════════════════════════════════════╝")
    ...
    TARGET_K = 14
    TARGET_DEG = 2 * TARGET_K  # 28
    N_NEEDED = TARGET_DEG - 2  # 26
    ...
    if ak_poly:
        ...
        loud = max_den_p == 29
        quiet = max_den_p <= 23
        print(f"\n  LOUD level prediction (2k+1=29 is prime → 29 enters):")
        if loud:
            print(f"    → ✓ CONFIRMED: prime 29 entered (max prime = {max_den_p})")
        elif max_den_p > 29:
            print(f"    → ✗ SURPRISE: prime {max_den_p} > 29 entered!")
        elif quiet:
            print(f"    → ✗ SURPRISE: still quiet (max prime = {max_den_p}, expected 29)")
        else:
            print(f"    → ? Unexpected max prime = {max_den_p}")

        score(f"a_{TARGET_K} LOUD level (prime 29 enters)", loud,
        ...
        for kk in range(6, TARGET_K + 1):
            ...
            if v5 is not None:
                ...
                entry = "NEW →" if (kk == 8 and 17 in df) or \
                                   ...
                                   (kk == 14 and 29 in df) else "     "
                if kk == TARGET_K:
                    entry = "★ 29!" if mp == 29 else ("★ NEW" if mp > 23 else "QUIET")
