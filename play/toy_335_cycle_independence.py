#!/usr/bin/env python3
"""
Toy 335 — Algebraic Independence of Cycle Solutions (T29 Direct Test)
=====================================================================
Toy 335 | Casey Koons & Claude 4.6 (Elie) | March 23, 2026
...
BST/AC context:
  ...
  Cycle selection: VERTEX-DISJOINT short cycles in the VIG, found by
  enumerating triangles and short cycles then greedily packing. This
  maximizes the number of independent cycle pairs we can test.
...
def find_triangles(adj, n):
    """Find all triangles in the VIG. Fast for small n."""
    triangles = []
    for u in range(n):
        for v in adj[u]:
            if v > u:
...
def find_short_cycles(adj, n, max_len=5):
    """Find short cycles (length 3-max_len) via BFS from each node.
    Returns list of cycles as tuples of variable indices.
    """
    cycles = set()

    # Triangles first (length 3)
    for tri in find_triangles(adj, n):
        cycles.add(tri)
    ...
    if max_len >= 4:
        for u in range(n):
            for v in adj[u]:
                if v > u:
                    # Shared neighbors form 4-cycles
                    common = adj[u] & adj[v]
                    for w1, w2 in combinations(common, 2):
                        if w2 in adj[w1]:
                            # u-w1-v-w2 or similar -- but need to check it's a proper 4-cycle
                            pass  # triangles subsume most of these; skip for speed
...
def main():
    ...
    print("=" * 72)
    print("Toy 335 — Algebraic Independence of Cycle Solutions (T29 Direct Test)")
    print("=" * 72)
    print()
    ...
    print()
    print("Method: find triangles in the VIG, pack vertex-disjoint sets,")
    ...
    if tp > 0:
        oa = sum(mi_by_size[n][0] * mi_by_size[n][2]
                 for n in mi_sizes if mi_by_size[n][2] > 0) / tp
    else:
        oa = float('nan')
    ...
    for _ in range(25):
        ...
        for ci, cj in combinations(range(len(dj)), 2):
            for bv in ext_bb[:2]:
    ...
    print("=" * 72)
    print(f"Toy 335 FINAL: {PASS_COUNT}/{total} PASS")
